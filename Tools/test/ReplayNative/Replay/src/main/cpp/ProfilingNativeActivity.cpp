...
// SELECT APP HERE ...
//
// Select the app you want by setting LPGPU2_APP to the class name of the
// app you want.
//
// Options are: Simple,Raymarching,RaymarchingOLT,Overdraw,OverdrawTex,
//              Menger,Hypercube,Carousel,Triangle1,
//              Correctness,LowPower
//
// To write your own app, derive a class from AppRenderer and implement the
// abstract functions.
//
// For an example to follow, study the apps listed in the "apps..." header
// section below
//
// Also, to turn the psuedo asm shader source on off, see the top of AppRenderer.cpp


#define LPGPU2_APP Carousel
...
#include "RaymarchingOLT.cpp"
#include "Overdraw.cpp"
#include "OverdrawTex.cpp"
#include "Menger.cpp"
#include "Hypercube.cpp"
#include "Carousel.cpp"
...
const float   FPSLimit = 100.0; // Note that choreographer caps to screen refresh - e.g. 60fps
const int32_t kFPSThrottleInterval = 1;
const int64_t kFPSThrottlePresentationInterval = (1.0f / FPSLimit) * 1000000000;
...
// SELECT APP HERE ...
//
// Select the app you want to run by reinstating the appropriate line.
// To write your own, derive a class from AppRenderer and implement the
// abstract functions.
//
// For an example to follow, study the apps listed here:

//AppRenderer    renderer_;
//Simple         renderer_;
//Raymarching    renderer_;
//RaymarchingOLT renderer_;
//Overdraw       renderer_;
//OverdrawTex    renderer_;
//Menger         renderer_;
//Hypercube      renderer_;
//Carousel       renderer_;
//Triangle1      renderer_;

  LPGPU2_APP renderer_;
...
void Engine::CheckAPISupport() {
  ...
  if (apilevel >= 24) {
    ...
    if ( eglPresentationTimeANDROID_ == NULL ){
      api_mode_ = kAPINone;
    }
    else {
  ...
  LOGI("Started FPS throttle mode in: %c",fps_throttle_?'T':'F');
  if (fps_throttle_) {
   StartFPSThrottle();
  } else {
   StopFPSThrottle();
...
Java_org_lpgpu_replaynative_ProfilingNativeActivity_choregrapherCallback(
...
void Engine::LoadResources() {
  ...
  context_ .Bind(&tap_camera_);
}

/**
 * Unload resources
 */
void Engine::UnloadResources() { renderer_.Shutdown(); }
...
void Engine::DrawFrame() {
  static int64_t epoch = GetCurrentTime();
  ...
  double t = (GetCurrentTime()-epoch)*1.e-9;

  context_.set_time(t);

  renderer_.Update(context_);//t);//monitor_.GetCurrentTime());

  // Just fill the screen with a color.
  // glClearColor(0.5f, 0.5f, 0.5f, 1.f);
  glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
  renderer_.Render(context_);//t);

  if (renderer_.QuitRequested()) finishing = true;
...
int32_t Engine::HandleInput(android_app* app, AInputEvent* event) {
  ...
  if (AInputEvent_getType(event) == AINPUT_EVENT_TYPE_MOTION) {
    ...
    if (doubleTapState == ndk_helper::GESTURE_STATE_ACTION) {
      ...
      LOGI("Switched FPS throttle mode to: %c",eng->fps_throttle_?'T':'F');
...
void Engine::UpdateFPS(float fFPS) {
  ...
  jni->CallVoidMethod(app_->activity->clazz, methodID, fFPS * AppRenderer::MULTI);
