...
void init_vertex_buffer(struct sample_info &info, const void *vertexData,
                        ...
                        bool use_texture);
void init_index_buffer(struct sample_info& infa, index_buffer_t& index_buffer);
...
void init_shaders(struct sample_info &info, VkPipelineShaderStageCreateInfo* shaderStages, const char *vertShaderText,
...
void init_pipeline(struct sample_info &info, VkPipeline& pipeline, VkPipelineShaderStageCreateInfo* shaderStages, VkPipelineLayout& pipeline_layout,
                   VkVertexInputBindingDescription& vi_binding, VkVertexInputAttributeDescription* vi_attribs,
                   VkBool32 include_depth, VkBool32 include_vi = true);
...
void init_texture(struct sample_info &info, uint32_t binding, const char *textureName = nullptr,
...
void destroy_pipeline(struct sample_info &info, VkPipeline& pipeline);
void destroy_pipeline_cache(struct sample_info &info);
void destroy_descriptor_pool(struct sample_info &info, VkDescriptorPool& desc_pool);
...
std::string load_shader_source(std::string filename, int binding = -1);
