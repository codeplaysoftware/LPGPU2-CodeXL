...
void UniformData::init(sample_info& info, uint32_t b, void* data, size_t num) {
    binding = b;
    ...
    buf_info.size = num; // sizeof(data); //.MVP);
    ...
    buffer_info.range = num; // sizeof(data);

    update(info,data); // (void*)&data);
}

void UniformData::update(sample_info& info, void* data) {
    uint8_t *pData;
    VkResult res = vkMapMemory(info.device, mem, 0, memsize, 0, (void **)&pData);
    assert(res == VK_SUCCESS);

    memcpy(pData, data, memsize);
