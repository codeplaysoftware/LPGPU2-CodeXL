...
struct UniformData {
    ...
    void init(sample_info& info, uint32_t b, void* data, size_t num);

    void update(sample_info& info, void* data);

    VkWriteDescriptorSet descriptor(VkDescriptorSet& ds);

    VkDescriptorSetLayoutBinding layout(VkShaderStageFlagBits stages);

    VkDeviceSize memsize;

    void destroy(sample_info& info);
}; // UniformData

struct TextureData {
    TextureData() = default;
    TextureData(DrawApp& app, uint32_t b, const char *textureName = nullptr, VkImageUsageFlags extraUsages = 0, VkFormatFeatureFlags extraFeatures = 0);

    uint32_t binding = -1;

    VkDescriptorImageInfo image_info;

    void init(DrawApp& app, uint32_t b, const char *textureName = nullptr, VkImageUsageFlags extraUsages = 0, VkFormatFeatureFlags extraFeatures = 0);
...
struct Textures {
    ...
    void add(const TextureData& t) {textures.push_back(t);}
}; // Textures
