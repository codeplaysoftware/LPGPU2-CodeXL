...
void init_uniform_buffer(struct sample_info &info, void* vud, size_t vus, void* fud, size_t fus, UniformData& vert_uniform_data, UniformData& frag_uniform_data) {
 vert_uniform_data.init(info,0,vud,vus); // binding = 0
 frag_uniform_data.init(info,1,fud,fus); // binding = 1
}
...
void init_descriptor_and_pipeline_layouts(struct sample_info &info, std::vector<VkDescriptorSetLayout>& desc_layout, VkPipelineLayout& pipeline_layout,
 ...
 Textures& texture_data
    ...
    layout_bindings.push_back(vert_uniform_data.layout(VK_SHADER_STAGE_VERTEX_BIT));
    layout_bindings.push_back(frag_uniform_data.layout(VK_SHADER_STAGE_FRAGMENT_BIT));

    for(auto& t:texture_data.textures) layout_bindings.push_back(t.layout(VK_SHADER_STAGE_FRAGMENT_BIT));
    ...
    res = vkCreateDescriptorSetLayout(info.device, &descriptor_layout, NULL, desc_layout.data());
    assert(res == VK_SUCCESS);
    ...
    res = vkCreatePipelineLayout(info.device, &pPipelineLayoutCreateInfo, NULL, &pipeline_layout);
    assert(res == VK_SUCCESS);
}
...
void init_vertex_buffer(struct sample_info &info, const void *vertexData, uint32_t dataSize, uint32_t dataStride,
    ...
    buf_info.flags = 0;
    res = vkCreateBuffer(info.device, &buf_info, NULL, &vertex_buffer.buf);
    assert(res == VK_SUCCESS);

    VkMemoryRequirements mem_reqs;
    vkGetBufferMemoryRequirements(info.device, vertex_buffer.buf, &mem_reqs);
    ...
    res = vkAllocateMemory(info.device, &alloc_info, NULL, &(vertex_buffer.mem));
    assert(res == VK_SUCCESS);
    vertex_buffer.buffer_info.range = mem_reqs.size;
    vertex_buffer.buffer_info.offset = 0;

    uint8_t *pData;
    res = vkMapMemory(info.device, vertex_buffer.mem, 0, mem_reqs.size, 0, (void **)&pData);
    assert(res == VK_SUCCESS);

    memcpy(pData, vertexData, dataSize);

    vkUnmapMemory(info.device, vertex_buffer.mem);

    res = vkBindBufferMemory(info.device, vertex_buffer.buf, vertex_buffer.mem, 0);
    assert(res == VK_SUCCESS);
...
void init_index_buffer(struct sample_info& info, index_buffer_t& index_buffer) {
}

void init_descriptor_pool(struct sample_info &info, VkDescriptorPool& desc_pool, Textures& texture_data) {
    ...
    // frag textures...
    for(auto& t:texture_data.textures) {
    ...
    res = vkCreateDescriptorPool(info.device, &descriptor_pool, NULL, &desc_pool);
    assert(res == VK_SUCCESS);
}

void init_descriptor_set(struct sample_info &info, VkDescriptorPool& desc_pool, std::vector<VkDescriptorSet>& desc_set, std::vector<VkDescriptorSetLayout>& desc_layout,
 UniformData& vert_uniform_data,
 UniformData& frag_uniform_data,
 Textures& texture_data
) {
    ...
    res = vkAllocateDescriptorSets(info.device, alloc_info, desc_set.data());
    assert(res == VK_SUCCESS);

    std::vector<VkWriteDescriptorSet> writes;

    writes.push_back(vert_uniform_data.descriptor(desc_set[0]));
    writes.push_back(frag_uniform_data.descriptor(desc_set[0]));

    for(auto& t:texture_data.textures) writes.push_back(t.descriptor(desc_set[0]));

    vkUpdateDescriptorSets(info.device, writes.size(), writes.data(), 0, NULL);
}

void init_shaders(struct sample_info &info, VkPipelineShaderStageCreateInfo* shaderStages, const char *vertShaderText, const char *fragShaderText) {
    VkResult U_ASSERT_ONLY res;
    ...
    if (vertShaderText) {
        ...
        res = vkCreateShaderModule(info.device, &moduleCreateInfo, NULL, &shaderStages[0].module);
        assert(res == VK_SUCCESS);
    }
    ...
    if (fragShaderText) {
        ...
        res = vkCreateShaderModule(info.device, &moduleCreateInfo, NULL, &shaderStages[1].module);
        assert(res == VK_SUCCESS);
    }
...
void init_pipeline(struct sample_info &info, VkPipeline& pipeline, VkPipelineShaderStageCreateInfo* shaderStages, VkPipelineLayout& pipeline_layout,
                   VkVertexInputBindingDescription& vi_binding, VkVertexInputAttributeDescription* vi_attribs,
                   VkBool32 include_depth, VkBool32 include_vi) {
    VkResult U_ASSERT_ONLY res;
...
// Blending enabled...
    att_state[0].colorWriteMask =
     ...
     VK_COLOR_COMPONENT_A_BIT; // (probably = 0xf)
    ...
    res = vkCreateGraphicsPipelines(info.device, info.pipelineCache, 1, &pipeline_ci, NULL, &pipeline);
    assert(res == VK_SUCCESS);
}
...
void destroy_pipeline(struct sample_info &info, VkPipeline& pipeline) { vkDestroyPipeline(info.device, pipeline, NULL); }

void destroy_pipeline_cache(struct sample_info &info) { vkDestroyPipelineCache(info.device, info.pipelineCache, NULL); }

void destroy_uniform_buffer(struct sample_info &info, UniformData& vert_uniform_data, UniformData& frag_uniform_data) {
    vert_uniform_data.destroy(info);
    frag_uniform_data.destroy(info);
}

void destroy_descriptor_and_pipeline_layouts(struct sample_info &info, std::vector<VkDescriptorSetLayout>& desc_layout, VkPipelineLayout& pipeline_layout) {
    for (int i = 0; i < NUM_DESCRIPTOR_SETS; i++) vkDestroyDescriptorSetLayout(info.device, desc_layout[i], NULL);
    vkDestroyPipelineLayout(info.device, pipeline_layout, NULL);
}

void destroy_descriptor_pool(struct sample_info &info, VkDescriptorPool& desc_pool) { vkDestroyDescriptorPool(info.device, desc_pool, NULL); }

void destroy_shaders(struct sample_info &info, VkPipelineShaderStageCreateInfo* shaderStages) {
    vkDestroyShaderModule(info.device, shaderStages[0].module, NULL);
    vkDestroyShaderModule(info.device, shaderStages[1].module, NULL);
}
...
void destroy_vertex_buffer(struct sample_info &info, vertex_buffer_t& vertex_buffer) {
    vkDestroyBuffer(info.device, vertex_buffer.buf, NULL);
    vkFreeMemory(info.device, vertex_buffer.mem, NULL);
}
...
// LPGPU2...
    VkWriteDescriptorSet UniformData::descriptor(VkDescriptorSet& ds) {
        ...
        r.pBufferInfo = &buffer_info;
        r.dstArrayElement = 0;
        r.dstBinding = binding;

        return r;
    }

    VkDescriptorSetLayoutBinding UniformData::layout(VkShaderStageFlagBits stages) {
        ...
        r.descriptorCount = 1;
        r.stageFlags = stages;
        r.pImmutableSamplers = NULL;

        return r;
    }

    void UniformData::destroy(sample_info& info) {
        vkDestroyBuffer(info.device, buf, NULL);
        vkFreeMemory   (info.device, mem, NULL);
    }



    inline VkWriteDescriptorSet TextureData::descriptor(VkDescriptorSet& ds) {
        VkWriteDescriptorSet r = {};
    ...
    inline VkDescriptorSetLayoutBinding TextureData::layout(VkShaderStageFlagBits stages) {
        VkDescriptorSetLayoutBinding r = {};
    ...
    TextureData::TextureData(DrawApp& app, uint32_t b, const char *textureName, VkImageUsageFlags extraUsages, VkFormatFeatureFlags extraFeatures) {
        init(app,b,textureName,extraUsages,extraFeatures);
    }



void TextureData::init(DrawApp& app, uint32_t b, const char *textureName, VkImageUsageFlags extraUsages, VkFormatFeatureFlags extraFeatures) {
    sample_info& info = app.info;
    ...
    image_info.imageView   = info.textures.back().view;
    image_info.sampler     = info.textures.back().sampler;
    image_info.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

    binding = b;

    app.texture_data.add(*this);
}



void recreate_command_buffers(struct sample_info &info) {
...
// For apps like the the Globe app, we need to use the same shader (perhaps many times) utilizing
// a dynamic binding point for the texture. It would be inconvenient to have, say, a sequence of
// shaders called shader1.frag, shader2.frag ... shader100.frag that are all identical apart from
// a hard-wired constant. The following function replaces the fist instance of the string
// specified by the 'match' constant with the supplied integer argument, 'binding'.
// If the 'match' string is matched more than one, the next instance of 'match' is replaced by
// binding + 1, the instance after that is replaced by binding + 1, etc.

std::string load_shader_source(std::string filename, int binding) {
    filename = get_base_data_dir() + filename;

#ifndef __ANDROID__
    FILE *fp = fopen(filename.c_str(), "rb");
#else
    FILE *fp = AndroidFopen(filename.c_str(), "rb");
#endif
    ...
    if (fp) {
        fseek(fp, 0, SEEK_END);
        shader.resize(ftell(fp));
        rewind(fp);
        fread(&shader[0], 1, shader.size(), fp);
        fclose(fp);
    }

    if (binding < 0) return shader;

    const std::string match = "<?>";

    for(auto pos = shader.find(match) ; pos != std::string::npos ; pos = shader.find(match)) {
        std::stringstream s;

        s << shader.substr(0,pos) << binding << shader.substr(pos+match.size());
