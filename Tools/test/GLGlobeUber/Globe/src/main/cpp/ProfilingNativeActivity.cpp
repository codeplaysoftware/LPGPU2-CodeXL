...
// Options to assign to LPGPU2_APP, below, are: Globe,Uber,Uber2
//
// Globe is the Globe App
// Uber is the Uber App
// Uber2 is the Uber App but with shared resources resulting in a faster start-up

#define LPGPU2_APP Uber2
...
#include "NDKHelper.h"
#define LPGPU2_PACKAGE_NAME  org_lpgpu_globe
...
void Engine::CheckAPISupport() {
  ...
  LOGI("Device API Level %d", apilevel);
    ...
    uint mask;
    lpgpu2_GetThreadAffinityMask(&mask);
    LOGI("Affinity Mask = 0x%x", mask);
    lpgpu2_SetThreadAffinityMask(0x8);
  if (apilevel >= 24) {
...
Java_org_lpgpu_globe_ProfilingNativeActivity_choregrapherCallback(
...
void Engine::LoadResources() {
  renderer_.UpdateViewport(context_);
  renderer_.Startup       (context_);
...
void Engine::DrawFrame() {
  ...
  lpgpu2_LogAffinityCounter();
    lpgpu2_LogUserCounter(LPGPU2_USER_COUNTER_0, (uint)fps);
