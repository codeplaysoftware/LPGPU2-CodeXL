...
UberEarth::UberEarth()//double o, double r, double x, double y, double z)
:geometry_cloud(nullptr) {}//,offset(o),radius(r),x4D(x),y4D(y),z4D(z) {}



UberEarth::~UberEarth() {Shutdown();}

void UberEarth::Shutdown() {delete[] geometry_cloud; geometry_cloud = nullptr;}
...
void Uber::UpdateViewport(ProfilingContext& context) {
 int32_t viewport[4];
...
void Uber::Update(ProfilingContext& context) {
...
void UberStars::Startup(ProfilingContext& context) {
 program_stars.compile("Shaders/stars.vsh", "Shaders/stars.fsh");

 program_stars.bindAttribLocation(UBER_ATTRIB_Position,"Position");
 program_stars.bindAttribLocation(UBER_ATTRIB_Normal  ,"Normal"  );
 program_stars.bindAttribLocation(UBER_ATTRIB_Tex     ,"Tex"     );
...
void UberEarth::Startup(ProfilingContext& context) {
...
//  glFrontFace(GL_CCW);
 ...
 program_earth.bindAttribLocation(UBER_ATTRIB_Position,"Position");
 program_earth.bindAttribLocation(UBER_ATTRIB_Normal  ,"Normal"  );
 program_earth.bindAttribLocation(UBER_ATTRIB_Tex     ,"Tex"     );
 ...
 program_cloud.bindAttribLocation(UBER_ATTRIB_Position,"Position");
 program_cloud.bindAttribLocation(UBER_ATTRIB_Normal  ,"Normal"  );
 program_cloud.bindAttribLocation(UBER_ATTRIB_Tex     ,"Tex"     );
 ...
 geometry_cloud = new UberGeometry[num_cloud_layers];
...
void UberStars::Render(ProfilingContext& context) {
...

 {
  ...
  int32_t stride = sizeof(UBER_VERTEX);

  VertexAttribGuard guard_attrib_Position(UBER_ATTRIB_Position, 4, GL_FLOAT, GL_FALSE, stride, BUFFER_OFFSET(0                  ));
  VertexAttribGuard guard_attrib_Normal  (UBER_ATTRIB_Normal  , 3, GL_FLOAT, GL_FALSE, stride, BUFFER_OFFSET(4 * sizeof(GLfloat)));
  VertexAttribGuard guard_attrib_Tex     (UBER_ATTRIB_Tex     , 2, GL_FLOAT, GL_FALSE, stride, BUFFER_OFFSET(7 * sizeof(GLfloat)));
...
void UberEarth::Render(ProfilingContext& context) {//double tim) {
...
UberProgramStars::UberProgramStars():
 surface(*this,0,"surface") // texture unit 0
{}

void UberProgramStars::uniforms() {
 UberProgram::uniforms();
}



UberProgramEarth::UberProgramEarth():
...
void UberProgramEarth::uniforms() {
 UberProgram::uniforms();
...
UberProgramCloud::UberProgramCloud():
 surface(*this,0,"surface") // texture unit 0
{}

void UberProgramCloud::uniforms() {
 UberProgram::uniforms();
...
void UberElements::Init(std::size_t nx, std::size_t ny, GLenum mode) {
  xsiz = nx;
  ysiz = ny;
...
void UberVertices::Init(UberElements& elem, double radius, GLenum mode, double offset) {
  xsiz = elem.xsize();
  ysiz = elem.ysize();

  UBER_VERTEX* dat = new UBER_VERTEX[xsiz*ysiz];
...
// radius * (dat[o].nrm[2]+2)/3; // could try almost anything here as long is it varies smoothly with position
  ...
  Vertices::Init(xsiz*ysiz,sizeof(UBER_VERTEX),dat,mode);
...
void UberGeometry::Init(int nx, int ny, double radius, GLenum mode, double offset) {
 elements.Init(nx,ny          ,mode);
 vertices.Init(elements,radius,mode,offset);
}

void UberGeometry::bind() {
 elements.bind();
 vertices.bind();
}

void UberGeometry::unbind() {
 elements.unbind();
 vertices.unbind();
}
