...
//++AT:LPGPU2
// We rm the assertion here because this branch check fails only once at the
// beginning of a live session, and it does not present again. After the first
// data packet is received, the rcStart and rcEnd will be set correctly
// and the assertion would not be triggered anymore.
                    if(rcStart && rcEnd)
//--AT:LPGPU2
                    {
...
//++CF:LPGPU2 Commenting out these lines as they only exist as a performance optimisation and
//            were causing issues where graphs were zoomed in very far, causing the nearest
//            points in either direction to not be visible. Rather than showing the interpolated
//            line between the points, no data was visible. This is because the nearest 'real'
//            points being found in both directions were actually the same.
...
int acVectorLineGraph::GetAllowedPointsToDraw(int& initialNumberPoints, int& lowerIndex, int& upperIndex) const
{
    ...
    if (rectWidth > 0)
    {
//++CF:LPGPU2 disabling this check for now, allowing points than there are pixels available
...
//++CF:LPGPU2 disable limit on drawn points
...
//++CF:LPGPU2 disable check that currentIndex < DRAW_MAXIMUM_POINTS, allowing full graph to be drawn
...
//++AT:LPGPU2
        if(lastPointIndex < numDataPoints && numDataPoints > 0)
//--AT:LPGPU2
        {
...
//++CF:LPGPU2 disable this for performance reasons, will be replaced by QPC brush in DrawLinePlot
...
void acVectorLineGraph::DrawLinePlot(QCPPainter* painter, QVector<QPointF>& pointsVector)
{
    ...
    else
    {
        // Use the qcustomplot draw line function
        drawLinePlot(painter, &pointsVector);
//++CF:LPGPU2 - TODO enable this when performance is acceptable
        // setBrush(mainBrush());
        // drawFill(painter, &pointsVector);
//--CF:LPGPU2
    }
}
