...
//++AT:LPGPU2
        ...
        SetVisibleRange(startTimeOnGrid, itemLength);
//--AT:LPGPU2
...
{
  //++CF:LPGPU2 fixing random segfault here
    int zoomScale = 700;
    int hScrollScale = 600;
    int vScrollScale = 600;
...
//++AT:LPGPU2
    GT_IF_WITH_ASSERT(visibleRange <= m_nFullRange)
//--AT:LPGPU2
    {
        ...
        //++TLRS: LPGPU2: Fix bug when visible range is larger than the full range
        if (dVisibleRange <= 0)
        {
            dVisibleRange = visibleRange;
        }            
        //--TLRS: LPGPU2: Fix bug when visible range is larger than the full range
...
//++AT:LPGPU2
/// @brief Trigger zooming of the ribbon.
/// @note used mainly from the FrameDetailView classes
void acTimeline::TriggerZoom()
{
  const int kTicksForTriggering = 120;
  userZoom(kTicksForTriggering);
  userZoom(-kTicksForTriggering);
}
//--AT:LPGPU2
