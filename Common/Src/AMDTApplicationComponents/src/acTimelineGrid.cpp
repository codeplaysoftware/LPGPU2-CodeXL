...
//++AT:LPGPU2
        QString argStr;
        argStr =  m_strDurationHintLabel.arg(startStrNum);
        int argTextWidth = fontMetrics().width(argStr);
        startHintRect = calcHintRect(argTextWidth, m_nSelectedTime);
        painter.fillRect(startHintRect, hintColor);
        painter.drawText(startHintRect, Qt::AlignCenter, argStr);
//--AT:LPGPU2
