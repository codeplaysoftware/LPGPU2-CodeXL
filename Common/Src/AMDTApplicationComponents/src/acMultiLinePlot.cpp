...
acSingleLineGraph* acMultiLinePlot::InitPlotInner(acMultiLinePlotItemData* pCurrentGraphItem, QColor& graphColor)
{
    ...
    GT_IF_WITH_ASSERT(NULL != m_pCustomPlot && NULL != pCurrentGraphItem)
    {
        ...
        pCurrentGraph->SetId(pCurrentGraphItem->GetId());
                                             ...
                                             bool removeOld, bool allowReplot)
{
    ...
    //++TLRS: LPGPU2: There is no need for this check. Removing this allows partial data to be added to the plot.
...
//++AT:LPGPU2
/// @brief Look up the index of the graph given its counter ID in integer form
/// @param id The id of the counter a given graph is tracking
/// @return Positive value if the a corresponding graph was found, -1 if not
int acMultiLinePlot::GetGraphIndexById(int id) const
{
    int index = -1;
    GT_IF_WITH_ASSERT(!m_pGraphsVec.isEmpty())
    {
        QString idAsStr;
        idAsStr.setNum(id);
        index = GetGraphIndexById(idAsStr);
    }

    return index;
}

/// @brief Look up the index of the graph given its counter ID
/// @param id The id of the counter a given graph is tracking
/// @return Positive value if the a corresponding graph was found, -1 if not
int acMultiLinePlot::GetGraphIndexById(const QString &id) const
{
    int retVal = -1;
    GT_IF_WITH_ASSERT(!m_pGraphsVec.isEmpty())
    {
        for (int i = 0; i < m_pGraphsVec.size(); i++)
        {
            QString nn = m_pGraphsVec[i]->GetId();
...
//++AT:LPGPU2
// Rm assert because with the LPGPU2 mode the addRemoveLine entry can be missing
        if(pAddRemoveLine != NULL)
        {
            // Set it to be gray:
...
bool acMultiLinePlot::GetInfoTableSelectedGraphValue(QString& graphName, QString& valueStr) const
{
    ...
    GT_IF_WITH_ASSERT(NULL != m_pPlotInfoTable)
    {
        ...
        gtString counterSelNameAsGtString;
        counterSelNameAsGtString.fromASCIIString(AC_STR_CounterSelectionString);
...
//++AT:LPGPU2
            ...
            if (name != counterSelNameAsGtString)
            {
...
//++AT:LPGPU2
            if (m_pPlotInfoTable->rowCount() > 0)
            {
              gtString name;
