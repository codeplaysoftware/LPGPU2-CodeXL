...
//++CF:LPGPU2
//  This is a modified version of the acNavigationChart class that uses gtUInt64
//  instead of int for storing data ranges etc. This allows fitting a reasonable
//  number of nanoseconds.
//--CF:LPGPU2

//------------------------------ LPGPU2acNavigationChart.cpp ------------------------------
...
static const int NAVIGATION_CHART_MAX_LABEL_LEN = 15;
...
//++CF:LPGPU2 This used to be 5000, but it was lowered to allow for finer granularity:
static const int MINIMUM_ACTIVE_RANGE = 1;
//--CF:LPGPU2
static const int INITIAL_YMAX = 20;
static const int RANGE_LABELS_Y_OFFSET = 1;

static const int DEFAULT_SAMPLING_INTERVAL = 100;

#define NavigationChartTimeLabelStyle "QLabel {color: %1; font-size: 10px; font-weight: normal}"
static const int NAV_TOP_MARGIN = 10;
static const int NAV_FULL_RANGE_X_AXIS_PADDING = 22;

static const int Y_NUMBER_TICKS = 3;

const Qt::PenStyle RANGE_BOUNDING_LINE_STYLE = Qt::SolidLine;

LPGPU2acNavigationChartLayer::LPGPU2acNavigationChartLayer() : m_layerId(-1), m_type(eNavigationLayerBar), m_visible(true), m_pGraphLine(nullptr)
{
    for (int i = 0; i < LPGPU2acNavigationChart::eNumHighlightedBars; i++)
    {
        m_pGraphBars[i] = nullptr;
    }
}

LPGPU2acNavigationChartLayer::~LPGPU2acNavigationChartLayer()
{
...
LPGPU2acNavigationChartLayerNonIndex::LPGPU2acNavigationChartLayerNonIndex()
{
...
LPGPU2acNavigationChart::LPGPU2acNavigationChart(QWidget* pParent,
    const QString& dataLabel,
    const LPGPU2acNavigationChartInitialData* initialData,
    LPGPU2acNavigationChartColorsData* colorsData,
    //++CF:LPGPU2 Param to force navigation range to be over the entire given range:
    bool forceFullRangeDisplay /*=false*/) :
    //--CF:LPGPU2
    QCustomPlot(pParent),
    ...
    m_xAxisSelectedValueLow(0), // -1
    m_xAxisSelectedValueHigh(0), // -1
    ...
    m_timelineSyncPos(0),
//++CF:LPGPU2
    m_forceFullRangeDisplay(forceFullRangeDisplay)
//--CF:LPGPU2
{
    // if initial data is null - create a default data
    if (initialData == nullptr)
    {
        initialData = new LPGPU2acNavigationChartInitialData();
    }
    ...
    {
        m_pColorsData = new LPGPU2acNavigationChartColorsData();
    }
...
LPGPU2acNavigationChart::~LPGPU2acNavigationChart()
{
}

void LPGPU2acNavigationChart::paintEvent(QPaintEvent* event)
{
...
void LPGPU2acNavigationChart::resizeEvent(QResizeEvent* event)
{
...
void LPGPU2acNavigationChart::enterEvent(QEvent* pEvent)
{
    QCustomPlot::enterEvent(pEvent);
    emit ChartEnter(this);
}

void LPGPU2acNavigationChart::leaveEvent(QEvent* pEvent)
{
    QCustomPlot::leaveEvent(pEvent);
    emit ChartLeave();
}

void LPGPU2acNavigationChart::GetBounds(gtUInt64& lower, gtUInt64& upper)
{
    ...
    gtUInt64 rightMargin = (axisRect()->minimumMargins().right() > 0 ? axisRect()->minimumMargins().right() - 1 : 0);
...
void LPGPU2acNavigationChart::mouseMoveEvent(QMouseEvent* event)
{
...
void LPGPU2acNavigationChart::mousePressEvent(QMouseEvent* event)
{
    if (m_zoomCtrlEnabled)
    {
...
void LPGPU2acNavigationChart::wheelEvent(QWheelEvent* pEvent)
{
    if (m_zoomCtrlEnabled)
    {
...
void LPGPU2acNavigationChart::mouseReleaseEvent(QMouseEvent* event)
{
    if (m_zoomCtrlEnabled)
    {
        QPoint mousePos = event->pos();
...
bool LPGPU2acNavigationChart::AddNewData(double key, double value, QCPRange& retRange, bool shouldReplot)
{
...
void LPGPU2acNavigationChart::DragLeftHandleTo(QPoint mousePos)
{
    GT_IF_WITH_ASSERT(nullptr != m_pAllSessionGraph)
    {
        ...
        if (vectorSize > 0)
        {
            ...
            else if (mousePos.x() > HighPos)
            {
                ...
                if ((nearestIndex >= 1) && (nearestIndex < static_cast<int>(pVectorData->size())))
                {
...
void LPGPU2acNavigationChart::DragRightHandleTo(QPoint mousePos)
{
...
//++AT:LPGPU2
            m_xAxisSelectedValueHigh = pVectorData->at(nearestIndex).Key();
//--AT:LPGPU2
        }
        else
        {
...
void LPGPU2acNavigationChart::DragRangeTo(const int& mouseXPosition)
{
    acDataVector* pVectorData = m_pAllSessionGraph->VectorData();
    gtUInt64 vectorSize = pVectorData->size();
    ...
    if (vectorSize > 0)
    {
        ...
        m_leftHandlePosition.setX(gtUInt64(m_pAllSessionXAxis->coordToPixel(m_xAxisSelectedValueLow) - m_leftHandleDataOffset + 1));
        m_rightHandlePosition.setX(gtUInt64(m_pAllSessionXAxis->coordToPixel(m_xAxisSelectedValueHigh) - m_rightHandleDataOffset + 1));
    }
}

bool LPGPU2acNavigationChart::IsOverLeftHandle(const QPoint& mousePos) const
{
    bool bRes = false;
    // check if mouse cursor is over range control handle:
...
bool LPGPU2acNavigationChart::IsOverRange(const QPoint& mousePos) const
{
    bool bRes = false;
...
bool LPGPU2acNavigationChart::IsOverRightHandle(const QPoint& mousePos) const
{
    bool bRes = false;
    // check if mouse cursor is over range control handle:
...
void LPGPU2acNavigationChart::DrawRangeBoundingLine(QPainter& painter)
{
    const auto rightMargin = (axisRect()->minimumMargins().right() > 0 ? axisRect()->minimumMargins().right() - 1 : 0);

    const std::array<QPoint, 6> polyline = 
    {
        QPoint(m_pActiveRangeXAxis->axisRect()->left(), m_pActiveRangeXAxis->axisRect()->bottom() + 3), // at the active range line left just below XAxis
        QPoint(m_pAllSessionXAxis->coordToPixel(m_xAxisSelectedValueLow), m_pActiveRangeXAxis->axisRect()->bottom() + 3),// at the m_nLow just below X-axis
        QPoint(m_pAllSessionXAxis->coordToPixel(m_xAxisSelectedValueLow), axisRect()->top() - 3), // at the m_nLow and x-Axis rect top
        QPoint(m_pAllSessionXAxis->coordToPixel(m_xAxisSelectedValueHigh), axisRect()->top() - 3), // at the m_nHigh and x_Axis rect top
        QPoint(m_pAllSessionXAxis->coordToPixel(m_xAxisSelectedValueHigh), m_pActiveRangeXAxis->axisRect()->bottom() + 3),// at the m_nHigh just below X-axis
        QPoint(m_pActiveRangeXAxis->axisRect()->right() + 1 + rightMargin, m_pActiveRangeXAxis->axisRect()->bottom() + 3) // at the active range line right just below XAxis
    };

    QPen myPen(acQAMD_ORANGE_PRIMARY_COLOUR, RANGE_BOUNDING_LINE_WIDTH, Qt::SolidLine);

//++AT:LPGPU2
    UpdateActiveRangeBoundingLineTimeLabels(polyline[1].x(), polyline[4].x(), polyline[3].y());
//--AT:LPGPU2
...
void LPGPU2acNavigationChart::DrawRangeControl(QPainter& painter)
{
...
void LPGPU2acNavigationChart::HighlightActiveRange()
{
    GT_IF_WITH_ASSERT(nullptr != m_pAllSessionGraph)
    {
        if (!m_vXData.empty())
...
void LPGPU2acNavigationChart::InitGraphs()
{
...
void LPGPU2acNavigationChart::PositionRangeControl(const QCPRange& range)
{
...
void LPGPU2acNavigationChart::SetRangeState()
{
    QVector<double>::iterator lastData = m_vXData.end();
    lastData--;
...
void LPGPU2acNavigationChart::LoadRangeControlImages()
{
...
QString LPGPU2acNavigationChart::TimeToString(eNavigationUnits units, double time, bool shouldShowSmallestUnits, bool shouldUseWords)
{
    QString retStr;
...
void LPGPU2acNavigationChart::SetStaticTickLabels()
{
    GT_IF_WITH_ASSERT(nullptr != m_pFullRangeStaticTicksAxis)
    {
        if (!m_vXData.empty() && m_pFullRangeStaticTicksAxis->axisRect()->width() >= 0)
        {            
            const auto nNumOfStaticTicks = m_pFullRangeStaticTicksAxis->axisRect()->width() / m_fullRangeTickInterval;
            m_vStaticTicks.clear();
            m_vStaticTickLabels.clear();
            
            const auto FirstTick = double(m_pFullRangeStaticTicksAxis->pixelToCoord(axisRect()->left()));
            const auto LastTick = double(m_pFullRangeStaticTicksAxis->pixelToCoord(axisRect()->right() + 1));
            const auto TickStep = (LastTick - FirstTick)  / (nNumOfStaticTicks - 1);
            const auto nValueStep = (LastTick - FirstTick)  / (nNumOfStaticTicks - 1);

            for (auto i = 0U; i < nNumOfStaticTicks; ++i)
            {
...
void LPGPU2acNavigationChart::SetYAxisTickLabels()
{
...
void LPGPU2acNavigationChart::SetActiveRangeXAxisTickLabels()
{
    GT_IF_WITH_ASSERT(nullptr != m_pActiveRangeXAxis && nullptr != m_pFullRangeStaticTicksAxis)
    {
        if (!m_vXData.empty() && !m_pActiveRangeXAxis->autoTicks() && m_pActiveRangeXAxis->axisRect()->width() > 0)
        {
...
void LPGPU2acNavigationChart::SetOfflineData(const QVector<double>& xData, const QVector<double>& yData)
{
    // Sanity check:
    GT_IF_WITH_ASSERT(!xData.isEmpty() && !yData.isEmpty() && xData.size() == yData.size() && nullptr != m_pAllSessionGraph)
    {
        ...
        if (!m_forceFullRangeDisplay && (m_xAxisSelectedValueHigh - m_xAxisSelectedValueLow > GetDefaultTimeRange()))
        {
...
void LPGPU2acNavigationChart::SetYAxisValues(int minY, int maxY, const QString& yAxisLabel)
{
...
void LPGPU2acNavigationChart::UpdateYAxisRange()
{
    GT_IF_WITH_ASSERT(m_pYAxis != nullptr)
    {
...
void LPGPU2acNavigationChart::UpdateMinHeightValues()
{
    ...
    GT_IF_WITH_ASSERT(m_pYAxis != nullptr)
    {
        ...
        {
            LPGPU2acNavigationChartLayer* pCurrentLayer = m_layersVector[nLayer];
...
double LPGPU2acNavigationChart::GetDefaultTimeRange()
{
...
double LPGPU2acNavigationChart::GetMinimumRange()
{
    double range = 0;
...
bool LPGPU2acNavigationChart::ShouldDrawBoundingLine() const
{
//++CF:LPGPU2
    // If the entire range is being initially displayed, still display the
    // bounding line to allow navigation:
    if (m_forceFullRangeDisplay)
    {
      return true;
    }
//--CF:LPGPU2
    ...
    if (retVal)
    {
        ...
        retVal = (m_vXData.size() >= 10000 / static_cast<int>(m_samplingInterval));
    }

    return retVal;
}

void LPGPU2acNavigationChart::UpdateActiveRangeBoundingLineTimeLabels(gtUInt64 startPosition, gtUInt64 endPosition, gtUInt64 yPosition)
{
    ...
    {
//++AT:LPGPU2
        QString qstrActiveRangeBeginTime(TimeToString(m_unitsX, m_xAxisSelectedValueLow , true));
        QString qstrActiveRangeTimeDelta(TimeToString(m_unitsX, (m_xAxisSelectedValueHigh - m_xAxisSelectedValueLow), true, true));
        QString qstrActiveRangeEndTime(TimeToString(m_unitsX, m_xAxisSelectedValueHigh , true));
//--AT:LPGPU2
        ...
        m_pActiveRangeBeginTimeLabel->setGeometry(startPosition,
                                                  yPosition - m_pActiveRangeBeginTimeLabel->height(),
        ...
        m_pActiveRangeTimeDeltaLabel->setGeometry(startPosition + (endPosition - startPosition) / 2 - m_pActiveRangeTimeDeltaLabel->width() / 2,
                                                  yPosition - m_pActiveRangeTimeDeltaLabel->height(),
        ...
        m_pActiveRangeEndTimeLabel->setGeometry(endPosition - m_pActiveRangeEndTimeLabel->width(),
                                                yPosition - m_pActiveRangeEndTimeLabel->height(),
...
void LPGPU2acNavigationChart::ApplyRangeAfterZoom(const QCPRange& rangeAfterZoom)
{
...
void LPGPU2acNavigationChart::CalculateRangeAfterZoom(bool shouldZoomIn, QCPRange& rangeAfterZoom)
{
...
void LPGPU2acNavigationChart::SetVisibleRange(const double startVisibleTime, const double visibleRange)
{
...
void LPGPU2acNavigationChart::OnRangeChangeEnded(const QPointF& newRange)
{
    ...
    for (int nLayer = 0; nLayer < numLayers; nLayer++)
    {
        ...
        if (pCurrentLayer != nullptr)
        {
            ...
            if (pCurrentLayer->m_visible)
            {
                ...
                LPGPU2acNavigationChartLayerNonIndex* pNonIndexLayer = dynamic_cast<LPGPU2acNavigationChartLayerNonIndex*>(pCurrentLayer);
...
void LPGPU2acNavigationChart::RemoveLayerPlotable(LPGPU2acNavigationChartLayer* pLayer)
{
    GT_IF_WITH_ASSERT(pLayer != nullptr)
    {
        if (pLayer->m_type == LPGPU2acNavigationChartLayer::eNavigationLayerGraph)
        {
...
void LPGPU2acNavigationChart::AddIndexedLayerGraphs(LPGPU2acNavigationChartLayer* pLayer, gtUInt64 fromDataPoint, gtUInt64 toDataPoint)
{
    GT_IF_WITH_ASSERT(pLayer != nullptr)
    {
        if (pLayer->m_type == LPGPU2acNavigationChartLayer::eNavigationLayerGraph)
        {
            ...
            if (static_cast<double>(toDataPoint) < m_vXData.size() - 1)
            {
...
void LPGPU2acNavigationChart::AddNonIndexedLayerGraphs(LPGPU2acNavigationChartLayerNonIndex* pLayer)
{
    GT_IF_WITH_ASSERT(pLayer != nullptr)
    {
...
void LPGPU2acNavigationChart::AddBar(LPGPU2acNavigationChartLayer* pLayer, gtUInt64 barIndex, gtUInt64 fromDataPoint, gtUInt64 toDataPoint)
{
    GT_IF_WITH_ASSERT(pLayer != nullptr)
    {
        ...
        LPGPU2acNavigationChartLayerNonIndex* pNonIndexLayer = dynamic_cast<LPGPU2acNavigationChartLayerNonIndex*>(pLayer);

        for (gtUInt64 i = fromDataPoint; i <= toDataPoint; i++)
        {
...
void LPGPU2acNavigationChart::AddDataLayer(LPGPU2acNavigationChartLayer* pLayer)
{
    GT_IF_WITH_ASSERT(pLayer != nullptr)
    {
        bool addLayer = false;
...
void LPGPU2acNavigationChart::SetLayerVisiblity(int layerID, bool visible)
{
...
void LPGPU2acNavigationChart::UpdateYAxisRangeBasedOnVisibleLayers()
{
...
bool LPGPU2acNavigationChart::IsDisplayingFullRange()
{
    bool retVal = false;
...
LPGPU2acNavigationChartLayer* LPGPU2acNavigationChart::GetLayerById(int layerID)
{
    LPGPU2acNavigationChartLayer* pRetVal = nullptr;
...
void LPGPU2acNavigationChart::OnShowTimeLine(bool visible, double timePos)
{
...
void LPGPU2acNavigationChart::DrawTimelineSync(QPainter& painter)
{
...
/// @brief Get the nearest time before a given time where a (possibly interpolated) sample
///        is located.
gtUInt64 LPGPU2acNavigationChart::GetNearestSamplePointBefore(gtUInt64 targetTime) const
{
    // Qt 5.5 doesn't have reverse iterator members for QVector:
    using RevIter_t = std::reverse_iterator<QVector<qreal>::const_iterator>;
    RevIter_t rbegin(m_vXData.end());
    RevIter_t rend(m_vXData.begin());

    auto foundX = std::find_if(rbegin, rend, [&](const double& val){ return val < targetTime; });
    if (foundX == rend)
    {
        return targetTime;
    }
...
/// @brief Get the nearest time after a given time where a (possibly interpolated) sample
///        is located.
gtUInt64 LPGPU2acNavigationChart::GetNearestSamplePointAfter(gtUInt64 targetTime) const
{
    auto foundX = std::find_if(m_vXData.begin(), m_vXData.end(), [&](const double& val){ return val > targetTime; });
    if (foundX == m_vXData.end())
    {
        return targetTime;
    }
    else
