...
//++AT:LPGPU2
    acMultiLinePlotItemData(QVector<double> KeyVec,
        QVector<double> valueVec, const QColor& color,
        const QString& graphName, const QString graphDescription,
        const QString &id, bool disableGraph = false)
    : m_keyVec{KeyVec}, m_valueVec{valueVec}, m_color{color},
      m_graphName{graphName}, m_description{graphDescription},
      m_id{id}, m_shouldBeDisabled{disableGraph}
    {
    }
//--AT:LPGPU2
...
//++AT:LPGPU2
    /// graph's id, which is the same as the counter it is tracking
    QString m_id;
//--AT:LPGPU2
