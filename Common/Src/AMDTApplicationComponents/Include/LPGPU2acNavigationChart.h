...
//------------------------------ LPGPU2acNavigationChart.h ------------------------------
#ifndef __LPGPU2ACNNAVIGATIONCHART_H
#define __LPGPU2ACNNAVIGATIONCHART_H
...
class LPGPU2acNavigationChartInitialData
{
public:
    /// default constructor
    LPGPU2acNavigationChartInitialData()
    {
    ...
    /// constructor
    LPGPU2acNavigationChartInitialData(gtUInt64 chartHeight, gtUInt64 tickCount, gtUInt64 fullRangeTickInterval,
                                 gtUInt64 xAxisLowHighValuesDelta, gtUInt64 activeRangeXAxisWidth, gtUInt64 activeRangeYAxisWidth)
    {
    ...
    gtUInt64 m_chartHeight;
    /// number of ticks
    gtUInt64 m_tickCount;
    /// ticks interval
    gtUInt64 m_fullRangeTickInterval;
    /// delta between xAxis low and high values. A negative value means the entire range
    gtUInt64 m_xAxisLowHighValuesDelta;
    ///active range xAxis width
    gtUInt64 m_activeRangeXAxisWidth;
    ///active range yAxis width
    gtUInt64 m_activeRangeYAxisWidth;
};

class LPGPU2acNavigationChartColorsData
{
public:

    /// default constructor
    LPGPU2acNavigationChartColorsData()
    {
    ...
    /// constructor
    LPGPU2acNavigationChartColorsData(QColor axidTicksLabelColor,
...
struct LPGPU2acNavigationChartPixmaps
{
...
class AC_API LPGPU2acNavigationChartLayer
{
public:
    LPGPU2acNavigationChartLayer();
    virtual ~LPGPU2acNavigationChartLayer();
...
class AC_API LPGPU2acNavigationChartLayerNonIndex : public LPGPU2acNavigationChartLayer
{
public:
    LPGPU2acNavigationChartLayerNonIndex();
    virtual ~LPGPU2acNavigationChartLayerNonIndex();
...
class AC_API LPGPU2acNavigationChart : public QCustomPlot
{
    Q_OBJECT
...
public:
    ...
    LPGPU2acNavigationChart(QWidget* pParent, const QString& dataLabel,
                      const LPGPU2acNavigationChartInitialData* initialData = nullptr,
                      LPGPU2acNavigationChartColorsData* colorsData = nullptr,
                      bool forceFullRangeDisplay = false);

    LPGPU2acNavigationChart() : LPGPU2acNavigationChart(nullptr, "") {};

    virtual ~LPGPU2acNavigationChart();
    ...
    /// \returns the axis
    QCPAxis* GetActiveRangeXAxis() const { return m_pActiveRangeXAxis; }

    /// Sets the interval between sample reading
    /// \param nInterval is the interval
    void SetInterval(const gtUInt64 nInterval) { m_samplingInterval = nInterval; }
    ...
    void SetDefaultTimeRange(const double defaultTimeRange) { m_defaultTimeRange = defaultTimeRange; }
    ...
    void SetMinimumRange(const double minimumRange) { m_minimumRange = minimumRange;  }

    /// Checks if currently the bounding line should be drawn:
    /// \return true if the bounding line should be painted.
    bool ShouldDrawBoundingLine() const;
    ...
    /// \returns the range
    //++CF:LPGPU2 update return type from QPoint to QPointF to preserve accuracy
    const QPointF GetCurrentRange() const { return QPointF(m_xAxisSelectedValueLow, m_xAxisSelectedValueHigh); };
    ...
    /// Add a layer of data
    void AddDataLayer(LPGPU2acNavigationChartLayer* pLayer);
    ...
    void SetNavigationUnitsX(const eNavigationUnits iUnits) { m_unitsX = iUnits; }
    void SetNavigationUnitsY(const eNavigationUnits iUnits) { m_unitsY = iUnits; }

    /// Get the units of the navigation chart
    eNavigationUnits GetNavigationUnitsX() const { return m_unitsX; }
    eNavigationUnits GetNavigationUnitsY() const { return m_unitsY; }

    /// Set the dislay Type
    void SetUnitsDisplayType(const eNaviationDisplay displayType) { m_unitDisplay = displayType; }

    /// Get the bounds position
    void GetBounds(gtUInt64& lower, gtUInt64& upper);
    ...
    LPGPU2acNavigationChartLayer* GetLayerById(int layerID);
    ...
    bool ShowingTimeline() const { return m_showTimelineSync; }

    /// Get the nearest time before a given time where a (possibly interpolated) sample is located.
    gtUInt64 GetNearestSamplePointBefore(gtUInt64 targetTime) const;

    /// Get the nearest time after a given time where a (possibly interpolated) sample is located.
    gtUInt64 GetNearestSamplePointAfter(gtUInt64 targetTime) const;
...
protected:
    ...
    void UpdateActiveRangeBoundingLineTimeLabels(gtUInt64 startPosition, gtUInt64 endPosition, gtUInt64 yPosition);

    /// Remove a layer plotables
    /// \param Layer to remove its plotables
    void RemoveLayerPlotable(LPGPU2acNavigationChartLayer* pLayer);
    ...
    void AddIndexedLayerGraphs(LPGPU2acNavigationChartLayer* pLayer, gtUInt64 fromDataPoint, gtUInt64 toDataPoint);

    /// Add a NON Index layer graphs
    /// \param Layer to adds it data to the plotables
    void AddNonIndexedLayerGraphs(LPGPU2acNavigationChartLayerNonIndex* pLayer);
    ...
    void AddBar(LPGPU2acNavigationChartLayer* pLayer, gtUInt64 barIndex, gtUInt64 fromDataPoint, gtUInt64 toDataPoint);
...
signals:
    ...
    void BoundingChanged(gtUInt64&, gtUInt64&);

    /// signal that the mouse is over the chart (needed by gpFilterButton)
    void ChartEnter(LPGPU2acNavigationChart*);
...
private:
    ...
    LPGPU2acNavigationChartPixmaps m_navigationPixmaps;
    ...
    gtUInt64 m_leftHandleDataOffset;
    gtUInt64 m_rightHandleDataOffset;

    gtUInt64 m_samplingInterval;
    ...
    gtUInt64 m_xAxisLowHighSelectedValuesDelta;

    LPGPU2acNavigationChartColorsData* m_pColorsData;
    ...
    /// a vector of layers
    gtVector<LPGPU2acNavigationChartLayer*> m_layersVector;
...
//++CF:LPGPU2
    /// Force the initial range to be over the entire given set of data, and 
    /// always allow zooming/panning. Fixes issues drawing navigationChart
    /// with some data sets.
    bool m_forceFullRangeDisplay;
//--CF:LPGPU2
};



#endif // __LPGPU2ACNNAVIGATIONCHART_H
