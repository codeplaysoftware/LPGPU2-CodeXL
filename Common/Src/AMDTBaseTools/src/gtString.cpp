...
gtString::~gtString()
{
    // Here to investigate wstring _impl misbehaviour corrupting memory
    if ((_a != 0) || (_b != 0) || (_c != 0) || (_d != 0))
    {
      if (_stringAsASCIICharArray != nullptr)
      {
        *_stringAsASCIICharArray = '0';
      }
    }
...
{
  const size_t sizeThis = _impl.size();
  const size_t sizeOther = otherString._impl.size();
  if (sizeThis < sizeOther)
  {
    _impl.reserve(sizeOther + 1);
  }
