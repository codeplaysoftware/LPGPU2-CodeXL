...
//++AT:LPGPU2
/// @brief popen3 Alternative implementation of the CodeXL popen2. Runs a
///        command in a forked process, capturing its output
/// @note This new function was created because popen2 leaks file descriptors
///       and because osExecAndGrabOutput does not need bidirectional
///       communication with the child process
/// @param pCmd The command to execute
/// @param childInfo A struct containing the information about the child process
/// @return True for success, false for failure
bool popen3(const char* pCmd, popen2_data_t& childInfo)
{
  bool ret = false;
  pid_t p;
  int pipe_stdin[2];

  if (pCmd != nullptr && !pipe(pipe_stdin))
  {
    // Create the child process.
    p = fork();
    ...
    if (p > -1)
    {
      ...
      // Set the output.
      childInfo.m_childPid = p;
      childInfo.m_fromChildChannel = pipe_stdin[0];
      close(pipe_stdin[1]);
...
// ---------------------------------------------------------------------------
//++AT:LPGPU2
// If possible, do not use this function. It leaks file descriptors, i.e. it
// keeps on opening file descriptors without properly closing them. That is due
// to the function popen2, which after creating 2 pipes does not clean up the
// file descriptors used for the piping operation
//--AT:LPGPU2
...
//++AT:LPGPU2
/// @brief Executes a command in a different process and captures its output
/// @note The function was adapted from the osExecAndGrabOutput which is is
///       native to CodeXL. This was necessary because that function leaks
///       file descriptors and causes an overflow of file descriptors in the
///       long run
/// @param cmd The command to be executed, as a string
/// @param cancelSignal A reference to the cancel flag. Upon calling this
///              routine, the cancelSignal flag should be set to false.
///              In case that the caller wants to terminate the commands'
///              execution, this flag should be set to true.
/// @param cmdOutput an output parameter to hold the command's output
/// @return True for success, False for failure
bool LPGPU2osExecAndGrabOutput(
    const char* cmd, const bool& cancelSignal, gtString& cmdOutput)
{
  // The default buffer size.
  constexpr auto kBuffSize = static_cast<size_t>(65536);
  auto ret = false;
  ...
  if (cmd != nullptr)
  {
    ...
    // Launch the command.
    popen2_data_t procData;
    if (popen3(cmd, procData))
    {
      // Prepare the temporary buffer.
      char tmpBuffer[kBuffSize];
      memset(tmpBuffer, 0, kBuffSize);

      while (true)
      {
        // The timeout interval in milliseconds.
        const auto kSleepIntervalMs = 50U;

        // Check the status of the CLI with a timeout. Note that the
        // busy-waiting period is already mitigated by the callee's sleep
        // period, so we can leave the loop as is.
        if (cancelSignal ||
            osWaitForProcessToTerminate(procData.m_childPid, kSleepIntervalMs))
        {
          // If the the build cancel signal was set, or the CLI has finished
          // running (or we have an error waiting), stop waiting.
          break;
      ...
      else
      {
        ...
        // Set this operation to be non-blocking in case that we have no
        // pending data.
        const auto prevFlags = fcntl(procData.m_fromChildChannel, F_GETFL);
        fcntl(procData.m_fromChildChannel, F_SETFL, prevFlags | O_NONBLOCK);
        ret = (read(procData.m_fromChildChannel, tmpBuffer, kBuffSize) != -1);
        fcntl(procData.m_fromChildChannel, F_SETFL, prevFlags);

        if (!ret)
        {
          OS_OUTPUT_DEBUG_LOG(L"Failed to read output", OS_DEBUG_LOG_ERROR);
        }
        else
        {
          // Assign the output buffer.
      ...
      // Close the child's output stream handle.
      const auto closeRC = close(procData.m_fromChildChannel);
      if (closeRC == -1)
      {
        OS_OUTPUT_DEBUG_LOG(L"Failed to close process output channel",
            OS_DEBUG_LOG_ERROR);
        ret = false;
      }
    }
