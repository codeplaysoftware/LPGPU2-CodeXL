...
//++TLRS: LPGPU2: Function to get the PATH of the SYCL runtime library
// ---------------------------------------------------------------------------
// Name:        osGetSystemSYCLModulePath
// Description: Calculates the System's SYCL module (DLL / shared library / framework / etc) path.
// Arguments: systemOCLModulePath - Will get the system's SYCL module path.
// Return Val: bool  - Success / failure.
// Author:      Thales Sabino.
// Date:        03/01/2018
// ---------------------------------------------------------------------------
bool osGetSystemSYCLModulePath(gtVector<osFilePath> &systemSYCLModulePaths)
{
    bool bReturn = false;
    systemSYCLModulePaths.clear();

    gtString syclSystemModuleDir;
    if (osGetCurrentProcessEnvVariableValue(L"COMPUTECPP_PACKAGE_ROOT_DIR", syclSystemModuleDir))
    {
        // Handle the case where the COMPUTECPP_PACKAGE_ROOT_DIR was set.

        osFilePath computeCppRootPath;
        computeCppRootPath.setFileDirectory(syclSystemModuleDir);

        osFilePath computeCppLibPath = computeCppRootPath;
        
        computeCppLibPath.appendSubDirectory(L"lib");
        
        // Append the lib dir
        if (computeCppLibPath.exists())
        {
            osFilePath computeCppModuleFilePath;
            computeCppModuleFilePath.setFileDirectory(computeCppLibPath);
            computeCppModuleFilePath.setFileName(OS_SYCL_ICD_MODULE_NAME);

            if (computeCppModuleFilePath.exists())
            {
                systemSYCLModulePaths.push_back(computeCppModuleFilePath);
                bReturn = true;
            }
        }        
    ...
    {
        // Tries to load the SYCL module from the CodeXL bin dir
        osFilePath computeCppModulePathPath{ osFilePath::OS_CODEXL_BINARIES_PATH, OS_SYCL_ICD_MODULE_NAME };

        if (computeCppModulePathPath.exists())
        {
            systemSYCLModulePaths.push_back(computeCppModulePathPath);
            bReturn = true;
        }        
    }
