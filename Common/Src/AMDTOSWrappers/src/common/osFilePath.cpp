...
//++AT:LPGPU2
/// @brief Ctor. Set file path from path + name + extension
/// @param predefinedfilePath The path to the file
/// @param fileName The name of the file
/// @param fileExtension The extension of the f ile
/// @param convertToLower Whether to convert the path to lowercase
/// @note Throws an assertion if setPath fails
/// @note It does not conform with the LPGPU2's convention of not doing any
///       work in the ctor to comply with the existing codebase
osFilePath::osFilePath(osApplicationSpecialDirectories predefinedfilePath,
    const gtString& fileName, const gtString& fileExtension,
    bool convertToLower)
  ...
  _fileFullPathString{}
{
  ...
  // We need to set the values of the attributes here manually because
  // SetInstallRelatedPath resets the attributes...
  _fileName = fileName;
  _fileExtension = fileExtension;
}
//--AT:LPGPU2
