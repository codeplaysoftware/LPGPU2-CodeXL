...
static constexpr int COMMAND_LINE_SIZE = 1024;

namespace impl {
namespace {
// Utility to easily provide overloads as callable objects
template <class... Lambdas>
struct lambda_overload_t : Lambdas...
{
    template <class... Ls>
    lambda_overload_t(Ls... lambdas) :
        Lambdas(lambdas)... {}
};

template <class... Lambdas>
auto overload(Lambdas... lambdas)
{
    return lambda_overload_t<Lambdas...>(lambdas...);
}
} // namespace
} // namespace impl

  // ---------------------------------------------------------------------------
  /// Extracts the .exe name from the command line arguments
  /// \tparam CharT A char type, either char or wchar_t(not checked)
  /// \param lpCommandLine The command line string
  /// \param buffer The buffer which will contain the extracted string
  // ---------------------------------------------------------------------------
template <class CharT>
static void ExtractApplicationNameFromCmd(const CharT* lpCommandLine, CharT* buffer)
{
    // Create overload to select the right implementation for substring
    auto findExeExtension = impl::overload(
        [](const char* str1)  { return std::strstr(str1, ".exe"); },
        [](const WCHAR* str1) { return std::wcsstr(str1, L".exe"); }
    );
    ...
    const CharT* pExe = findExeExtension(lpCommandLine);

    if (pExe != nullptr)
    {
        // find length of string up to start of '.exe'
        int length = static_cast<int>(pExe - lpCommandLine);
        ...
        {
            CharT currentChar = lpCommandLine[loop];
...
public:
    ...
    DLLInjectorLoadLibrary() = default;
    virtual ~DLLInjectorLoadLibrary() = default;
    ...
    // ---------------------------------------------------------------------------
    virtual bool CalledFromRundll() override
    {
        char modulename[MAX_PATH];
        GetModuleFileNameA(NULL, modulename, MAX_PATH);

        return (std::strstr(modulename, "rundll") != nullptr);
    }
    ...
    // ---------------------------------------------------------------------------
    virtual void Inject(LPSTR lpszCmdLine) override
    {
        char modulename[MAX_PATH];
        GetModuleFileNameA(NULL, modulename, MAX_PATH);
        ...
        int processID = std::atoi(lpszCmdLine);
    ...
    /// \param nDllCount The number of dlls to inject into the process
    /// \param lpDllNames Full paths and names of the dlls to inject into the new
    ///        process, as an array
    /// \return non-zero if function succeeds, 0 if error
    // ---------------------------------------------------------------------------
    virtual BOOL WINAPI CreateProcessAndInjectDllsA(_In_opt_ LPCSTR lpApplicationName,
        ...
        _In_ int nDllCount,
        _In_ LPCSTR* lpDllNames) override
    {
        if (nDllCount < 1)
        {
            return FALSE;
        ...
        GetModuleFileNameA(nullptr, currentFileName, MAX_PATH);
        ...
        if (lpApplicationName != nullptr && strlen(lpApplicationName) > 0)
        {
        ...
        else
        {
            ...
            ExtractApplicationNameFromCmd(lpCommandLine, buffer);
        ...
        BOOL createSuccess = pfCreateProcess(lpApplicationName, lpCommandLine, lpProcessAttributes, lpThreadAttributes, bInheritHandles, dwFlags,
        ...
        if (createSuccess == TRUE)
        {
            ...
            {
                // This module and the application to be created are the same bitness, so just inject the dlls into the new process
                InjectDLLsIntoProcess(nDllCount, lpDllNames, lpProcessInformation->hProcess);
            }
            else
            {
                // This module and the application to be created are different bitness, so use rundll to call a function in MicroDLL to
                // do the dll injection.
                StartRundlls(nDllCount, lpDllNames, lpProcessInformation->dwProcessId, false);
            }

            //Once the dlls have been injected, resume the thread
    ...
    {
        return CreateProcessAndInjectDllsA(lpApplicationName,
            lpCommandLine,
            lpProcessAttributes,
            lpThreadAttributes,
            bInheritHandles,
    ...
    /// \return non-zero if function succeeds, 0 if error
    // ---------------------------------------------------------------------------
    virtual BOOL WINAPI CreateProcessAndInjectDllsW(_In_opt_ LPCWSTR lpApplicationName,
        ...
        _In_ LPCSTR* lpDllNames) override
    {
        ...
        else
        {
            ...
            ExtractApplicationNameFromCmd(lpApplicationName, buffer);
        ...
        if (createSuccess == TRUE)
        {
            if (appType == currentType)
            {
                // These modules and the application to be created are the same bitness, so just
                // inject them into the new process 
                InjectDLLsIntoProcess(nDllCount, lpDllNames, lpProcessInformation->hProcess);
            }
            else
            {
                // These modules and the application are not of the same bitness
                StartRundlls(nDllCount, lpDllNames, lpProcessInformation->dwProcessId, true);
            }

            // Resume thread when all the Dlls have been injected
            ResumeThread(lpProcessInformation->hThread);
        }
    ...
    virtual BOOL WINAPI CreateProcessAndInjectDllW(_In_opt_ LPCWSTR lpApplicationName,
        ...
        _In_ LPCSTR lpDllName) override
    {
        return CreateProcessAndInjectDllsW(lpApplicationName,
            lpCommandLine,
            lpProcessAttributes,
            lpThreadAttributes,
            bInheritHandles,
...
private:
    ...
    // ---------------------------------------------------------------------------
    /// Does all the work in this class - loads the DLLs into the target process.
    /// \param nDllCount The number of Dlls to load
    /// \param lpDllNames The names of the Dlls to load, as an array
    /// \param hProcess Handle to target application process
    /// \return true if successful, false otherwise.
    // ---------------------------------------------------------------------------
    bool InjectDLLsIntoProcess(int nDllCount, LPCSTR* lpDllNames, HANDLE hProcess)
    {
        while (--nDllCount >= 0)
        {
            LPCSTR lpDllName = lpDllNames[nDllCount];

            if (!InjectDLLIntoProcess(lpDllName, hProcess))
                return false;
        }
        return true;
    }
    ...
    // ---------------------------------------------------------------------------
    /// Create a process for rundll32.dll and use this to call a function inside
    /// the dlls specified. This is needed when injecting a 64 bit process from a
    /// 32 bit process. This loads all the dlls by calling the appropriate function
    /// \param nDllCount The number of Dlls to load
    /// \param lpDllNames The names of the dlls to load, as array
    /// \param appProcessID The process ID of the application to be injected into
    /// \param useWideChars Used to select the right function to forward to
    // ---------------------------------------------------------------------------
    void StartRundlls(int nDllCount, LPCSTR* lpDllNames, DWORD appProcessID, bool useWideChars)
    {
        while (--nDllCount >= 0)
        {
            LPCSTR lpDllName = lpDllNames[nDllCount];
            ...
            if (useWideChars)
            {
                StartRundllW(lpDllName, appProcessID);
            }
            else
            {
                StartRundllA(lpDllName, appProcessID);
            }
        }
    }
