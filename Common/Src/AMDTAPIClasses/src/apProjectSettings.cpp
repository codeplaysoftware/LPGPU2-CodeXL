...
/// @brief                  Store the given string as the SYCL runtime library path.
/// @param[in] vRuntimeName The SYCL runtime library path.
void apProjectSettings::SetSYCLRuntimeLib(const gtString &vRuntimeName)
{ 
    m_SYCLRuntimeLibName = vRuntimeName;
}

/// @brief              Set whether or not the SYCL runtime library is being used by the project.
/// @param[in] vbInUse  true = The SYCL runtime library is being used,
///                     false = The SYCL runtime library is not being used.
void apProjectSettings::SetFlagSYCLRuntimeLibInUse(bool vbInUse) 
{ 
   m_bSYCLRuntimeLibInUse = vbInUse;
}

/// @brief          Gets the current SYCL runtime library as seen by the project settings.
/// return gtString The SYCL runtime library that was set.
const gtString apProjectSettings::GetSYCLRuntimeLibName() const
{
    return m_SYCLRuntimeLibName; 
}

/// @brief
/// @return bool true = The SYCL runtime library is being used,
///              false = The SYCL runtime library is not being used.
bool apProjectSettings::GetFlagSYCLRuntimeLibInUse() const 
{ 
    return m_bSYCLRuntimeLibInUse;
}
//--IOR: LPGPU2 Code SYCL implementation runtime location
