...
def initCXLVars (CXL_vars) :
    ...
    CXL_vars.AddVariables(enum_mod.EnumVariable(
        ...
        default = 'x86_64',
#++AT:LPGPU2
        allowed_values = ('x86', 'x86_64', 'armv7a', 'aarch64')))
#--AT:LPGPU2
    ...
    CXL_vars.Add(
        ...
        help = 'Build only the remote agent and its dependencies',
        default = '')
    #--KB: LPGPU2
#++AT:LPGPU2
    # CxL support for building CCVG
    CXL_vars.Add(
        key = 'LPGPU2_BUILD_CCVG',
        help = 'Build the LPGPU2 CCVG project',
        default = 'false',
        allowed_values = ('true', 'false'))

    # CxL support for building CCVG
    # By default the Android remote protocol is always built
    CXL_vars.Add(
        key = 'LPGPU2_BUILD_ANDROID_PROTOCOL',
        help = 'Build the support for the Android remote protcol',
        default = 'true',
        allowed_values = ('true', 'false'))
...
#++AT:LPGPU2
        ...
        bitness = '32'
        env.Append(CPPDEFINES=['CODEXL_ARCH_IS_ARMV7A'])
    elif (arch == 'aarch64'):
        bitness = '64'
        env.Append(CPPDEFINES=['CODEXL_ARCH_IS_AARCH64'])
#--AT:LPGPU2
...
def initCompilerFlags (env) :
    compiler_base_flags = " -Wall -Wextra -g -fmessage-length=0 -Wno-unknown-pragmas -pthread -std=c++11 -D_LINUX"    
    linker_base_flags = ''
...
#++AL:LPGPU2
    if ((env['CXL_arch'] != 'armv7a' and env['CXL_arch'] != 'aarch64') and env['CXL_bitness'] == '32'):
#--AL:LPGPU2
...
def initLegal (env) :    
    ...
    #++ TLRS: LPGPU2 - Using os.walk to get the files in Legal folder
    legals = list()
    for root, _, files in os.walk(legalSrcDir):
        for name in files:
            legalFullFileName = os.path.join(root, name)
            legals.append(legalFullFileName)
    #-- TLRS: LPGPU2     
...
#++AT:LPGPU2
def setCompilerDependingOnarch (env) :
    # Depending on the architecture in use, set the compiler to use
    prefix = ""
    if env['CXL_arch'] == 'armv7a':
        if env['CXL_os'] == 'Linux':
            prefix = "arm-linux-gnueabihf-"
        # MIGHT SUPPORT THIS IN FUTURE
        # elif env['CLX_os'] == 'Android':
...
def initCXLBuild (env) :
    ...
    initOs(env)
#++AT:LPGPU2
    setCompilerDependingOnarch(env)
#--AT:LPGPU2
    initDistro(env)
...
def UseQt4(env):
    #++KB: LPGPU2
    if (env['CXL_build_only_remote_agent'] == ''):
...
def initGtk (env) :
# TODO Have GTK always built no matter what the arch
# AT:LPGPU2: Ignore the branching on the arch for now
...
def UseGtk(env):
    #++KB: LPGPU2
    # Only allow use of Gtk if not a remote-agent-only build
    if (env['CXL_build_only_remote_agent'] == '' and
            (env['CXL_arch'] != 'armv7a' and env['CXL_arch'] != 'aarch64')):
        env.Append (CPPPATH = [env['CXL_Gtk_inc_path']])
    #--KB: LPGPU2
from distutils.version import StrictVersion

def initStdc(env):
    print(env['CXXVERSION'])
    stdclib_dir = env['CXL_common_dir'] + '/Lib/Ext/libstdc/6.0.16/CentOS64/'
#++AT:LPGPU2
    if (StrictVersion(env['CXXVERSION']) > StrictVersion('5.0.0') and
        env['CXL_bitness'] == '64' and env['CXL_arch'] == 'aarch64'):
        stdclib_dir = env['CXL_common_dir'] + '/Lib/Ext/libstdc/6.0.22/CentOS64/'
    elif (StrictVersion(env['CXXVERSION']) > StrictVersion('5.0.0') and
        not(env['CXL_bitness'] == '32' and env['CXL_arch'] == 'x86')):
...
#++AT:LPGPU2
def copyBoostLibsToCXLLibDir(boost_lib_dir, env) :
...
#++AT:LPGPU2
      if (env['CXL_arch'] == 'x86_64'):
        ...
        copyBoostLibsToCXLLibDir(boost_lib_dir, env)
      elif (env['CXL_arch'] == 'x86'):
        boost_lib_dir = boost_lib_dir + '/x86/'
      elif (env['CXL_arch'] == 'armv7a'):
        boost_lib_dir = boost_lib_dir + '/armv7a/'
        copyBoostLibsToCXLLibDir(boost_lib_dir, env)
      elif (env['CXL_arch'] == 'aarch64'):
        boost_lib_dir = boost_lib_dir + '/aarch64/'
        copyBoostLibsToCXLLibDir(boost_lib_dir, env)
#--AT:LPGPU2
    else:
...
#++CF:LPGPU2
def UseSPIRVTools (env):
    SPIRVTools_dir = env['CXL_common_dir'] + '/Lib/Ext/SPIRV-Tools/2018.0/'
    SPIRVTools_inc = SPIRVTools_dir + 'include'
    SPIRVTools_lib = SPIRVTools_dir + 'lib'

    libsrc = []
    
    for file in os.listdir(SPIRVTools_lib):
        print(file)
        libsrc.append(SPIRVTools_lib + "/" + file)
        # Copy the shared libs and symlinks to the install location
        tmp = re.match( "^libSPIRV-Tools-shared" + ".so", file)
        if tmp:
            copySharedLibrary(env, file, SPIRVTools_lib, env['CXL_lib_dir']) 

    env.Prepend(LIBS = ["SPIRV-Tools"])
    env.Append(LIBPATH = [SPIRVTools_lib])
    env.Append(CPPPATH = [SPIRVTools_inc])
#--CF:LPGPU2
