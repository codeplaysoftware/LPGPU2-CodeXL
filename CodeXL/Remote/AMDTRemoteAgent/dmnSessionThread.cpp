...
//++TLRS: LPGPU2: For reading the results from CXLSystemInformationHelper
...
// Executable names and paths:
    ...
    //++TLRS: LPGPU2: Strings for calling the CXLSystemInformationHelper
    #define DMN_SYSTEMINFO_64_BIT_EXECUTABLE_NAME L"CXLSystemInformationHelper-x64" GDT_PROJECT_SUFFIX_W
    #define DMN_SYSTEMINFO_EXECUTABLE_EXTENSION L"exe"
    //--TLRS: LPGPU2: Strings for calling the CXLSystemInformationHelper
    ...
    //++TLRS: LPGPU2: Strings for calling the CXLSystemInformationHelper
    #define DMN_SYSTEMINFO_64_BIT_EXECUTABLE_NAME L"CXLSystemInformationHelper-x64"
    #define DMN_SYSTEMINFO_EXECUTABLE_EXTENSION L""
    //--TLRS: LPGPU2: Strings for calling the CXLSystemInformationHelper
#else
...
static bool FillPathBuffers(REMOTE_OPERATION_MODE mode, bool is64BitTarget, osFilePath& fileBuffer, osFilePath& pathBuffer)
{
    ...
    else if (mode == romPROFILE)
    {
        ...
        GT_IF_WITH_ASSERT(pConfigMgr != NULL)
        {
            ...
            //++KB: LPGPU2
            const wchar_t* profilerExeName = DMN_PROFILER_BACKEND_NORMAL_EXECUTABLE_NAME;
            bool isInternalVersion = pConfigMgr->IsInternalVersion();
            exePath.append(isInternalVersion ? DMN_PROFILER_BACKEND_INTERNAL_EXECUTABLE_NAME : profilerExeName).append(osFilePath::osExtensionSeparator)
...
#error Unknown build target!
    ...
    //++TLRS: LPGPU2: Fill the paths for the SYSTEMINFO process
    else if (mode == romSYSTEMINFO)
    {
        ret = false;        
        exePath.append(dirPath)
            .append(DMN_SYSTEMINFO_64_BIT_EXECUTABLE_NAME)
            .append(osFilePath::osExtensionSeparator)
            .append(DMN_SYSTEMINFO_EXECUTABLE_EXTENSION);
        ret = true;
    }
    //--TLRS: LPGPU2: Fill the paths for the SYSTEMINFO process
...
int dmnSessionThread::entryPoint()
{
    ...
    if (m_pConnHandler != NULL)
    {
        ...
        while (!isTerminationRequired && !m_isForcedTerminationRequired)
        {
            ...
            {
                //++TLRS: LPGPU2: Changing in order to reflect the Enum definition
                // The code that was here before was to enable the connection between CodeXL
                // and a remote agent running on an ARM device. Since it was dummy code it was
                // removed and can be retreived here: 
                // http://confluence:8090/display/ComputeSuiteTools/CodeXL+Remote+Connection+Backup+Code
                switch (buffer[0])
                {
                    case docLaunchRds:
                    {
...
//++AT:LPGPU2
                    ...
                    //case docPowerSetSamplingPeriod:
...
//++TLRS: LPGPU2: Query for SYCL availability
/// @brief  Return whether or not SYCL is available in the local or remote target.
/// @return bool    true = SYCL is available
///                 false = SYCL is not available.
bool dmnSessionThread::IsSYCLEnabled()
{
    bool retVal = false;

    // First, generate a unique pipe name for communicating with
    // the CXLSystemInformationHelper process.
    gtString pipeName{ L"AMDT_SI_PIPE_NAME" };
    ...
    pipeName << L"-" << dateAsString << L"-" << timeAsString;

    // Create socket server:
    auto pServer = std::unique_ptr<osPipeSocketServer>(new osPipeSocketServer(pipeName));
    pServer->setReadOperationTimeOut(OS_CHANNEL_DEFAULT_TIME_OUT);
    
    // Open the socket server side (Remote Agent side)
    const auto bRCAPISocket = pServer->open();

    GT_IF_WITH_ASSERT(bRCAPISocket)
    {
        auto cmdLineArgs = pipeName;
        cmdLineArgs << L" SYCL";

        std::vector<osEnvironmentVariable> envVars;
        GT_IF_WITH_ASSERT(CreateProcess(romSYSTEMINFO, cmdLineArgs, envVars))
        {
            GT_IF_WITH_ASSERT(pServer->waitForClientConnection())
            {
                gtString curStr;
                int mainListSize;
                int curListSize;
                ...
                *pServer >> curStr;
                ...
                for (int i = 0; i < mainListSize; i++)
                {
                    *pServer >> curStr;
                    ...
                    // there is at least one device available               
                    retVal = true;
                    break;
                }
            }            
