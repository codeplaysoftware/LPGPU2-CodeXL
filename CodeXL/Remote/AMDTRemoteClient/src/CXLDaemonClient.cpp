...
//++AT:LPGPU2
// Introduce the enums with the Android packet types
...
AMDT_REMOTE_CLIENT_API const gtUInt64 kTCPTimeoutDefault = 3000;
...
//++AT:LPGPU2
// Implement the utility function defined in LPGPU2_AndroidPacketGlobals
gtUInt32 toGtUI32(RemProtoToken tok)
{
  return static_cast<gtUInt32>(tok);
}
//--AT:LPGPU2
...
//++AT:LPGPU2
        if (m_isConnected)
        {
          GT_ASSERT_EX(m_tcpClient.close(), L"Closing TCP client.");
        }
//--AT:LPGPU2
        CleanAllocatedTasks();
    }
...
//++AT:LPGPU2
    /// @brief Read a chunk header from the tcp stream
    /// @param chunkHeader[out] Out param, an object into which to read the
    ///        chunk header from the stream
    /// @return The return status of the function once it completes. True =
    ///         no issues, else issues.
    /// @note We use bools rather than introducing a type alias to conform
    ///       with the existing codebase
    bool LPGPU2_ReadChunkHeader(ChunkHeader &chunkHeader)
    {
      // Read
      bool rc = m_tcpClient.read(reinterpret_cast<gtByte *>(&chunkHeader), CRP_CHUNK_HEADER_SIZE);
      // Convert endianess
      GT_IF_WITH_ASSERT(rc)
      {
        chunkHeader.id =
          static_cast<RemProtoToken>(
            lpgpu2::swapEndianess(static_cast<gtUInt32>(chunkHeader.id)));
        chunkHeader.payloadSize =
          lpgpu2::swapEndianess(chunkHeader.payloadSize);
      }

      OS_OUTPUT_FORMAT_DEBUG_LOG(OS_DEBUG_LOG_INFO,
          L"DMN Client: The read chunk header is id: %d",
          static_cast<gtUInt32>(chunkHeader.id));

      return rc;
    }

    /// @brief Read an entire chunk from the tcp stream
    /// @param chunk[out] Out param, an object into which to read the chunk from the stream. If
    ///        the chunk.size() > 0,  there are more chunks to be received. When
    ///        chunk.sice() == 0, there are no more chunks to receive.
    /// @return The return status of the function once it completes. True =
    ///         no issues, else issues.
    /// @note We use bools rather than introducing a type alias to conform
    ///       with the existing codebase
    bool LPGPU2_ReceiveCollectedAndroidChunk(std::vector<gtByte> &chunk)
    {
      bool ret = false;
      ...
      {
        // Receive the header to see how big the payload is
        ChunkHeader chunkHeader;
        bool rc = LPGPU2_ReadChunkHeader(chunkHeader);

        GT_IF_WITH_ASSERT(rc)
        {
          // When a data packet with size 0 is sent, that indicates the end of the collection on
          // the device side
          if (chunkHeader.id == RemProtoToken::CRP_DATA_PACKET && chunkHeader.payloadSize == 0U)
          {
            ret = true;
          }
          else
          {
            ...
            {
              // Read in the packet
              chunk.resize(chunkHeader.payloadSize);
              ret = m_tcpClient.read(chunk.data(), chunkHeader.payloadSize);
            }
          }
        }
      }
    ...
    /// @brief Used to flush the stream at the end of profiling. Consumes all
    ///        the available data in the stream without checking the types of
    ///        the chunks, but only their size.
    /// @param chunk[out] Out param, an object into which to flush the stream.
    ///                   It will also contain the chunk header at its front.
    /// @return The return status of the function once it completes. True =
    ///         no issues, else issues.
    /// @note We use bools rather than introducing a type alias to conform
    ///       with the existing codebase
    bool LPGPU2_FlushStream(std::vector<gtByte> &chunk, gtSize_t &dataRead)
    {
      bool ret = false;
      dataRead = 0;
      ...
      GT_IF_WITH_ASSERT(m_isConnected)
      {
        // Receive the header to see how big the payload is
        ChunkHeader chunkHeader;
        ret = LPGPU2_ReadChunkHeader(chunkHeader);

        GT_IF_WITH_ASSERT(ret)
        {
          // Set size of buffer
          chunk.resize(CRP_CHUNK_HEADER_SIZE + chunkHeader.payloadSize);
          // Write the header into the buffer
          memcpy(chunk.data(), &chunkHeader, CRP_CHUNK_HEADER_SIZE);

          // Read the rest of the data into the buffer
          if (chunkHeader.payloadSize > 0)
          {
            ret = m_tcpClient.readAvailableData(chunk.data(), chunk.size(),
                dataRead);
          }
        }
      }
    ...
    /// @brief Write a chunk header into a data packet
    /// @param chunkID The ID to write in the header
    /// @param packet The chunk of data onto which to write the header
    /// @param payloadSize The size of the payload to write into the header
    /// @return The return status of the function once it completes. True =
    ///         no issues, else issues.
    ///         Always returns true.
    /// @note We use bool rather than introducing a type alias to conform
    ///       with the existing codebase
    bool LPGPU2_WriteHeaderToPacket(RemProtoToken chunkID,
        std::vector<gtByte> &packet, gtUInt32 payloadSize)
    {
      packet.resize(payloadSize + CRP_CHUNK_HEADER_SIZE);

      gtUInt32 chunkID_u32 = lpgpu2::swapEndianess(toGtUI32(chunkID));
      payloadSize = lpgpu2::swapEndianess(payloadSize);

      // SAMS's service expect big-endian data
      memcpy(packet.data(), &chunkID_u32, sizeof(chunkID_u32));
      memcpy(packet.data() + sizeof(chunkID_u32), &payloadSize, sizeof(payloadSize));

      return true;
    }

    /// @brief Write an entire chunk to the tcp stream
    /// @param chunkID The ID to write in the header of the chunk
    ...
    /// @note We use bool rather than introducing a type alias to conform
    ///       with the existing codebase
    bool LPGPU2_WriteChunk(RemProtoToken chunkID, const gtByte *payload = nullptr, gtUInt32 payloadSize = 0U)
    {
      // Make copy of payload in buffer larger enough to also store the header
      std::vector<gtByte> packet(payloadSize + CRP_CHUNK_HEADER_SIZE);
      auto rc = LPGPU2_WriteHeaderToPacket(chunkID, packet, payloadSize);
      if (!rc)
      {
        return false;
      }

      std::copy(payload, payload + payloadSize, (packet.data() + CRP_CHUNK_HEADER_SIZE));

      return m_tcpClient.write(packet.data(), packet.size());
    }

    /// @brief Query the Remote Agent if it is alive
    /// @return The return status of the function once it completes. True =
    ///         no issues, else issues.
    /// @note We use bool rather than introducing a type alias to conform
    ///       with the existing codebase
    bool LPGPU2_IsAndroidClientAlive()
    {
      bool ret = false;
      OS_DEBUG_LOG_TRACER_WITH_RETVAL(ret);
      ...
      {
        // Send the opcode.
        ret = LPGPU2_WriteChunk(RemProtoToken::CRP_ALIVE);
        GT_ASSERT(ret);

        // Verify.
        ChunkHeader chunkHeader;
        bool rc = LPGPU2_ReadChunkHeader(chunkHeader);
        ret = (rc && chunkHeader.id == RemProtoToken::CRP_RESPONSE_ACK);

        GT_IF_WITH_ASSERT(ret)
        {
          OS_OUTPUT_DEBUG_LOG(L"DMN Client: The Android device confirmed that it is alive.", OS_DEBUG_LOG_INFO);
        }
      }
    ...
    /// @brief Write a chunk of bytes to a file in the filesystem
    /// @param filePath The path to the file to write/create
    /// @param data The data to write on disk
    /// @param size Size of the data to write on disk
    /// @return The return status of the function once it completes. True =
    ///         no issues, else issues.
    /// @note We use bool rather than introducing a type alias to conform
    ///       with the existing codebase
    bool LPGPU2_WriteChunkToFilePath(const osFilePath& filePath,
        const gtByte *data, gtSizeType size)
    {
      bool retVal = false;
      ...
      {
        retVal = fileToWrite.write(data, size);
      }

      return retVal;
    }

    /// @brief Send a file into the stream and mark it as a certain chunk type
    ///        for the remote device to know what type it is
    /// @param filePath The path to the file to read and send
    /// @return The return status of the function once it completes. True =
    ///         no issues, else issues.
    /// @note We use bool rather than introducing a type alias to conform
    ///       with the existing codebase
    bool LPGPU2_SendFromFile(RemProtoToken chunkType, const osFilePath& filePath)
    {
        bool retVal = false;
            ...
            GT_IF_WITH_ASSERT(rcOpen)
            {
                ...
                GT_IF_WITH_ASSERT(rcSize)
                {
                    if (fileSize > 0)
                    {
                        ...
                        gtSizeType payloadSize = fileSize;

                        // Assign a buffer large enough to contain the payload
                        std::vector<gtByte> payload(payloadSize);

                        // Read the file into the buffer:
                        gtSize_t readData = 0;
                        retVal = fileToRead.readAvailableData(payload.data(), payloadSize, readData);

                        // Make sure we read the whole file:
                        GT_ASSERT(readData == payloadSize && retVal);

                        // Send the chunk
                        retVal = LPGPU2_WriteChunk(chunkType, payload.data(), payloadSize);
                    }
                    else
                    {
    ...
    /// @brief Utility function to read a file sent from the device into a
    ///        file on disk
    /// @param localTargetFilename Path to the file onto which to write the
    ///        retrieve info file
    /// @return The return status of the function once it completes. True =
    ///         no issues, else issues.
    /// @note We use bool rather than introducing a type alias to conform
    ///       with the existing codebase
    bool LPGPU2_GetFileFromAndroidDevice(const gtString& localTargetFileName,
        gtSize_t payloadSize)
    {
      bool ret = false;
      OS_DEBUG_LOG_TRACER_WITH_RETVAL(ret);

      if (!localTargetFileName.isEmpty())
      {
        GT_IF_WITH_ASSERT(m_isConnected)
        {
          std::vector<gtByte> payload(payloadSize);
          ret = m_tcpClient.read(payload.data(), payload.size());

          GT_IF_WITH_ASSERT(ret)
          {
            osFilePath outputFilePath(localTargetFileName);
            ret = LPGPU2_WriteChunkToFilePath(outputFilePath, payload.data(),
                payload.size());

            GT_IF_WITH_ASSERT(ret)
            {
              OS_OUTPUT_DEBUG_LOG(L"DMN Client: The file was successfully received and written to disk.", OS_DEBUG_LOG_INFO);
            }
          }
        }
      }
    ...
    /// @brief Retrieve the device info (TargetCharacteristics.xml)
    /// @param localTargetFilename Path to the file onto which to write the
    ///        retrieve info file
    /// @return The return status of the function once it completes. True =
    ///         no issues, else issues.
    /// @note We use bool rather than introducing a type alias to conform
    ///       with the existing codebase
    bool LPGPU2_GetAndroidDeviceInfo(const gtString& localTargetFileName)
    {
      bool ret = false;
      OS_DEBUG_LOG_TRACER_WITH_RETVAL(ret);
      ...
      if (!localTargetFileName.isEmpty())
      {
        ...
        {
          // Send the opcode.
          bool rc = LPGPU2_WriteChunk(RemProtoToken::CRP_GET_DEVICE_INFO);
          if (!rc)
          {
            GT_ASSERT_EX(false, L"Failed to write chunk!");
            return false;
          }

          // Receive the ack first and then read the file in
          ChunkHeader chunkHeader;
          rc = LPGPU2_ReadChunkHeader(chunkHeader);
          ret = (rc && chunkHeader.id == RemProtoToken::CRP_RESPONSE_ACK &&
              chunkHeader.payloadSize > 0U);

          GT_IF_WITH_ASSERT(ret)
          {
            ret = LPGPU2_GetFileFromAndroidDevice(localTargetFileName,
              chunkHeader.payloadSize);
          }
        }
      }
    ...
    /// @brief Send the collection options (CollectionDefinition.xml)
    /// @param fileName Path to the file containing the collection options
    /// @return The return status of the function once it completes. True =
    ///         no issues, else issues.
    /// @note We use bool rather than introducing a type alias to conform
    ///       with the existing codebase
    bool LPGPU2_SendAndroidCollectionOptions(const gtString &fileName)
    {
      bool ret = false;
      OS_DEBUG_LOG_TRACER_WITH_RETVAL(ret);
      ...
      GT_IF_WITH_ASSERT(m_isConnected)
      {
          ...
          ret = LPGPU2_SendFromFile(RemProtoToken::CRP_SEND_COLLECTION_OPTIONS,
              filePath);
          GT_IF_WITH_ASSERT(ret)
          {
              // Receive the ack first and then read the file in
              ...
              GT_IF_WITH_ASSERT(ret)
              {
                OS_OUTPUT_DEBUG_LOG(L"DMN Client: Successfully retrieved Android collection options XML file", OS_DEBUG_LOG_INFO);
              }
          }
        }
    ...
    /// @brief Start collection of profile data on the device
    /// @return The return status of the function once it completes. True =
    ///         no issues, else issues.
    /// @param appName Name of the application to launch for profiling
    /// @note We use bool rather than introducing a type alias to conform
    ///       with the existing codebase
    bool LPGPU2_StartAndroidCollection(const gtString &appName)
    {
      bool ret = false;
      OS_DEBUG_LOG_TRACER_WITH_RETVAL(ret);
      ...
      {
        // Set the payload size:
        gtSizeType payloadSize = appName.length();

        // Use a buffer large enough to contain the payload
        std::vector<gtByte> payload(payloadSize);
        // Copy data into the buffer
        // We do not account for the \0 character as the receiving end does
        // not account for it
        memcpy(payload.data(), appName.asUTF8CharArray(), payloadSize);

        // Send the chunk
        auto rc = LPGPU2_WriteChunk(RemProtoToken::CRP_START_COLLECTION,
        ...
        GT_IF_WITH_ASSERT(ret)
        {
          OS_OUTPUT_DEBUG_LOG(L"DMN Client: Successfully started Android collection", OS_DEBUG_LOG_INFO);
        }
      }
    ...
    /// @brief Stop collection of profile data on the device
    /// @param flush Whether we want to retrieve the last remaining data which
    ///        was buffered on the device before we asked it to stop
    /// @return The return status of the function once it completes. True =
    ///         no issues, else issues.
    /// @note We use bool rather than introducing a type alias to conform
    ///       with the existing codebase
    bool LPGPU2_StopAndroidCollection()
    {
      bool ret = false;
      OS_DEBUG_LOG_TRACER_WITH_RETVAL(ret);
      ...
      {
        // Send the opcode.
        ret = LPGPU2_WriteChunk(RemProtoToken::CRP_STOP_COLLECTION);
        if (!ret)
        {
          GT_ASSERT_EX(false, L"Failed to write chunk!");
        }
        ...
        // Receive the ack first and then read the file in
        ChunkHeader chunkHeader;
        auto rc = LPGPU2_ReadChunkHeader(chunkHeader);
        ret = (rc && chunkHeader.id == RemProtoToken::CRP_RESPONSE_ACK);

        if(!ret)
        {
          OS_OUTPUT_DEBUG_LOG(L"Err receiving ACK", OS_DEBUG_LOG_ERROR);
        }
      }
    ...
    /// @brief Get the list of profilable applications from the device
    /// @param supportedApps[out] Out param, it will be filled with the supported apps names
    /// @return The return status of the function once it completes. True =
    ///         no issues, else issues.
    /// @note We use bool rather than introducing a type alias to conform
    ///       with the existing codebase
    bool LPGPU2_GetAndroidSupportedApps(gtVector<gtString> &supportedApps)
    {
      bool ret = false;
      OS_DEBUG_LOG_TRACER_WITH_RETVAL(ret);
      ...
      {
        // Send the opcode.
        bool rc = LPGPU2_WriteChunk(RemProtoToken::CRP_GET_SUPPORTED_APPS);
        if (!rc)
        {
          GT_ASSERT_EX(false, L"Failed to write chunk!");
          return false;
        ...
        GT_IF_WITH_ASSERT(ret)
        {
          ...
          std::vector<gtByte> payload(chunkHeader.payloadSize + 1);
          gtSizeType dataRead = 0;
          rc = m_tcpClient.readAvailableData(payload.data(),
              chunkHeader.payloadSize, dataRead);

          ret = (rc && dataRead == chunkHeader.payloadSize);

          GT_IF_WITH_ASSERT(ret)
          {
            payload.back() = '\0';
            // We now need to parse this string; the apps list is formatted as such:
            //   "com.appone.test:com.apptwo.test:etc..."
            gtString payloadString;
            payloadString.fromUtf8String(payload.data());
            gtStringTokenizer tokeniser{payloadString, gtString{L":"}};

            gtString token;
            while (tokeniser.getNextToken(token))
            {
              supportedApps.push_back(token);
            }

            ret = true;
            OS_OUTPUT_DEBUG_LOG(L"DMN Client: The Android device name was successfully retrieved.", OS_DEBUG_LOG_INFO);
          }
        }
      }
    ...
    /// @brief Clean the profile state on the remote device
    /// @return The return status of the function once it completes. True =
    ///         no issues, else issues.
    /// @note We use bool rather than introducing a type alias to conform
    ///       with the existing codebase
    bool LPGPU2_CleanAndroidState()
    {
      bool ret = false;
      OS_DEBUG_LOG_TRACER_WITH_RETVAL(ret);
      ...
      {
        // Send the opcode.
        bool rc = LPGPU2_WriteChunk(RemProtoToken::CRP_CLEAN_STATE);
        if (!rc)
        {
          GT_ASSERT_EX(false, L"Failed to write chunk!");
          return false;
        ...
        GT_IF_WITH_ASSERT(ret)
        {
          OS_OUTPUT_DEBUG_LOG(L"DMN Client: Successfully cleaned Android device state", OS_DEBUG_LOG_INFO);
        }
      }
    ...
    /// @brief Disconnect from the Android remote device
    /// @return The return status of the function once it completes. True =
    ///         no issues, else issues.
    /// @note We use bool rather than introducing a type alias to conform
    ///       with the existing codebase
    bool LPGPU2_DisconnectFromAndroidDevice()
    {
      bool ret = false;
      OS_DEBUG_LOG_TRACER_WITH_RETVAL(ret);
      ...
      {
        // Send the opcode.
        ret = LPGPU2_WriteChunk(RemProtoToken::CRP_DISCONNECT);
        GT_ASSERT(ret);

        GT_IF_WITH_ASSERT(ret)
        {
          OS_OUTPUT_DEBUG_LOG(L"DMN Client: The Android device disconnected.", OS_DEBUG_LOG_INFO);
        }
      }
    ...
    /// @brief Get the Remote Agent info data structure
    /// @param localTargetFilename Path to the file onto which to write the
    ///        retrieve info file
    /// @return The return status of the function once it completes. True =
    ///         no issues, else issues.
    /// @note We use bool rather than introducing a type alias to conform
    ///       with the existing codebase
    /// @note This differs from the retrieval of the TargetCharacteristics.xml
    ///       file and goes in parallel with it and not orthogonally
    bool LPGPU2_GetAndroidRemoteAgentInfo(const gtString& localTargetFileName)
    {
      bool ret = false;
      OS_DEBUG_LOG_TRACER_WITH_RETVAL(ret);
      ...
      {
        // Send the opcode.
        bool rc = LPGPU2_WriteChunk(RemProtoToken::CRP_RAGENT_INFO);
        if (!rc)
        {
          GT_ASSERT_EX(false, L"Failed to write chunk!");
          return false;
...
//++AT:LPGPU2
    bool TerminateWholeSession(bool isAndroid)
    {
        bool ret = true;
        OS_DEBUG_LOG_TRACER_WITH_RETVAL(ret);
        ...
        if (m_isConnected)
        {
          ...
          {
//++AT:LPGPU2
// TODO Uncomment here once SAMS implements a service which we can disconnect from and reconnect
            //LPGPU2_DisconnectFromAndroidDevice();
//--AT:LPGPU2
          }
          else
          {
...
//--AT:LPGPU2
    ...
    //++TLRS: LGPU2: Reads if SYCL is enabled from the remote server
    bool IsSYCLEnabled()
    {
        auto bReturn = false;
        auto bIsSYCLEnabled = false;
        m_tcpClient << docIsSYCLEnabled;
        // Verify
        gtInt32 opStatus = dosFailure;
        m_tcpClient >> opStatus;
        bReturn = (opStatus == dosSuccess);
        GT_ASSERT_EX(bReturn, L"OpCode receive ack.");

        if (bReturn)
        {
            m_tcpClient >> bIsSYCLEnabled;
            // Verify
            opStatus = dosFailure;
            bReturn = (opStatus == dosSuccess);
            GT_ASSERT_EX(bReturn, L"OpCode receive ack.");
...
//++AT:LPGPU2
    /// @brief Utility function which returns whether the client is connected to the device or not
    /// @return The return status of the function once it completes. True =
    ///         no connected, false = disconnected.
    /// @note We use bool rather than introducing a type alias to conform
    ///       with the existing codebase
    bool IsConnected()
    {
...
//++AT:LPGPU2
bool CXLDaemonClient::TerminateWholeSession(bool isAndroid)
{
    bool ret = false;

    if (m_pImpl != NULL)
    {
        ret = m_pImpl->TerminateWholeSession(isAndroid);
    }
...
//++TLRS: LPGPU2:
bool CXLDaemonClient::IsSYCLEnabled()
{
    auto bReturn = false;
    if (m_pImpl != nullptr)
    {
        bReturn = m_pImpl->IsSYCLEnabled();
    }
    return bReturn;
}
//--TLRS: LPGPU2:
...
//++AT:LPGPU2
bool CXLDaemonClient::LPGPU2_IsAndroidClientAlive()
{
    bool result = false;
    if (m_pImpl != nullptr)
    {
        result = m_pImpl->LPGPU2_IsAndroidClientAlive();
    }

    return result;
}

bool CXLDaemonClient::LPGPU2_GetAndroidDeviceInfo(const gtString& localTargetFileName)
{
    bool result = false;
    if (m_pImpl != nullptr)
    {
        result = m_pImpl->LPGPU2_GetAndroidDeviceInfo(localTargetFileName);
    }

    return result;
}

bool CXLDaemonClient::LPGPU2_SendAndroidCollectionOptions(const gtString &fileName)
{
    bool result = false;
    if (m_pImpl != nullptr)
    {
        result = m_pImpl->LPGPU2_SendAndroidCollectionOptions(fileName);
    }

    return result;
}

bool CXLDaemonClient::LPGPU2_StartAndroidCollection(const gtString &appName)
{
    bool result = false;
    if (m_pImpl != nullptr)
    {
        result = m_pImpl->LPGPU2_StartAndroidCollection(appName);
    }

    return result;
}
  
bool CXLDaemonClient::LPGPU2_StopAndroidCollection()
{
    bool result = false;
    if (m_pImpl != nullptr)
    {
        result = m_pImpl->LPGPU2_StopAndroidCollection();
    }

    return result;
}

bool CXLDaemonClient::LPGPU2_GetAndroidSupportedApps(gtVector<gtString> &supportedApps)
{
    bool result = false;
    if (m_pImpl != nullptr)
    {
        result = m_pImpl->LPGPU2_GetAndroidSupportedApps(supportedApps);
    }

    return result;
}

bool CXLDaemonClient::LPGPU2_CleanAndroidState()
{
    bool result = false;
    if (m_pImpl != nullptr)
    {
        result = m_pImpl->LPGPU2_CleanAndroidState();
    }

    return result;
}

bool CXLDaemonClient::LPGPU2_WriteChunk(RemProtoToken chunkID, const gtByte *payload, gtUInt32 payloadSize)
{
    bool result = false;
    if (m_pImpl != nullptr)
    {
        result = m_pImpl->LPGPU2_WriteChunk(chunkID, payload, payloadSize);
    }

    return result;
}

bool CXLDaemonClient::LPGPU2_DisconnectFromAndroidDevice()
{
    bool result = false;
    if (m_pImpl != nullptr)
    {
        result = m_pImpl->LPGPU2_DisconnectFromAndroidDevice();
    }

    return result;
}
 
bool CXLDaemonClient::LPGPU2_ReceiveCollectedAndroidChunks(std::vector<gtByte> &chunk)
{
    bool result = false;
    if (m_pImpl != nullptr)
    {
        result = m_pImpl->LPGPU2_ReceiveCollectedAndroidChunk(chunk);
    }

    return result;
}

bool CXLDaemonClient::LPGPU2_GetAndroidRemoteAgentInfo(
    const gtString& localTargetFileName)
{
    bool result = false;
    if (m_pImpl != nullptr)
    {
        result =
          m_pImpl->LPGPU2_GetAndroidRemoteAgentInfo(localTargetFileName);
    }

    return result;
}

bool CXLDaemonClient::IsConnected() const
{
    bool result = false;
    if (m_pImpl != nullptr)
    {
        result = m_pImpl->IsConnected();
    }

    return result;
}

bool CXLDaemonClient::LPGPU2_ReceiveHeader(ChunkHeader &header)
{
    bool result = false;
    if (m_pImpl != nullptr)
    {
        result = m_pImpl->LPGPU2_ReadChunkHeader(header);
    }

    return result;
}

bool CXLDaemonClient::LPGPU2_FlushStream(std::vector<gtByte> &chunk,
    gtSize_t &dataRead)
{
    bool result = false;
    if (m_pImpl != nullptr)
    {
        result = m_pImpl->LPGPU2_FlushStream(chunk, dataRead);
    }
