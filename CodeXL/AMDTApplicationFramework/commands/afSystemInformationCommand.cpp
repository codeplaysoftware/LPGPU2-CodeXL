...
//++TLRS: LPGPU2: Needed for some extra strings
...
bool afSystemInformationCommand::CollectAllOpenCLDevicesInformation(gtList< gtList <gtString> >& infoData)
{
    ...
    //++TLRS: LPGPU2: Changing the call to conform to the enum class
    bool res64bit = CollectDevicesInformation(listOf64BitRows, afApplicationType::af64BitApp, EnumDeviceType::kOpenCL);
    bool res32bit = CollectDevicesInformation(listOf32BitRows, afApplicationType::af32BitApp, EnumDeviceType::kOpenCL);
    //--TLRS: LPGPU2: Changing the call to conform to the enum class
...
// ---------------------------------------------------------------------------
//++TLRS: LPGPU2: Generalizing the call to collect device information
bool afSystemInformationCommand::CollectDevicesInformation(gtList<gtList<gtString>>& infoData, afApplicationType appType, EnumDeviceType deviceType)
//--TLRS: LPGPU2: Generalizing the call to collect device information
{
    bool retVal = false;
    ...
    GT_IF_WITH_ASSERT(rcAPISocket)
    {
        ...
        //++TLRS: LPGPU2: Building the command line argument to call CXLSystemInformationHelper
        switch (deviceType)
        {
            case EnumDeviceType::kOpenCL:
                cmdLineArgs << L" " << AP_STR_OpenCL;
                break;
            case EnumDeviceType::kSYCL:
                cmdLineArgs << L" " << AP_STR_SYCL;
                break;
            default:
                break;
        }
        ...
        if (afApplicationType::af32BitApp == appType)
        {
            fileName = AF_STR_SystemInformationCommandHelper32Name;
        }
        else if (afApplicationType::af64BitApp == appType)
        {
...
// ---------------------------------------------------------------------------
// Name:        afSystemInformationCommand::CollectAllSYCLDevicesInformation
// Description: Get the SYCL devices available in the system and insert it one
//              by one to the infoData map.
// Return Val:  bool - Success / failure.
// Author:      Thales Sabino
// Date:        02/01/2018
// ---------------------------------------------------------------------------
bool afSystemInformationCommand::CollectAllSYCLDevicesInformation(gtList<gtList<gtString>>& infoData)
{
    // The SYCL runtime only works on Windows 64Bits and we only support Linux 64Bits.
    // No need to call with af32BitApp
    return CollectDevicesInformation(infoData, afApplicationType::af64BitApp, EnumDeviceType::kSYCL);
}
//--TLRS: LPGPU2: Collects information about SYCL devices
