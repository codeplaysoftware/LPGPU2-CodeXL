...
// ---------------------------------------------------------------------------
// Name:        afProjectManager::closeCurrentProject. CodeXL also has a
//              (disguised) close project event by another name - 
//              apEvent::APP_GLOBAL_VARIABLE_CHANGED + 
//              afGlobalVariableChangedEvent::CURRENT_PROJECT).
// Description: Notify the extension the project is closed.
// Arguments:   int extensionIndex
// Return Val:  bool - Success / failure.
// Author:      Illya Rudkin
// Date:        13/1/2017
// ---------------------------------------------------------------------------
bool afProjectManager::closeCurrentProject(int extensionIndex)
{
  bool retVal = false;
  ...
  GT_IF_WITH_ASSERT((extensionIndex >= 0) && (extensionIndex < (int)m_projectSettingsExtensions.size()))
  {
    ...
    GT_IF_WITH_ASSERT(pExtension != nullptr)
    {
      retVal = pExtension->CloseCurrentProject();
    }
  }
  return retVal;
}
...
/// @param[out]  vInvalidExtnTreePath The extension that had invalid setting.
/// @return      bool true on success, else false equal settings error.
bool afProjectManager::AreSettingsValidSecondPass(gtString &vInvalidMessageStr, gtString &vInvalidExtnTreePath)
{
  bool bOk = true;

  for (size_t i = 0; i < m_projectSettingsExtensions.size(); i++)
  {
    afProjectSettingsExtension *pCurrentExtn = m_projectSettingsExtensions[i];
    GT_IF_WITH_ASSERT(pCurrentExtn != nullptr)
    {
      bOk = pCurrentExtn->AreSettingsValidSecondPass(vInvalidMessageStr);
      if (!bOk)
      {
        // Break on the first error
        vInvalidExtnTreePath = pCurrentExtn->ExtensionTreePathAsString();
        break;
      }
    }
  }
...
//++AT:LPGPU2
/// @brief Set the remote target (Remote Agent) hostname for the project
/// @note These functions return void because that conforms with how CXL does things,
///       and being in CXL's codebase here, we conform with such rules
void afProjectManager::SetRemoteTargetHostname(const gtString& remoteHostName)
{
  m_currentProject.SetRemoteTargetHostname(remoteHostName);
}

/// @brief Set the remote target (Remote Agent) port for the project
/// @note These functions return void because that conforms with how CXL does things,
///       and being in CXL's codebase here, we conform with such rules
void afProjectManager::setRemoteTargetDaemonPort(gtUInt16 daemonConnectionPort)
{
  m_currentProject.setRemoteTargetDaemonPort(daemonConnectionPort);
}
//--AT:LPGPU2
...
//++AT:LPGPU2
/// @brief Set the the project mode to Android remote debugging, compared to Local or Remote
///        (but AMD based)
/// @note These functions return void because that conforms with how CXL does things,
///       and being in CXL's codebase here, we conform with such rules
void afProjectManager::SetAndroidRemoteDebugging()
{
  m_currentProject.setAndroidRemoteDebugging();
}
//--AT:LPGPU2