...
#define AF_NEW_PROJECT_BUTTON_STANDARD_SIZE 21  // Copied in CCVG_wgtButtonDirFolder.cpp
...
afNewProjectDialog::afNewProjectDialog()
    : QDialog(afMainAppWindow::instance()),
      ...
      m_pWinStoreAppRadioButton(nullptr), m_pRemoteHostRadioButton(nullptr), m_pLocalHostRadioButton(nullptr),
//++AT:LPGPU2
#if LPGPU2_ANDROID_REMOTE_PROTOCOL_COMPILE_IN == 1
      m_pAndroidDeviceRadioButton(nullptr),
#endif
//--AT:LPGPU2
      m_pDummyRemoteWidgetA(nullptr),
...
//++AT:LPGPU2
    // When the application is first launched, the host type is always
...
//++IOR: LPGPU2 Code Coverage Analysis
    // Move the creation of widgets to here as needed in other parts of this 
    // dialog's code base and not trip asserts
...
//++AT:LPGPU2
#if LPGPU2_ANDROID_REMOTE_PROTOCOL_COMPILE_IN == 1
    m_pAndroidDeviceRadioButton = new QRadioButton(AF_LPGPU2_STR_newProject_LPGPU2DeviceRadio);
#endif
//--AT:LPGPU2
    ...
    // The General section caption bar
    ...
    // Should CodeXL be run with only the plugin LPGPU2 Code Coverage plugin
    // enabled, there is no other execution modes, then we do not want to
    // display in the project settings dialog options for those other modes.
...
//++AT:LPGPU2
#if LPGPU2_ANDROID_REMOTE_PROTOCOL_COMPILE_IN == 1
    m_pAndroidDeviceRadioButton->setToolTip(AF_LPGPU2_STR_newProject_LPGPU2DeviceRadioToolTip);
#endif
//--AT:LPGPU2
...
//++AT:LPGPU2
#if LPGPU2_ANDROID_REMOTE_PROTOCOL_COMPILE_IN == 1
    m_pRemoteHostLayoutB->addWidget(m_pAndroidDeviceRadioButton, 0, Qt::AlignLeft);
#endif
//--AT:LPGPU2
...
//++AT:LPGPU2
#if LPGPU2_ANDROID_REMOTE_PROTOCOL_COMPILE_IN == 1
    rc = connect(m_pAndroidDeviceRadioButton, SIGNAL(toggled(bool)), this, SLOT(OnHostRadioButtonSelection(bool)));
    GT_ASSERT(rc);
#endif
//--AT:LPGPU2
...
//++AT:LPGPU2
        // Add this branch so that, in case the device is of Android type, we can let the
        // project manager know that the profiling session is of type Android and in turn
        // have it saved in the project settings
        if (m_projectSettings.isAndroidDeviceTarget())
        {
          projectManager.SetAndroidRemoteDebugging();
        }
//--AT:LPGPU2
...
//++AT:LPGPU2
        // If android, do not use the main page's IP and port settings but use the ones set by the PP LPGPU2
        // settings extension
        if (m_projectSettings.isAndroidDeviceTarget())
        {
          m_projectSettings.SetRemoteTargetHostname(projectManager.GetRemoteTargetHostName());
          m_projectSettings.setRemoteTargetDaemonPort(projectManager.GetRemoteTargetDaemonPort());
        }
//--AT:LPGPU2
...
//++AT:LPGPU2
        m_pAndroidDeviceRadioButton->setChecked(
            m_projectSettings.isAndroidDeviceTarget());
//--AT:LPGPU2
...
bool afNewProjectDialog::AreSettingsValid(gtString& invalidMessageStr, gtString& invalidExtensionTreePath) const
{
    ...
    if (retVal)
    {
      ...
      retVal = retVal && afProjectManager::instance().AreSettingsValidSecondPass(invalidMessageStr, invalidExtensionTreePath);
    }
...
{
//++AT:LPGPU2
    // Check which host was selected:
    if (sender() == m_pAndroidDeviceRadioButton)
    {
      // Disable/Enable all the non-android widgets in the main settings page
      AdjustGUIToLPGPU2HostChange(isSelected);
    ...
    else if (sender() == m_pRemoteHostRadioButton)
    {
      // Adjust the GUI to the host selection:
      adjustGuiToHostChange(isSelected);

      if (isSelected)
      {
        NotifyExtensionsOfHostTypeChange(lpgpu2::HostType::Remote);
      }
    }
    else if (sender() == m_pLocalHostRadioButton)
    {
      // We don't adjust this GUI here in this case because it is sufficient
      // to simply do it from the branch of the remote hos button
...
//++AT:LPGPU2
/// @brief Inform other extensions about the change of host type
/// @param hostType The new host type
void afNewProjectDialog::NotifyExtensionsOfHostTypeChange(
    lpgpu2::HostType hostType)
{
  // Get the project manager instance:
  auto &projectsManager = afProjectManager::instance();
  const auto amountOfExtensions = projectsManager.amountOfProjectExtensions();

  for (int i = 0; i < amountOfExtensions; i++)
  {
    // Necessary to pass a string
    gtString extensionName;
    auto *pCurrentWidget = projectsManager.getExtensionSettingsWidget(i, extensionName);
    GT_IF_WITH_ASSERT(pCurrentWidget != nullptr)
    {
      auto *pCurrentExtension =
        qobject_cast<afProjectSettingsExtension*>(pCurrentWidget);

      if (pCurrentExtension != nullptr)
      {
        pCurrentExtension->NotifyHostChange(hostType);
      }
    }
  }
}
...
void afNewProjectDialog::SaveProfileExtnParamProfileSessionType(const gtString& sessionType) 
{ 
  m_projectSettings.setLastActiveSessionType(sessionType); 
}

//--IOR: LPGPU2 Code SYCL implementation runtime location
//++AT:LPGPU2
/// @brief Enable or disable widgets on the general project settings page
///        depending on whether the LPGPU2 mode is selected or not. Disable
///        AMD-related widgets if the mode is LPGPU2.
/// @param isLPGPU2RadioBtnSelected Whether the radio button is selected or not
void afNewProjectDialog::AdjustGUIToLPGPU2HostChange(
    bool isLPGPU2RadioBtnSelected)
{
  GT_IF_WITH_ASSERT((m_pRemoteHostIpLineEdit != nullptr) &&
    (m_pRemoteHostPortLabel                != nullptr) &&
    (m_pTestConnectionButton               != nullptr) &&
    (m_pRemoteHostPortLineEdit             != nullptr) &&
    (m_pRemoteHostAddressLabel             != nullptr) &&
    (m_pProgramExeTextEdit                 != nullptr) &&
    (m_pBrowseForExeButton                 != nullptr) &&
    (m_pBrowseForExeButton                 != nullptr) &&
    (m_pWorkingFolderTextEdit              != nullptr) &&
    (m_pWorkingFolderPathButton            != nullptr) &&
    (m_pProgramArgs                        != nullptr) &&
    (m_pEnvironmentVariablesTextEdit       != nullptr) &&
    (m_pSourceFilesDirectoryTextEdit       != nullptr) &&
    (m_pSourceFilesDirectoryButton         != nullptr) &&
    (m_pSourceCodeRootDirectoryTextEdit    != nullptr) &&
    (m_pSourceCodeRootDirectoryButton      != nullptr))
  {
    m_pRemoteHostIpLineEdit->setEnabled(!isLPGPU2RadioBtnSelected);
    m_pRemoteHostPortLabel->setEnabled(!isLPGPU2RadioBtnSelected);
    m_pRemoteHostAddressLabel->setEnabled(!isLPGPU2RadioBtnSelected);
    m_pTestConnectionButton->setEnabled(!isLPGPU2RadioBtnSelected);
    m_pProgramExeTextEdit->setEnabled(!isLPGPU2RadioBtnSelected);
    m_pBrowseForExeButton->setEnabled(!isLPGPU2RadioBtnSelected);
    m_pBrowseForExeButton->setEnabled(!isLPGPU2RadioBtnSelected);
    m_pWorkingFolderTextEdit->setEnabled(!isLPGPU2RadioBtnSelected);
    m_pWorkingFolderPathButton->setEnabled(!isLPGPU2RadioBtnSelected);
    m_pProgramArgs->setEnabled(!isLPGPU2RadioBtnSelected);
    m_pEditEnvironmentVariables->setEnabled(!isLPGPU2RadioBtnSelected);
    m_pSourceFilesDirectoryTextEdit->setEnabled(!isLPGPU2RadioBtnSelected);
    m_pSourceFilesDirectoryButton->setEnabled(!isLPGPU2RadioBtnSelected);
    m_pSourceCodeRootDirectoryTextEdit->setEnabled(!isLPGPU2RadioBtnSelected);
    m_pSourceCodeRootDirectoryButton->setEnabled(!isLPGPU2RadioBtnSelected);
  }
}
//--AT:LPGPU2
