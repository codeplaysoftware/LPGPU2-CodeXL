...
//++AT:LPGPU2
            // Check if we are in an Android session.
            bool isAndroidSession = false;

            // Get daemon configurations: are we in an Android session.
            TiXmlNode* pIsAndroidSession = getProjectRootChildNode(docHandle, AF_LPGPU2_STR_loadProjectIsAndroidDeviceSession);

            if (pIsAndroidSession)
            {
                TiXmlText* pIsAndroidSessionText = pIsAndroidSession->ToText();
                GT_IF_WITH_ASSERT(pIsAndroidSessionText)
                {
                    gtString isAndroidSessionValue;
                    readXmlFieldValue(*pIsAndroidSessionText, isAndroidSessionValue);
                    isAndroidSession = (0 == isAndroidSessionValue.compareNoCase(L"yes"));
                    if (isAndroidSession)
                    {
                        // Set the project settings to register that the session is of type android
...
//++CF:LPGPU2 - Read the SYCL Runtime library if available
    		gtString SYCLRuntimeDirStr;
    		TiXmlNode* pSYCLRuntimeDir = getProjectRootChildNode(docHandle, AF_STR_loadProjectSYCLRuntimePathNode);

    		if (pSYCLRuntimeDir)
    		{
    		  TiXmlText* pSYCLRuntimeDirText = pSYCLRuntimeDir->ToText();

    			if (pSYCLRuntimeDirText)
    			{
    				QString SYCLRuntimeDirText = pSYCLRuntimeDirText->Value();

    				if (SYCLRuntimeDirText != OS_STR_EmptyXMLString)
    				{
    					SYCLRuntimeDirStr = acQStringToGTString(SYCLRuntimeDirText);
    				}

    				m_projectSettings.SetSYCLRuntimeLib(SYCLRuntimeDirStr);
    			}
    		}
//--CF:LPGPU2
...
bool afLoadProjectCommand::setExtensionXMLData()
{
    ...
    for (/* iter */; iter != iterEnd; iter++)
    {
        ...
        if (!rc)
        {
            ...
            {
                gtString msg;
                msg.appendFormattedString(L"Warning: Unable to find project settings for %ls plug-in in this project. Project will continue loading.", (*iter).first.asCharArray());
