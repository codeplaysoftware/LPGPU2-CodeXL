    ...
    switch (pageIndex)
    {
        ...
        case SYS_INFO_OPENCL_DEVICES:
        {
            ...
            if (_openCLDevicesInfoFirstTime)
            {
                ...
                //++TLRS: LPGPU2: Calling the right constructor to query OpenCL devices
                m_pOpenCLDeviceInfoCollectorThread = new afDeviceInformationCollector{ EnumDeviceType::kOpenCL };
                //--TLRS: LPGPU2: Calling the right constructor to query OpenCL devices
        ...
        //++TLRS: LPGPU2: Populate the SYCL devices tab
        case SYS_INFO_SYCL_DEVICES:
        {
            if (m_bSyclDevicesInfoFirstTime)
            {
                if (m_pSYCLDeviceInfoCollectorThread != nullptr)
                {
                    delete m_pSYCLDeviceInfoCollectorThread;
                }

                m_pSYCLDeviceInfoCollectorThread = new afDeviceInformationCollector{ EnumDeviceType::kSYCL };

                GT_IF_WITH_ASSERT(m_pListCtrlSYCLDevices != nullptr)
                {
                    GT_IF_WITH_ASSERT(m_pSYCLDeviceInfoCollectorThread->StartCollectingInfo())
                    {
                        m_pListCtrlSYCLDevices->clearList();
                        m_pListCtrlSYCLDevices->setColumnCount(1);
                        m_pListCtrlSYCLDevices->horizontalHeader()->hide();
                        m_pListCtrlSYCLDevices->verticalHeader()->hide();
                        m_pListCtrlSYCLDevices->addRow(AF_STR_CollectingOpenCLDevicesInfo);
                        m_bListCtrlSYCLDevicesOnMsgMode = true;
                        m_syclDeviceInfoCollectorTimerId = startTimer(AF_THREAD_WAIT_INTERVAL);
                    }
                }
            }

            // FIXME: Add proper description
            _pNotebookDescription->setText(AF_STR_DescriptionSYCLDevices);
            _pCurrentShownListCtrl = m_pListCtrlSYCLDevices;
        }
        break;
        //--TLRS: LPGPU2: Populate the SYCL devices tab
...
{
    //++TLRS: LPGPU2: Delegate the event to the right handler
    GT_IF_WITH_ASSERT(event)
    {
        if (event->timerId() == m_openCLDeviceInfoCollectorTimerId)
        {
            HandleOpenCLThreadTimerEvent();
        }
        else if (event->timerId() == m_syclDeviceInfoCollectorTimerId)
        {
            HandleSYCLThreadTimerEvent();
        }
    }
    //--TLRS: LPGPU2: Delegate the event to the right handler
}
...
// ---------------------------------------------------------------------------
//++TLRS: LPGPU2: Handlers for the time events of the device information collection threads.
/// @brief Handle query for OpenCL devices time event. It checks for the collection
///        thread results and writes the member _openCLDevicesInfoData with the collected
///        data.
void afSystemInformationDialog::HandleOpenCLThreadTimerEvent()
{
    ...
    GT_IF_WITH_ASSERT(nullptr != m_pOpenCLDeviceInfoCollectorThread)
    {
        ...
        {
            bool rcGetInfo = m_pOpenCLDeviceInfoCollectorThread->GetDeviceInformation(_openCLDevicesInfoData);
            delete m_pOpenCLDeviceInfoCollectorThread;
            m_pOpenCLDeviceInfoCollectorThread = nullptr;
...
/// @brief Handle query for SYCL devices time event. It checks for the collection
///        thread results and writes the member m_syclDevicesInfoData with the collected
///        data.
void afSystemInformationDialog::HandleSYCLThreadTimerEvent()
{
    auto bStopThread = false;
    m_syclDeviceInfoCollectorThreadRunTime += AF_THREAD_WAIT_INTERVAL;

    // If past the max waiting time, stop timer to prevent it from going off in the mean while
    if (m_syclDeviceInfoCollectorThreadRunTime > AF_THREAD_TOTAL_WAIT_TIME)
    {
        StopSYCLDeviceInformationTimer();
        bStopThread = true;
    }

    GT_IF_WITH_ASSERT(m_pSYCLDeviceInfoCollectorThread != nullptr)
    {
        // if thread is running past the time allowed, stop it
        if (bStopThread)
        {
            m_pSYCLDeviceInfoCollectorThread->StopCollectingInfo();
        }

        // If thread is not active, might have finish before the timer, handle the thread
        if (!m_pSYCLDeviceInfoCollectorThread->IsActive())
        {
            const auto bRetGetInfo = m_pSYCLDeviceInfoCollectorThread->GetDeviceInformation(m_syclDevicesInfoData);

            delete m_pSYCLDeviceInfoCollectorThread;
            m_pSYCLDeviceInfoCollectorThread = nullptr;

            GT_IF_WITH_ASSERT(bRetGetInfo)
            {
                // Collecting data had no errors, if list is empty its because there is nothing to display
                LoadSYCLDeviceInformation(AF_STR_NoSYCLDevices);
            }
            else
            {
                // Error occurred while collecting the data
                LoadSYCLDeviceInformation(AF_STR_FailToCollectSYCLDevicesInfo);
            }
...
// ---------------------------------------------------------------------------
//++TLRS: LPGPU2: Stop the SYCL collection timer
/// @brief Stop the SYCL collection timer
void afSystemInformationDialog::StopSYCLDeviceInformationTimer()
{
    if (m_syclDeviceInfoCollectorTimerId > 0)
    {
        killTimer(m_syclDeviceInfoCollectorTimerId);
        m_syclDeviceInfoCollectorTimerId = 0;
        m_syclDeviceInfoCollectorThreadRunTime = 0;
    }
}
//--TLRS: LPGPU2: Stop the SYCL collection timer
...
// ---------------------------------------------------------------------------
//++TLRS: LPGPU2: Load the SYCL devices information into the GUI
/// @brief Load the SYCL devices information into the GUI
/// @param[in] errMsg The error message to display in case the collection failed.
void afSystemInformationDialog::LoadSYCLDeviceInformation(const QString & errMsg)
{
    GT_IF_WITH_ASSERT(m_pListCtrlSYCLDevices != nullptr)
    {
        if (m_bListCtrlSYCLDevicesOnMsgMode)
        {
            m_pListCtrlSYCLDevices->clearList();
            m_pListCtrlSYCLDevices->horizontalHeader()->show();
            m_pListCtrlSYCLDevices->verticalHeader()->show();
            m_pListCtrlSYCLDevices->setColumnCount(0);
            m_bListCtrlSYCLDevicesOnMsgMode = false;
        }

        if (!m_syclDevicesInfoData.empty())
        {
            fillSysInfoDataIntoListCtrl(m_syclDevicesInfoData, m_pListCtrlSYCLDevices, false, AF_STR_OpenCLDevicesTableMinColumnWidth);
            m_pListCtrlSYCLDevices->verticalHeader()->resizeSections(QHeaderView::ResizeToContents);
            m_bSyclDevicesInfoFirstTime = false;
            _pNotebookDescription->setText(AF_STR_DescriptionSYCLDevices);
        }
        else
        {
            displayErrorMessage(errMsg, m_pListCtrlSYCLDevices);
        }
    }
}
//--TLRS: LPGPU2: Load the SYCL devices information into the GUI
...
void afSystemInformationDialog::setDialogLayout()
{
    ...
    //++TLRS: LPGPU2: Add the new tab to the dialog
    m_pListCtrlSYCLDevices = new acListCtrl{ this };
    _pNotebookInfo->addTab(m_pListCtrlSYCLDevices, AF_STR_InformationSYCLDevices);
    //--TLRS: LPGPU2: Add the new tab to the dialog
...
void afSystemInformationDialog::onSysInfoSaveButton(/*QAbstractButton* pButton*/)
{
    ...
    GT_IF_WITH_ASSERT(pApplicationCommands != nullptr)
    {
        ...
        if (!selectedFilePath.isEmpty())
        {
            ...
            {
                // Create a thread to collect data
                //++TLRS: LPGPU2: Calling the right constructor
                m_pOpenCLDeviceInfoCollectorThread = new afDeviceInformationCollector{ EnumDeviceType::kOpenCL };
                //--TLRS: LPGPU2: Calling the right constructor
                GT_IF_WITH_ASSERT(m_pOpenCLDeviceInfoCollectorThread != nullptr && m_pOpenCLDeviceInfoCollectorThread->StartCollectingInfo())
                {
                    ...
                    //++TLRS: LPGPU2: Calling the generalized method
                    m_pOpenCLDeviceInfoCollectorThread->GetDeviceInformation(_openCLDevicesInfoData);
                    //--TLRS: LPGPU2: Calling the generalized method
            ...
            //++TLRS: LPGPU2: Exporting the SYCL information to a file
            _sysInfoSaveOutputString.append(L"\n\n\n\n");
            _sysInfoSaveOutputString.append(L"////////////////////////////////////////////////////////////\n");
            _sysInfoSaveOutputString.append(L"// SYCL Devices \n");
            _sysInfoSaveOutputString.append(L"////////////////////////////////////////////////////////////\n");

            if (m_bSyclDevicesInfoFirstTimeToFile)
            {
                // Create a thread to collect the data
                m_pSYCLDeviceInfoCollectorThread = new afDeviceInformationCollector{ EnumDeviceType::kSYCL };

                GT_IF_WITH_ASSERT(m_pSYCLDeviceInfoCollectorThread != nullptr && m_pSYCLDeviceInfoCollectorThread->StartCollectingInfo())
                {
                    osWaitForFlagToTurnOff(m_pSYCLDeviceInfoCollectorThread->IsActive(), AF_THREAD_TOTAL_WAIT_TIME);

                    m_pSYCLDeviceInfoCollectorThread->GetDeviceInformation(m_syclDevicesInfoData);

                    delete m_pSYCLDeviceInfoCollectorThread;
                    m_pSYCLDeviceInfoCollectorThread = nullptr;
                }
                m_bSyclDevicesInfoFirstTimeToFile = false;
            }
            //--TLRS: LPGPU2: Exporting the SYCL information to a file

            addSystemInfoIntoOutputString(m_syclDevicesInfoData, 33, true);
