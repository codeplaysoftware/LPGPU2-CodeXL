...
//++IOR: LPGPU2 Notify extensions the project is closed
    bool closeCurrentProject(int extensionIndex);
//--IOR: LPGPU2 Notify extensions the project is closed
    ...
    bool AreSettingsValidSecondPass(gtString &vInvalidMessageStr, gtString &vInvalidExtnTreePath);
...
//++AT:LPGPU2
    void SetAndroidRemoteDebugging();
    void SetRemoteTargetHostname(const gtString& remoteHostName);
    void setRemoteTargetDaemonPort(gtUInt16 daemonConnectionPort);
    const gtString &GetRemoteTargetHostName() const { return m_currentProject.remoteTargetName(); }
    gtUInt16 GetRemoteTargetDaemonPort() const { return m_currentProject.remoteTargetDaemonConnectionPort(); }
//--AT:LPGPU2
