...
//++TLRS: LPGPU2: Changing to enum class in order to avoid implicit cast to bool in some places
enum class afApplicationType
{
    af32BitApp,
    af64BitApp
};
...
//++TLRS: LPGPU2: Enum to define the type of device to query for information
...
public:
    ...
    bool CollectDevicesInformation(gtList< gtList <gtString> >& infoData, afApplicationType appType, EnumDeviceType deviceType);
    ...
    //++TLRS: LPGPU2: Command to query SYCL devices in the local system
    bool CollectAllSYCLDevicesInformation(gtList<gtList<gtString>> &infoData);
    //--TLRS: LPGPU2: Command to query SYCL devices in the local system
