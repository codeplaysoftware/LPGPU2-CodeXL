...
class afDeviceInformationCollector;
...
public:
    ...
    enum InformationTabs
    {
        ...
        //++TLRS: LPGPU2: Enum to add the SYCL Devices tab
        SYS_INFO_SYCL_DEVICES
        //--TLRS: LPGPU2: Enum to add the SYCL Devices tab
    };
...
private:
    ...
    void LoadSYCLDeviceInformation(const QString &vErrMsg);
    //--TLRS: LPGPU2: Load the SYCL devices information into the GUI
    ...
    //++TLRS: LPGPU2: Stop the SYCL info collection thread
    void StopSYCLDeviceInformationTimer();
    //--TLRS: LPGPU2: Stop the SYCL info collection thread

    //++TLRS: LPGPU2: Handlers for time events
    void HandleOpenCLThreadTimerEvent();
    void HandleSYCLThreadTimerEvent();
    //--TLRS: LPGPU2: Handlers for time events
...
private:
    ...
    //++TLRS: LPGPU2: Adding members for holding the SYCL Devices tab
    acListCtrl *m_pListCtrlSYCLDevices;
    bool m_bSyclDevicesInfoFirstTime = true;
    bool m_bSyclDevicesInfoFirstTimeToFile = true;
    gtList<gtList<gtString>> m_syclDevicesInfoData;
    gtList<gtList<gtString>> m_syclDevicesInfoDataFile;
    //--TLRS: LPGPU2: Adding members for holding the SYCL Devices tab
    ...
    afDeviceInformationCollector* m_pOpenCLDeviceInfoCollectorThread;
    
    //++TLRS: LPGPU2: Holds information about the SYCL collector thread
    int m_syclDeviceInfoCollectorTimerId = -1;
    int m_syclDeviceInfoCollectorThreadRunTime = 0;
    bool m_bListCtrlSYCLDevicesOnMsgMode = false;
    afDeviceInformationCollector* m_pSYCLDeviceInfoCollectorThread = nullptr;
    //--TLRS: LPGPU2: Holds information about the SYCL collector thread    
};
