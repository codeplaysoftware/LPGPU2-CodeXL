    ...
    cl_event*   event)
{
    ...
    // register callback for event completion to get queue/device times:
    GetRealDispatchTable()->SetEventCallback(m_event, CL_COMPLETE, EventProfilingCallback, NULL);
    ...
    cl_event*   event)
{
    ...
    // register callback for event completion to get queue/device times:
    cl_int ret = GetRealDispatchTable()->SetEventCallback(m_event, CL_COMPLETE, EventProfilingCallback, NULL);
    SP_UNREFERENCED_PARAMETER(ret);
...
bool CLAPI_clEnqueueNDRangeKernel::WriteTimestampEntry(std::ostream& sout, bool bTimeout)
{
    ...
    sout << TAB_OUT(25) << StringUtils::ToHexString(kernel);
    sout << GetKernelName();
...
bool CLAPI_clEnqueueNativeKernel::WriteTimestampEntry(std::ostream& sout, bool bTimeout)
{
    ...
    sout << TAB_OUT(25) << StringUtils::ToHexString(m_user_func);
    ...
    cl_event*   event)
{
    ...
    m_retVal = next(command_queue, pTmpEvent);
...
cl_int CLAPI_clEnqueueSVMFree::Create(KHRpfn_clEnqueueSVMFree next,
                                      cl_command_queue command_queue,
...
cl_int CLAPI_clEnqueueSVMMemcpy::Create(KHRpfn_clEnqueueSVMMemcpy next,
                                        cl_command_queue command_queue,
...
cl_int CLAPI_clEnqueueSVMMemFill::Create(KHRpfn_clEnqueueSVMMemFill next,
                                         cl_command_queue command_queue,
                                         void*            svm_ptr,
                                         const void*      pattern,
                                         size_t           pattern_size,
...
cl_int CLAPI_clEnqueueSVMMap::Create(KHRpfn_clEnqueueSVMMap next,
                                     cl_command_queue command_queue,
                                     cl_bool          blocking_map,
                                     cl_map_flags     flags,
                                     void*            svm_ptr,
...
cl_int CLAPI_clEnqueueSVMUnmap::Create(KHRpfn_clEnqueueSVMUnmap next,
                                       cl_command_queue command_queue,
                                       void*            svm_ptr,
                                       cl_uint          num_events_in_wait_list,
                                       const cl_event*  event_wait_list,
...
#define WARN_ONCE(msg) static bool bWarned = false;\
                       if (!bWarned)\
                         std::cerr << msg << std::endl;\
                       bWarned = true

extern "C" {
  void EVENT_CALL_CONVENTION EventProfilingCallback(cl_event event, cl_int event_command_exec_status, void* user_data)
  {
    SP_UNREFERENCED_PARAMETER(user_data);
    SP_UNREFERENCED_PARAMETER(event_command_exec_status);

    cl_ulong tStart, tEnd, tQueue, tSubmit;
    tStart = tEnd = tQueue = tSubmit = 0;
    cl_int ret;
    ret = GetRealDispatchTable()->GetEventProfilingInfo(event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &tStart, NULL);
    ret = GetRealDispatchTable()->GetEventProfilingInfo(event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &tEnd, NULL);
    ret = GetRealDispatchTable()->GetEventProfilingInfo(event, CL_PROFILING_COMMAND_QUEUED, sizeof(cl_ulong), &tQueue, NULL);
    ret = GetRealDispatchTable()->GetEventProfilingInfo(event, CL_PROFILING_COMMAND_SUBMIT, sizeof(cl_ulong), &tSubmit, NULL);

    auto clEvent = CLEventManager::Instance()->GetCLEvent(event);

    // The device timer may not be tied to the host system time, but rather count from some arbitrary 
    // epoch (like the creation of the queue). In this case we can't know the start of the queue or
    // execution relative to the API times, but we assume this callback is fired straight after the 
    // the event completes, which isn't guaranteed but almost definitely the case. 
    // So far this issue has only occurred with ComputeAorta.
    if (tEnd < clEvent->m_pOwner->m_ullStart)
    {
        // clEvent->m_ullQueued = clEvent->m_pOwner->m_ullStart;// clEvent->m_ullRunning - (tSubmit - tQueue);
        // clEvent->m_ullSubmitted = clEvent->m_pOwner->m_ullEnd;// clEvent->m_ullRunning;
        clEvent->m_ullComplete = OSUtils::Instance()->GetTimeNanos(); //clEvent->m_ullRunning + (tEnd - tStart);
        clEvent->m_ullRunning = clEvent->m_ullComplete - (tEnd - tStart);  // clEvent->m_pOwner->m_ullEnd;
        WARN_ONCE("WARNING: CL Trace Agent: Device timer and host system time aren't aligned. Enqueue event queue and execution"
                  " durations will be correct, but their end times will be estimated.");
    }
    else
    {
        clEvent->m_ullRunning = tStart;
        clEvent->m_ullComplete = tEnd;
        clEvent->m_ullQueued = tQueue;
        clEvent->m_ullSubmitted = tSubmit;
        clEvent->m_ullCPUQueued = tQueue;
    }
    ...
    SpAssert(ret == CL_SUCCESS);
    (void) ret; // The SpAssert above doesn't count as using the variable on release builds...
  }
}