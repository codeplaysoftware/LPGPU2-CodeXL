...
bool WriteTimestampEntry(std::ostream& sout, CLAPIInfo* pInfo)
{
    ...
    // APIType
    sout << left << TAB_OUT(5) << pInfo->m_uiAPIID;

    // APIName
    sout << left << TAB_OUT(45) << pInfo->m_strName;

    // start time
    sout << left << TAB_OUT(21) << pInfo->m_ullStart;
    // end time
    sout << left << TAB_OUT(21) << pInfo->m_ullEnd;
    ...
    if (IsEnqueueAPI(pInfo->m_uiAPIID))
    {
        ...
        {
            sout << left << TAB_OUT(8) << pEnAPI->m_uiCMDType;
            sout << left << TAB_OUT(40) << pEnAPI->m_strCMDType;

            sout << left << TAB_OUT(21) << pEnAPI->m_ullQueue;
            sout << left << TAB_OUT(21) << pEnAPI->m_ullSubmit;
            sout << left << TAB_OUT(21) << pEnAPI->m_ullRunning;
            sout << left << TAB_OUT(21) << pEnAPI->m_ullComplete;

            // command queue handle
            sout << std::dec << TAB_OUT(10) << pEnAPI->m_uiQueueID;
            sout << TAB_OUT(25) << pEnAPI->m_strCmdQHandle;
            // context handle
            sout << std::dec << TAB_OUT(10) << pEnAPI->m_uiContextID;
            sout << TAB_OUT(25) << pEnAPI->m_strCntxHandle;

            // device name
            sout << TAB_OUT(30) << pEnAPI->m_strDevice;
            ...
            if (pEnAPI->m_uiCMDType <= CL_COMMAND_TASK)
            {
                ...
                sout << TAB_OUT(25) << pKAPI->m_strKernelHandle;
                     ...
                     (pEnAPI->m_uiCMDType >= CL_COMMAND_READ_BUFFER_RECT && pEnAPI->m_uiCMDType <= CL_COMMAND_COPY_BUFFER_RECT))
            {
                ...
                sout << std::dec << TAB_OUT(20) << pMAPI->m_uiTransferSize;
            }
                ...
                pEnAPI->m_uiCMDType == CL_COMMAND_SVM_MAP || pEnAPI->m_uiCMDType == CL_COMMAND_SVM_UNMAP)
            {
                ...
                sout << std::dec << TAB_OUT(20) << pMAPI->m_uiDataSize;
            }
        }
...
bool CLAtpFilePart::WriteContentSection(SP_fileStream& sout, const std::string& strTmpFilePath, const string& strPID)
{
    ...
    if (m_config.bTimeOut || m_config.bMergeMode)
    {
        ...
        //CF:LPGPU2 TODO - Figure out why this crashes: (And also doesn't seem to do anything useful)
