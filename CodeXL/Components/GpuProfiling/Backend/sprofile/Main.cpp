...
#include "../SYCLProfiler/SYCLAtpFile.h"
#include "../SYCLProfiler/SYCLTraceAgent.h"
...
static void DeleteExistingPIDLogFile();

#if defined (_LINUX) || defined (LINUX)
    static bool SetPreLoadLibs();
    static bool UnsetPreLoadLibs();
#endif
...
static bool SetSYCLRuntimeEnvVar(const std::string&);
...
bool ParseCmdLine(int argc, char* argv[], Config& configOut)
{
    ...
    return ParseCmdLine(argc, convertedArg.data(), configOut);
}
}
...
static int CreateProcessWithDetour(gtString& strDirPath, gtString& strAppCommandLine, gtString& strAppWorkingDirectory, bool useDetours, bool syclTrace)
{
    ...
    if (useDetours)
    {
        // Make space for both the MicroDll and the potential sycl tracer
        LPCSTR dllPaths[2];
...
#endif
{
    ...
    if (!config.bAnalyzeOnly)
    {
        ...
        if (config.bMergeMode)
        {
            ...
            // ++CF:LPGPU2 This will disable OpenCL tracing on ARM builds! Currently we have no
            //              OpenCL tracing support on these architectures. On top of this, the
            //              libraries to detect support do not build.
#ifdef CODEXL_ARCH_IS_ARMV7A
#pragma message("CF:LPGPU2 OpenCL Trace is DISABLED")
            config.bTrace = true;
#else
#pragma message("CF:LPGPU2 OpenCL Trace is ENABLED")
            config.bTrace = true;
#endif // CODEXL_ARCH_IS_ARMV7A
            // --CF:LPGPU2
        ...
        //++CF:LPGPU2 We need to clear any existing PID log before starting, rather than relying on
        //            the previous session having deleted it properly.
        DeleteExistingPIDLogFile();
        //--CF:LPGPU2
...
static void MergeTraceFile(int sig)
{
    ...
    if ((config.bTrace || config.bHSATrace || config.bSYCLTrace || config.bMergeMode) && processId > 0)
    {
        //++CF:LPGPU2 Get the PID of the last created ocl frag file
        std::vector<std::string> loggedPIDs;
        std::ifstream pidLogFile;
        pidLogFile.open(FileUtils::GetDefaultOutputPath() + "/cxlpidlog");
        std::string pidLine;
        while (pidLogFile.good())
        {
            pidLogFile >> pidLine;
            loggedPIDs.push_back(pidLine);
        }

        // Current behaviour is to pick the last logged PID, as we assume the last created subproccess
        // is the one we want to profile. In future it could be possible to try to merge all process
        // files.
        // TODO: fix this if OCL trace agent fails and we have SYCL only:
        std::string pid = loggedPIDs.back();
        //--CF:LPGPU2
        ...
        if (config.bSYCLTrace)
        {
          pSyclTrace = new(std::nothrow) SYCLAtpFilePart(config);

          if (nullptr != pSyclTrace)
          {
            writer.AddAtpFilePart(pSyclTrace);
          }
        }
...
static bool SetHSAServer(const gtString& strDirPathUnicode)
{
    ...
    if (!FileUtils::FileExist(strServerPath))
    {
        ...
        std::cout << "1Make sure you have " << strServerPath << " under " << strDirPath.c_str() << std::endl;
...
static bool SetAgent(const gtString& strDirPathUnicode)
{
    ...
    {
//++CF:LPGPU2
        SP_strcpy(szAgentDllPath, SP_MAX_PATH, CL_ICD_LOADER_DLL);
...
//--CF:LPGPU2
    ...
    for (unsigned int i = 0; i < vAgentDllPath.size(); ++i)
    {
        if (!FileUtils::FileExist(vAgentDllPath[i]))
        {
            ...
            std::cout << "2Make sure you have " << strAgent << " under " << strDirPath.c_str() << std::endl;
...
static std::string original_preload_libs = OSUtils::Instance()->GetEnvVar(LD_PRELOAD_ENV_VAR_NAME);
...
static bool SetPreLoadLibs()
{
    ...
    {
        std::string curPreLoadPath = original_preload_libs;
...
static bool UnsetPreLoadLibs()
{
  return OSUtils::Instance()->SetEnvVar(LD_PRELOAD_ENV_VAR_NAME, original_preload_libs.c_str());
}
...
{
    SP_UNREFERENCED_PARAMETER(strAppName);
    SP_UNREFERENCED_PARAMETER(iProfilerNbrBits);
    bool bStatus = true;
...
/// @brief Remove the temporary PID log file if it still exists.
static void DeleteExistingPIDLogFile()
{
    gtString pidLogPathString;
    pidLogPathString = pidLogPathString.fromASCIIString(FileUtils::GetDefaultOutputPath().c_str());
    pidLogPathString.append(L"/cxlpidlog");
    osFilePath pidLogPath{pidLogPathString};
    if (pidLogPath.exists())
    {
        osFile pidLogFile{pidLogPath};
        pidLogFile.deleteFile();
    }
}
