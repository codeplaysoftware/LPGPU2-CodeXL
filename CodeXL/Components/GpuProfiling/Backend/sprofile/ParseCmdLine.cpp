...
bool ParseCmdLine(int argc, wchar_t* argv[], Config& configOut)
{
    ...
    try
    {
...
#endif
        ("sycltrace", "Trace SYCL application and generate CPU time stamps and detailed API call traces. Requires opencl")
		("lsycl", po::value<string>(), "Path to the SYCL Library used by the SYCL application")
        ...
        {
			boost::filesystem::wpath filterFile(unicodeOptionsMap["apifilterfile"]);
        ...
        configOut.bSYCLTrace = unicodeOptionsMap.count("sycltrace") > 0;
	...
	//++TLRS: LPGPU2: Sycl Runtime command line parse
	if (unicodeOptionsMap.count("lsycl") > 0)
	{
	    wstring valueStr = unicodeOptionsMap["lsycl"];
	    string valueStringConverted;
	    StringUtils::WideStringToUtf8String(valueStr, valueStringConverted);
	    outputFile = valueStringConverted;
	    configOut.strSYCLRuntimeLib = outputFile.string();
	}
	else
	{
            configOut.strSYCLRuntimeLib.clear();
	}
        //--TLRS: LPGPU2: Sycl Runtime command line parse
