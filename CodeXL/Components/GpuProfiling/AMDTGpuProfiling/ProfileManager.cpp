        ...
        if (GetProfilerServer(serverFile, strErrorMessageOut))
        {
            ...
            if (m_profileParameters.ProfileTypeValue() == API_TRACE)
            {
                GT_IF_WITH_ASSERT(m_pCurrentProjectSettings != nullptr)
                {
                    ...
                    //++CF:LPGPU2
                    if ((m_pCurrentProjectSettings->m_traceOptions.m_apiToTrace & APIToTrace_SYCL) == APIToTrace_SYCL)
                    {
                        // We still rely on the opencl trace
                        strProfileTypeArguments += " --apitrace --sycltrace ";

                        // We also want to pass the SYCL runtime library used by the SYCL application
                        strProfileTypeArguments.append(QString(" --lsycl \"%1\" ").arg(acGTStringToQString(m_pCurrentProjectSettings->m_traceOptions.m_syclRuntimeLibName)));
                    }
                    //--CF:LPGPU2
                    ...
                    {
                        if ((m_pCurrentProjectSettings->m_traceOptions.m_apiToTrace & APIToTrace_OPENCL) == APIToTrace_OPENCL
							               || (m_pCurrentProjectSettings->m_traceOptions.m_apiToTrace & APIToTrace_SYCL) == APIToTrace_SYCL)
                        {
...
{
    QString strProjectType = (m_pCurrentProjectSettings->m_traceOptions.m_apiToTrace == APIToTrace_OPENCL) ? "an OpenCL or a DirectCompute program" : 
                              ((m_pCurrentProjectSettings->m_traceOptions.m_apiToTrace == APIToTrace_SYCL) ? "a SYCL program" : "an HSA program");
    QString strExtraReason;
    strExtraReason.clear();

    if (m_profileParameters.ProfileTypeValue() == API_TRACE)
    {
        strProjectType = (m_pCurrentProjectSettings->m_traceOptions.m_apiToTrace == APIToTrace_OPENCL) ? "an OpenCL program" : ((m_pCurrentProjectSettings->m_traceOptions.m_apiToTrace == APIToTrace_SYCL) ? "a SYCL program" : "an HSA program");
