...
bool gpViewsCreator::onMDISubWindowClose(afQMdiSubWindow* pMDISubWindow)
{
    ...
    GT_IF_WITH_ASSERT(pMDISubWindow != nullptr)
    {
        ...
        const gtString &rPath(filePath.asString());
        const gtString &rAnalPath(gpUIManager::Instance()->CurrentExecutingFrameAnalysisPath().asString());
        if (!rPath.isEmpty() && !rAnalPath.isEmpty())
        {
