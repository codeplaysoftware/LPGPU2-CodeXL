...
static const QColor s_SYCL_DEFAULT_COLOR_GRAY = QColor(90, 90, 90);
...
APIColorMap::APIColorMap()
{
    ...
    // m_commandListsColors << Qt::yellow;
}
...
QColor APIColorMap::GetAPIColor(const QString& apiName, const QColor& defaultColor)
{
    ...
    if (apiName == "clEnqueueNDRangeKernel" || apiName == "clEnqueueTask" || apiName == "clEnqueueNativeKernel" || apiName == "SYCL_KERNEL_EXECUTE" || apiName == "SYCL_COMMAND_GROUP")
    {
        retVal = Qt::darkGreen;
    }
    else if (apiName == "clEnqueueReadBuffer" || apiName == "clEnqueueReadBufferRect" || apiName == "clEnqueueReadImage" || apiName == "SYCL_ACCESSOR_CREATE" || apiName == "SYCL_BUFFER_CREATE" || apiName == "SYCL_BUFFER_DESTROY")
    {
        retVal = Qt::darkBlue;
    }
    else if (apiName == "clEnqueueWriteBuffer" || apiName == "clEnqueueWriteBufferRect" || apiName == "clEnqueueWriteImage" || apiName == "syclCreateAccessor")
    {
        retVal = Qt::blue;
    }
...
QColor APIColorMap::GetSYCLDefaultColor() const
{
    return s_SYCL_DEFAULT_COLOR_GRAY;
}
...
//++CF:LPGPU2
QColor APIColorMap::GetSYCLTransactionColor(unsigned int tid) const
{
  QColor retVal = s_LIGHT_BLUE_PERF_MARKER_COLOR;
  const int indexInVec = tid % (m_commandListsColors.size() - 1);
  GT_IF_WITH_ASSERT(indexInVec < m_commandListsColors.size())
  {
      retVal = m_commandListsColors[indexInVec];
  }
  return retVal;
