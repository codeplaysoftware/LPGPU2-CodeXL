...
//++TLRS: LPGPU2: Ability to query for SYCL availability
/// @brief Utility to check if SYCL is available in the local or remote target.
/// @return bool true = SYCL is available,
///              false = SYCL is not available.
bool Util::IsSYCLEnabled()
{
    const auto& projectSettings = afProjectManager::instance().currentProjectSettings();
    if (projectSettings.isRemoteTarget())
    {
        ...
        auto *pDmnClient = CXLDaemonClient::GetInstance();
        GT_IF_WITH_ASSERT(pDmnClient != NULL)
        {
            if (retVal)
            {
                ...
                GT_IF_WITH_ASSERT(retVal)
                {
                    return pDmnClient->IsSYCLEnabled();
                }
            }
        }
    }
    ...
    {
        // check for local SYCL support
        afSystemInformationCommand sysInfoCmd;
        gtList<gtList<gtString>> syclDevicesInfo;
        if (sysInfoCmd.CollectAllSYCLDevicesInformation(syclDevicesInfo))
        {
            return !syclDevicesInfo.empty();
        }        
    }
