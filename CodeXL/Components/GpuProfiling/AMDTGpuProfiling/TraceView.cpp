...
#include <AMDTGpuProfiling/SYCLTimelineItems.h>
...
const unsigned int s_syclBranchHeightPx = 25;
...
//++CF:LPGPU2
void TraceView::SetAPIDepth(osThreadId threadId, std::map<int, unsigned int> mapCategoryDepth)
{
  m_syclThreadDepthMap[threadId] = mapCategoryDepth;
  
  // This exists because std::adjacent_difference wouldn't work with maps:
  unsigned int cumulativeDepth = 0;  
  for (const auto& pairCategoryDepth : mapCategoryDepth)
  {
    m_syclThreadCumulativeDepthMap[threadId][pairCategoryDepth.first] = cumulativeDepth;
    cumulativeDepth += pairCategoryDepth.second;
  }
  // TODO: special case NONE
}

void TraceView::SetAPIQueueDepth(unsigned int queueId, std::map<int, unsigned int> mapCategoryDepth)
{
  m_syclQueueDepthMap[queueId] = mapCategoryDepth;
}
//--CF:LPGPU2
...
//++CF:LPGPU2
void TraceView::OnParse(SYCLAPIInfo* pAPIInfo, bool& stopParsing)
{
    // Save the API type for later use:
    m_api = APIToTrace_SYCL;

    m_parseCallsCounter++;
    HandleSYCLAPIInfo(pAPIInfo);
    stopParsing = CheckStopParsing();
}
//--CF:LPGPU2
...
acTimelineBranch* TraceView::GetHostBranchForAPI(osThreadId threadId, const QString& branchText)
{
    ...
    retVal = rootBranch;
    // The root host branch has a larger default size to accomodate stacked items
    unsigned int cumulativeDepth = 0;
    for (auto pairCategoryDepth : m_syclThreadDepthMap[threadId])
    {
      cumulativeDepth += pairCategoryDepth.second;
    }
    const auto maxRootBranchHeight = s_syclBranchHeightPx * cumulativeDepth;
    ...
    if (!branchText.isEmpty())
    {
        ...
        if (retVal == nullptr)
        {
            ...
            // Enable stacked root branch for SYCL host threads only:
            if (branchText == GPU_STR_timeline_SYCLBranchName)
            {
              retVal->setHeight(maxRootBranchHeight);
              retVal->setMaskEnabled(false);
            }
...
//++CF:LPGPU2
    // add part parser for SYCL API Trace and Timestamp sections
    SYCLAtpFilePart syclAtpPart(config);
    syclAtpPart.AddProgressMonitor(this);
    parser.AddAtpFilePart(&syclAtpPart);
//--CF:LPGPU2
...
//++CF:LPGPU2
    // TODO: is there a SYCL stack trace?
    // add part parser for sycl Stack Trace section
    StackTraceAtpFilePart syclStackPart("sycl", config);
    syclStackPart.AddProgressMonitor(this);
    parser.AddAtpFilePart(&syclStackPart);
//--CF:LPGPU2
...
//++CF:LPGPU2
    syclAtpPart.AddListener(this);
//--CF:LPGPU2
    hsaAtpPart.AddListener(this);
    clStackPart.AddListener(this);
//++CF:LPGPU2
    syclStackPart.AddListener(this);
//--CF:LPGPU2
...
void TraceView::HandleCLAPIInfo(CLAPIInfo* pApiInfo)
{
    ...
    std::unique_ptr<const QList<OccupancyInfo*>> pOccupancyInfosList = nullptr;
    int nOccIndex = 0;
    ...
    if (m_pCurrentSession->LoadAndGetOccupancyTable().contains(threadId))
    {
        ...
        pOccupancyInfosList.reset(new QList<OccupancyInfo*>
          (m_pCurrentSession->GetOccupancyTable()[threadId]));
    ...
    if ((pApiInfo->m_Type & CL_ENQUEUE_BASE_API) == CL_ENQUEUE_BASE_API) // this is an enqueue api
    {
        ...
        GT_IF_WITH_ASSERT(enqueueApiInfo != nullptr)
        {
            ...
            quint64 gpuStart;
            quint64 gpuEnd;
            if (enqueueApiInfo->m_ullRunning != 0 && enqueueApiInfo->m_ullComplete != 0)
            {
                gpuStart = enqueueApiInfo->m_ullRunning;
                gpuEnd = enqueueApiInfo->m_ullComplete;
            }
            else
            {
                gpuStart = enqueueApiInfo->m_ullStart;
                gpuEnd = enqueueApiInfo->m_ullEnd;
            }
            // --CF:LPGPU2
            ...
            else
            {
                ...
                else if ((pApiInfo->m_Type & CL_ENQUEUE_OTHER_OPERATIONS) == CL_ENQUEUE_OTHER_OPERATIONS)
                {
                    ...
                    GT_IF_WITH_ASSERT((pOtherEnqueueOperationsInfo != nullptr) && (pBranchInfo != nullptr) && (pBranchInfo->m_pQueueBranch != nullptr))
                    {
                        ...
                        if (enqueueApiInfo->m_ullRunning != 0 && enqueueApiInfo->m_ullComplete != 0)
                        {
                            startTime = pOtherEnqueueOperationsInfo->m_ullRunning;
                            endTime = pOtherEnqueueOperationsInfo->m_ullComplete;
                        }
                        else
                        {
                            startTime = pOtherEnqueueOperationsInfo->m_ullStart;
                            endTime = pOtherEnqueueOperationsInfo->m_ullEnd;
                        }
                        // --CF:LPGPU2
...
//++CF:LPGPU2
/// @brief Handle SYCL API info by creating relevant timeline item, table entry, etc.
/// @param[in]  pApiInfo  API info to process.
void TraceView::HandleSYCLAPIInfo(SYCLAPIInfo *pApiInfo) {
  ...
  else
  {
    ...
    // Show all columns for SYCL API:
    tableModel->SetVisualProperties(palette().color(QPalette::Text), palette().color(QPalette::Link), font());
    m_modelMap.insert(threadId, tableModel);
  }

  acTimelineBranch* hostBranch = GetHostBranchForAPI(threadId, GPU_STR_timeline_SYCLBranchName);
  GT_ASSERT(hostBranch != nullptr);

  // Display info in the parameter list as well as the tooltip:
  // (Because profiled SYCL events aren't real API calls this is otherwise unused)
  for (auto&& timelineEntry : pApiInfo->m_syclProfileEvent->TimelineInfo())
  {
    pApiInfo->m_ArgList.append(timelineEntry.first + " = " + timelineEntry.second + "   ");
  }
  ...
  QColor apiColor;
  auto eventName = pApiInfo->m_syclProfileEvent->EventName();
  // Special case for transactions as they should be colored according to their ID:
  if (eventName.find("SYCL_TRANS_") != std::string::npos)
  {
    const auto* pTransactionEvent = dynamic_cast<SYCLProfiler::TransactionEvent*>(pApiInfo->m_syclProfileEvent.get());
    if (pTransactionEvent != nullptr)
    {
      apiColor = APIColorMap::Instance()->GetSYCLTransactionColor(pTransactionEvent->TransactionID());
    }
  }
  else
  {
    apiColor = APIColorMap::Instance()->GetAPIColor(apiName, APIColorMap::Instance()->GetSYCLDefaultColor());
  }

  APITimelineItem* pAPITimelineItem = new SYCLAPITimelineItem(itemStartTime, itemEndTime, pApiInfo->m_uiDisplaySeqID, pApiInfo->m_syclProfileEvent->TimelineInfo());
  pAPITimelineItem->setText(apiName);
  pAPITimelineItem->setBackgroundColor(apiColor);
  pAPITimelineItem->setForegroundColor(Qt::white);
  
  // Fractional heights used to stack items in the Host thread:
  // TODO maybe move this somewhere rather than calculating it each time
  const unsigned int maxNestLevel = std::accumulate(m_syclThreadDepthMap[threadId].cbegin(),
                                                    m_syclThreadDepthMap[threadId].cend(),
                                                    0,
                                                    [](int acc, const MapCatIDToDepth_t::value_type& catDepth) {
                                                      return acc + catDepth.second;
                                                    });
  
  int eventCategoryId = static_cast<int>(pApiInfo->m_syclProfileEvent->Category());
  const double hostItemOffset = (m_syclThreadCumulativeDepthMap[threadId][eventCategoryId] + pApiInfo->m_nestLevel) / static_cast<double>(maxNestLevel);
  
  // If this item belongs to a queue:
  if (pApiInfo->m_syclProfileEvent->HasQueue())
  {
    APITimelineItem* pAPITimelineItem = new SYCLAPITimelineItem(itemStartTime, itemEndTime, pApiInfo->m_uiDisplaySeqID, pApiInfo->m_syclProfileEvent->TimelineInfo());
    ...
    auto queueID = pApiInfo->m_syclProfileEvent->QueueID();
    auto categoryID = static_cast<int>(pApiInfo->m_syclProfileEvent->Category());
    pAPITimelineItem->setFractionalHeight(1.0 / m_syclQueueDepthMap[queueID][categoryID]);
    pAPITimelineItem->setFractionalOffset((pApiInfo->m_queueNestLevel-1.0) / m_syclQueueDepthMap[queueID][categoryID]);

    uint64_t queueId = pApiInfo->m_syclProfileEvent->QueueID();

    // get branch or create new branch, and the item if possible:
    SYCLQueueBranchInfo* pBranchInfo = GetSYCLBranchInfo(queueId); 

    if (pApiInfo->m_syclProfileEvent->Category() == SYCLProfiler::EventCategory::COMMAND_GROUP_DETAIL)
    {
      pBranchInfo->m_pCommandGroupDetailBranch->addTimelineItem(pAPITimelineItem);
    }
    else if (pApiInfo->m_syclProfileEvent->Category() == SYCLProfiler::EventCategory::COMMAND_GROUP)
    {
      pBranchInfo->m_pCommandGroupBranch->addTimelineItem(pAPITimelineItem);
    }
    else if (pApiInfo->m_syclProfileEvent->Category() == SYCLProfiler::EventCategory::DATA_TRANSFER)
    {
      pBranchInfo->m_pMemoryBranch->addTimelineItem(pAPITimelineItem);
    }
    else if (pApiInfo->m_syclProfileEvent->Category() == SYCLProfiler::EventCategory::KERNEL_EXECUTION)
    {
      pBranchInfo->m_pKernelBranch->addTimelineItem(pAPITimelineItem);
    }
  }

  if (pAPITimelineItem != nullptr)
  {
      pAPITimelineItem->setFractionalHeight(1.0 / maxNestLevel);
      pAPITimelineItem->setFractionalOffset(hostItemOffset);
      hostBranch->addTimelineItem(pAPITimelineItem);
      pAPITimelineItem->setTraceTableItem(tableModel->AddTopLevelTraceItem(GPU_STR_timeline_SYCLBranchName, apiName, pApiInfo, pAPITimelineItem, NULL, NULL));
  }
}
//--CF:LPGPU2
...
//++CF:LPGPU2
    //set up SYCL Queues
    if (!m_syclQueueToQBIMap.isEmpty())
    {

        timelineDataLoaded = true;

        //for (QMap<uint64_t, OCLQueueBranchInfo*>::iterator i = m_syclQueueToQBIMap.begin(); i != m_syclQueueToQBIMap.end(); ++i)
        //{
            //m_pSYCLBranch->addSubBranch((*i)->m_pQueueBranch);
        //}

        m_pTimeline->addBranch(m_pSYCLBranch);

        for (QMap<uint64_t, SYCLQueueBranchInfo*>::iterator i = m_syclQueueToQBIMap.begin(); i != m_syclQueueToQBIMap.end(); ++i)
        {
            SAFE_DELETE(*i);
        }
...
TraceView::OCLQueueBranchInfo* TraceView::GetBranchInfo(unsigned int contextId, unsigned int queueId, const QString& strContextHandle, const QString& deviceNameStr, const QString& strQueueHandle)
{
    ...
    else
    {
        ...
        pRetVal->m_pKernelBranch->setText(tr(GPU_STR_timeline_KernelBranchName));
        pRetVal->m_pKernelBranch->SetBGColor(QColor::fromRgb(230, 230, 230));

        pRetVal->m_pMemoryBranch = new acTimelineBranch();
        pRetVal->m_pMemoryBranch->setText(tr(GPU_STR_timeline_DataTransferBranchName));
...
//++CF:LPGPU2
/// @brief     Return a pointer to the SYCL info branch for the given queue ID.
///            If one does not exist, it is created.
/// @param[in] queueId  The ID of the SYCL queue.
TraceView::SYCLQueueBranchInfo* TraceView::GetSYCLBranchInfo(uint64_t queueId)
{
  SYCLQueueBranchInfo* pRetVal = nullptr;

  // Create SYCL branch if it does not yet exist
  if (m_pSYCLBranch == nullptr)
  {
    m_pSYCLBranch = new acTimelineBranch();

    m_pSYCLBranch->setText(GPU_STR_timeline_SYCLBranchName);
  }

  if (m_syclQueueToQBIMap.contains(queueId))
  {
    pRetVal = m_syclQueueToQBIMap[queueId];
  }
  else
  {
    pRetVal = new SYCLQueueBranchInfo;
    auto& queueDepths = m_syclQueueDepthMap[queueId];
    // Create the queue branch
    pRetVal->m_pQueueBranch = new acTimelineBranch();
    pRetVal->m_pQueueBranch->setText(QString(tr(GPU_STR_timeline_QueueBranchName)).arg(queueId));
    pRetVal->m_pQueueBranch->SetBGColor(QColor::fromRgb(230, 230, 230));
    pRetVal->m_pQueueBranch->setMaskEnabled(false);

    pRetVal->m_pKernelBranch = new acTimelineBranch();
    pRetVal->m_pKernelBranch->setText(tr(GPU_STR_timeline_KernelBranchName));
    pRetVal->m_pKernelBranch->SetBGColor(QColor::fromRgb(230, 230, 230));
    pRetVal->m_pKernelBranch->setHeight(s_syclBranchHeightPx * queueDepths[static_cast<int>(SYCLProfiler::EventCategory::KERNEL_EXECUTION)]);
    pRetVal->m_pKernelBranch->setMaskEnabled(false);


    pRetVal->m_pMemoryBranch = new acTimelineBranch();
    pRetVal->m_pMemoryBranch->setText(tr(GPU_STR_timeline_DataTransferBranchName));
    pRetVal->m_pMemoryBranch->SetBGColor(QColor::fromRgb(230, 230, 230));
    pRetVal->m_pMemoryBranch->setHeight(s_syclBranchHeightPx * queueDepths[static_cast<int>(SYCLProfiler::EventCategory::DATA_TRANSFER)]);
    pRetVal->m_pMemoryBranch->setMaskEnabled(false);


    pRetVal->m_pCommandGroupBranch = new acTimelineBranch();
    pRetVal->m_pCommandGroupBranch->setText(tr(GPU_STR_timeline_CmdGroupBranchName));
    pRetVal->m_pCommandGroupBranch->SetBGColor(QColor::fromRgb(230, 230, 230));
    pRetVal->m_pCommandGroupBranch->setHeight(s_syclBranchHeightPx * queueDepths[static_cast<int>(SYCLProfiler::EventCategory::COMMAND_GROUP)]);
    pRetVal->m_pCommandGroupBranch->setMaskEnabled(false);

//++CF:LPGPU2 - Create new branches here if needed...
    pRetVal->m_pCommandGroupDetailBranch = new acTimelineBranch();
    pRetVal->m_pCommandGroupDetailBranch->setText(tr(GPU_STR_timeline_CmdGroupDetailBranchName));
    pRetVal->m_pCommandGroupDetailBranch->SetBGColor(QColor::fromRgb(230, 230, 230));
    pRetVal->m_pCommandGroupDetailBranch->setHeight(s_syclBranchHeightPx * queueDepths[static_cast<int>(SYCLProfiler::EventCategory::COMMAND_GROUP_DETAIL)]);
    pRetVal->m_pCommandGroupDetailBranch->setMaskEnabled(false);

//--CF:LPGPU2
    // Notice:
    // we do not initialize the fill operations branch by default. We only do it when there are fill operation functions

    pRetVal->m_pQueueBranch->addSubBranch(pRetVal->m_pCommandGroupBranch);
    pRetVal->m_pQueueBranch->addSubBranch(pRetVal->m_pCommandGroupDetailBranch);
    pRetVal->m_pQueueBranch->addSubBranch(pRetVal->m_pMemoryBranch);
    pRetVal->m_pQueueBranch->addSubBranch(pRetVal->m_pKernelBranch);

    m_pSYCLBranch->addSubBranch(pRetVal->m_pQueueBranch);
    m_syclQueueToQBIMap.insert(queueId, pRetVal);
  }
