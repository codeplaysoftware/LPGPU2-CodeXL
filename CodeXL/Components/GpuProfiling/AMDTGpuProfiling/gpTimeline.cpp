...
acAPITimelineItem* gpTimeline::CreateTimelineItem(ProfileSessionDataItem* pItem)
{
    ...
    GT_IF_WITH_ASSERT(pItem != nullptr)
    {
        ...
        bool isEnqueue = false, isEnqueueMem = false, isEnqueueOther = false;
        ...
        if (pItem->ItemType().m_itemMainType == ProfileSessionDataItem::CL_API_PROFILE_ITEM)
        {
            ...
            isEnqueueMem = (pItem->ItemType().m_itemSubType & CL_ENQUEUE_MEM) == CL_ENQUEUE_MEM;
            isEnqueueOther = (pItem->ItemType().m_itemSubType & CL_ENQUEUE_OTHER_OPERATIONS) == CL_ENQUEUE_OTHER_OPERATIONS;
        }
        ...
        GT_IF_WITH_ASSERT(pItem->GetAPIFunctionID(functionId))
        {
            ...
            {
                if (isEnqueue || isEnqueueMem || isEnqueueOther)
                {
    ...
    GT_IF_WITH_ASSERT(pGPUIDataItem != nullptr)
    {
        ...
        GT_IF_WITH_ASSERT(pGPUIDataItem->ItemType().m_itemMainType == ProfileSessionDataItem::CL_GPU_PROFILE_ITEM)
        {
            ...
            GT_IF_WITH_ASSERT((pGPUIDataItem->OwnerAPIItem() != nullptr) && (pAPITimelineItem != nullptr))
            {
                ...
                bool isEnqueueMem = (gpuItemType == ProfileSessionDataItem::GPU_MEMORY_ITEM);
                bool isEnqueueKernel = (gpuItemType == ProfileSessionDataItem::GPU_KERNEL_ITEM);
                bool isEnqueueOther = (gpuItemType == ProfileSessionDataItem::GPU_OTHER_ITEM);
