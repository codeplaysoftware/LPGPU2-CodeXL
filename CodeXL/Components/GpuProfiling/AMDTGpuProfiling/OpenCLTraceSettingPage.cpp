...
//++CF:LPGPU2 (this pointer is a member so it can be greyed out later if sycl is toggled off)
    m_pSyclCaption = new QLabel(GP_Str_ProjectSettingsSYCLTraceOptions);
//--CF:LPGPU2
...
//++CF:LPGPU2
    m_pSyclCaption->setStyleSheet(AF_STR_captionLabelStyleSheetMain);
//--CF:LPGPU2
    ...
    m_pOpenCLRadioButton     = new QRadioButton(GP_Str_ProjectSettingsOpenCLAPI);
	  m_pSYCLRadioButton = new QRadioButton(GP_Str_ProjectSettingsSYCLAPI);
...
//++CF:LPGPU2
    m_pSYCLRadioButton->setToolTip(GP_Str_ProjectSettingsSYCLAPITooltip);
//++CF:LPGPU2
...
//++CF:LPGPU2
    pGroup->addButton(m_pSYCLRadioButton);
//--CF:LPGPU2
...
//++CF:LPGPU2
    QHBoxLayout* pSyclRuntimeLibLayout = new QHBoxLayout();
    m_pSYCLRuntimeLibTitle = new QLabel(GP_Str_SYCLRuntimeLibPathCaption);
    m_pSYCLRuntimeLibTextEdit = new QLineEdit;
    m_pSYCLRuntimeLibPathButton = new QToolButton;
    m_pBrowseForSYCLLibButtonAction = new afBrowseAction(GP_Str_SYCLBrowseActionID);
    
    // Get the size for a button with standard icon:
    int buttonDim = (int)acScalePixelSizeToDisplayDPI(21);
    QSize buttonSize(buttonDim, buttonDim);
    
    m_pSYCLRuntimeLibPathButton->setText(QString::fromWCharArray(AF_STR_newProjectEditEnvVars));
    m_pSYCLRuntimeLibPathButton->setContentsMargins(0, 0, 0, 0);
    m_pSYCLRuntimeLibPathButton->setContentsMargins(0, 0, 0, 0);
    m_pSYCLRuntimeLibPathButton->setMaximumSize(buttonSize);
    m_pSYCLRuntimeLibPathButton->setMinimumSize(buttonSize);
    m_pSYCLRuntimeLibPathButton->setToolTip(GP_Str_SYCLRuntimeLibPathTooltip);
    m_pSYCLRuntimeLibPathButton->setDefaultAction(m_pBrowseForSYCLLibButtonAction);
    m_pSYCLRuntimeLibPathButton->setToolTip(AF_STR_newProjectProjectBrowseForExePathTooltipApp);
    m_pBrowseForSYCLLibButtonAction->setIcon(style()->standardIcon(QStyle::SP_DialogOpenButton));
    m_pBrowseForSYCLLibButtonAction->setToolTip(AF_STR_newProjectProjectBrowseForExePathTooltipApp);
    
    pSyclRuntimeLibLayout->addWidget(m_pSYCLRuntimeLibTitle);
    pSyclRuntimeLibLayout->addWidget(m_pSYCLRuntimeLibTextEdit);
    pSyclRuntimeLibLayout->addWidget(m_pSYCLRuntimeLibPathButton);
//--CF:LPGPU2
...
//++CF:LPGPU2
    pMainLayout->addWidget(m_pSYCLRadioButton);
//--CF:LPGPU2
...
//++CF:LPGPU2
    pMainLayout->addWidget(m_pSyclCaption);
    pMainLayout->addLayout(pSyclRuntimeLibLayout);
//--CF:LPGPU2
    ...
    GT_ASSERT(rc);
//++CF:LPGPU2
    rc = connect(m_pSYCLRadioButton, SIGNAL(clicked()), this, SLOT(OnAPITypeRadioButtonToggled()));
//--CF:LPGPU2
    GT_ASSERT(rc);
    ...
    GT_ASSERT(rc);
//++CF:LPGPU2
    rc = connect(m_pBrowseForSYCLLibButtonAction, SIGNAL(triggered()), this, SLOT(OnClickBrowseForSYCLLib()));
    GT_ASSERT(rc);

    connect(m_pSYCLRadioButton, &QRadioButton::toggled, this, &OpenCLTraceOptions::SetSYCLSettingsControlsEnabled);
//--CF:LPGPU2
...
void OpenCLTraceOptions::writeSession(gtString& projectAsXMLString, const gtString& type)
{
    ...
    gtString apiTypeStr = (m_currentSettings.m_apiToTrace == APIToTrace_HSA) ? GPU_STR_ProjectSettingsAPITypeHSA : ((m_currentSettings.m_apiToTrace == APIToTrace_SYCL) ? GPU_STR_ProjectSettingsAPITypeSYCL : GPU_STR_ProjectSettingsAPITypeOpenCL);
...
//++CF:LPGPU2 write out SYCL runtime library file value
    writeValue(projectAsXMLString, GPU_STR_ProjectSettingsSYCLRuntimePath, m_currentSettings.m_syclRuntimeLibName);
//--CF:LPGPU2
...
//++CF:LPGPU2
            else if (nodeVal == acGTStringToQString(GPU_STR_ProjectSettingsAPITypeSYCL))
            {
              m_currentSettings.m_apiToTrace = APIToTrace_SYCL;
            }
//--CF:LPGPU2
		        else
            {
...
//++CF:LPGPU2
        else if (childNode.nodeName() == acGTStringToQString(GPU_STR_ProjectSettingsSYCLRuntimePath))
        {
          m_currentSettings.m_syclRuntimeLibName = acQStringToGTString(nodeVal);
        }
//--CF:LPGPU2
...
{
    //++TLRS: LPGPU2: Set the state of the SYCL radio button
    m_pOpenCLRadioButton->setChecked(m_currentSettings.m_apiToTrace == APIToTrace_OPENCL);
    m_pSYCLRadioButton->setChecked(m_currentSettings.m_apiToTrace == APIToTrace_SYCL);
    ...
    m_pHSARadioButton->setEnabled(Util::IsHSAEnabled());
    SetSYCLSettingsControlsEnabled(m_pSYCLRadioButton->isChecked());
    
//++CF:LPGPU2
    m_pSYCLRuntimeLibTextEdit->setText(acGTStringToQString(m_currentSettings.m_syclRuntimeLibName));
//--CF:LPGPU2
...
bool OpenCLTraceOptions::AreSettingsValidSecondPass(gtString &vInvalidMessageStr)
{
  //++TLRS: LPGPU2: Validating the selection of HSA or SYCL
  if (m_currentSettings.m_apiToTrace == APIToTrace_HSA && !Util::IsHSAEnabled())
  {
      vInvalidMessageStr = GP_Str_AppTraceNoHSA_W;
      return false;
  }

  if (m_currentSettings.m_apiToTrace == APIToTrace_SYCL && !Util::IsSYCLEnabled())
  {
      vInvalidMessageStr = GP_Str_AppTraceNoSYCL_W;
      return false;
  }   
  //--TLRS: LPGPU2: Validating the selection of HSA or SYCL
...
bool OpenCLTraceOptions::SetTraceOptions(APITraceOptions& apiTraceOptions)
{
    ...
    apiTraceOptions.m_apiToTrace = m_pHSARadioButton->isChecked() ? APIToTrace_HSA : (m_pSYCLRadioButton->isChecked() ? APIToTrace_SYCL : APIToTrace_OPENCL);
//++CF:LPGPU2
    apiTraceOptions.m_syclRuntimeLibName = acQStringToGTString(m_pSYCLRuntimeLibTextEdit->text());
//--CF:LPGPU2
    UpdateAPIFilterList();
...
/// @brief Retrieve the Trace options 
/// @return APITraceOptions Reference to athe settings object.
const APITraceOptions& OpenCLTraceOptions::GetTraceOptions() const
{
...
{
    const bool bSYCLInUse = m_pSYCLRadioButton->isChecked();
//++IOR: LPGPU2 Code SYCL implementation runtime location
    m_currentSettings.m_apiToTrace = m_pHSARadioButton->isChecked() ? APIToTrace_HSA : (bSYCLInUse ? APIToTrace_SYCL : APIToTrace_OPENCL);
//--IOR: LPGPU2 Code SYCL implementation runtime location
...
//++CF:LPGPU2
    m_currentSettings.m_syclRuntimeLibName = acQStringToGTString(m_pSYCLRuntimeLibTextEdit->text());
//--CF:LPGPU2
...
void OpenCLTraceOptions::UpdateAPIFilterList()
{
    ...
    if (nullptr != pCurrProjectSetting)
    {
        ...
        GT_IF_WITH_ASSERT(m_pHSARadioButton != nullptr)
        {
            ...
            const auto bIsOpenCL = m_pOpenCLRadioButton->isChecked();
            //++TLRS: LPGPU2: Need to check if SYCL radio button is selected
            const auto bIsSYCL = m_pSYCLRadioButton->isChecked();
            //--TLRS: LPGPU2: Need to check if SYCL radio button is selected
            ...
            while ((*iterator) != nullptr)
            {
                if ((*iterator)->childCount() == 0)
                {
                    if ((*iterator)->checkState(0) == Qt::Unchecked)
                    {
                        ...
                        //++TLRS: LPGPU2: Handle the case where the SYCL radio button is selected
                        else if(bIsOpenCL || bIsSYCL)
                        //--TLRS: LPGPU2: Handle the case where the SYCL radio button is selected
                        {
    ...
    try
    {
        ...
        if (file.open(QIODevice::WriteOnly))
        {
            ...
            if (item != nullptr)
            {
                ...
                if (m_currentSettings.m_apiToTrace == APIToTrace_OPENCL || m_currentSettings.m_apiToTrace == APIToTrace_SYCL)
                {
...
void OpenCLTraceOptions::OnAPITypeRadioButtonToggled()
{
    ...
    {
        // Check if the OpenCL/SYCL or HSA is selected:
        APIToTrace api = m_pHSARadioButton->isChecked() ? APIToTrace_HSA : (m_pSYCLRadioButton->isChecked() ? APIToTrace_SYCL : APIToTrace_OPENCL);
        m_pShowAPIErrorCodeCB->setEnabled(api == APIToTrace_OPENCL || api == APIToTrace_SYCL);
        m_pCollapseCallsCB->setEnabled(api == APIToTrace_OPENCL || api == APIToTrace_SYCL);
        m_pGenerateOccupancyCB->setEnabled(api == APIToTrace_OPENCL || api == APIToTrace_SYCL);
...
void OpenCLTraceOptions::FitTreesToAPIType(APIToTrace api)
{
    ...
    GT_IF_WITH_ASSERT((m_pAPIRulesTW != nullptr) && (m_pAPIsToTraceTW != nullptr) && (m_pOpenCLRootItem != nullptr) && (m_pHSARootItem != nullptr))
    {
        ...
        QString rulesCaption = (api == APIToTrace_OPENCL || api == APIToTrace_SYCL) ? GP_Str_ProjectSettingsAllCLRulesItemName : GP_Str_ProjectSettingsAllHSARulesItemName;
        ...
        while (*iter)
        {
            ...
            if (api != APIToTrace_OPENCL || api != APIToTrace_SYCL)
            {
        ...
        //++TLRS: LPGPU2: Fix to show OpenCL APIs for selection in the tree view
        // We want to hide the nodes if, and only if, api is neither OpenCL nor SYCL
        m_pAPIsToTraceTW->setItemHidden(m_pOpenCLRootItem, api != APIToTrace_OPENCL && api != APIToTrace_SYCL);
        //--TLRS: LPGPU2: Fix to show OpenCL APIs for selection in the tree view
...
//++TLRS: LPGPU2:
/// @brief  Enables or disables the SYCL related controls on this projects settings.
/// @param[in] vbEnabled The new status of the SYCL related controls.
void OpenCLTraceOptions::SetSYCLSettingsControlsEnabled(const bool vbEnabled)
{    
    m_pSyclCaption->setEnabled(vbEnabled);
    m_pSYCLRuntimeLibTitle->setEnabled(vbEnabled);
    m_pSYCLRuntimeLibTextEdit->setEnabled(vbEnabled);
    m_pSYCLRuntimeLibPathButton->setEnabled(vbEnabled);
}
//--TLRS: LPGPU2:
...
//++CF:LPGPU2
void OpenCLTraceOptions::OnClickBrowseForSYCLLib()
{
  afApplicationCommands* pApplicationCommands = afApplicationCommands::instance();
  
  GT_IF_WITH_ASSERT(m_pSYCLRuntimeLibTextEdit && pApplicationCommands)
  {
    // Get the shared library file types
    QString sharedLibTypes = AF_STR_sharedLibFilesDetails;
  
    // Get the current selected library
    QString currentLibFile = m_pSYCLRuntimeLibTextEdit->text();
  
    QString selectedLib = pApplicationCommands->ShowFileSelectionDialog(AF_STR_newProjectSYCLRuntimeLibSelectionTitle,
      currentLibFile, sharedLibTypes, m_pBrowseForSYCLLibButtonAction, false);
  
    if (!selectedLib.isEmpty())
    {
      m_pSYCLRuntimeLibTextEdit->setText(selectedLib);
    }
  }
}
//--CF:LPGPU2
