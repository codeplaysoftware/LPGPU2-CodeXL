...
public:
    ...
    ExplorerSessionId GetCurrentDisplaySessionId() const;
    //--TLRS: LPGPU2: Need to know what is the current session being displayed
...
signals:
    ...
    //++TLRS: LPGPU2: Adding signal for when a session is added to the tree
    void SessionAdded(SessionTreeNodeData *vpNewSessionData);
    //--TLRS: LPGPU2: Adding signal for when a session is added to the tree
...
private:
    ...
    ExplorerSessionId m_currentDisplaySessionId = SESSION_ID_ERROR;
};
