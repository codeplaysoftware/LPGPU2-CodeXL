...
ExplorerSessionId ProfileApplicationTreeHandler::AddSession(SessionTreeNodeData* pNewItemSessionData, bool activateItem)
{
    ...
    if (WasTreeCreated())
    {
        ...
        //++TLRS: LPGPU2: Emit the same signal as above but with the session that was added.
        emit SessionAdded(pNewItemSessionData);
        //--TLRS: LPGPU2: Emit the same signal as above but with the session that was added.
    }
...
//++TLRS: LPGPU2: Need to know what is the current session being displayed
/// @brief                      Returns the current session being displayed.
/// @return ExplorerSessionId   The id of the current session being displayed.
ExplorerSessionId ProfileApplicationTreeHandler::GetCurrentDisplaySessionId() const
{
    return m_currentDisplaySessionId;
}
//--TLRS: LPGPU2: Need to know what is the current session being displayed
    ...
    GT_IF_WITH_ASSERT((nullptr != m_pApplicationTree) && (nullptr != m_pApplicationTree->treeControl()))
    {
        ...
        if (isInProfileMode || isInFrameAnalysisMode)
        {
            ...
            if (nullptr != pItemToSelect)
            {
                ...
                //++TLRS: LPGPU2: Need to emit the signal here to inform the timeline view
                auto* pSessionNodeData = qobject_cast<SessionTreeNodeData*>(pItemToSelect->extendedItemData());
                if (pSessionNodeData != nullptr)
                {
                    m_currentDisplaySessionId = pSessionNodeData->m_sessionId;
                    emit SessionShown(pSessionNodeData->m_sessionId);
                }
                //--TLRS: LPGPU2: Need to emit the signal here to inform the timeline view
