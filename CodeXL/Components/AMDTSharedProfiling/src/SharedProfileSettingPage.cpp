...
#include <../GpuProfiling/AMDTGpuProfiling/gpStringConstants.h>
...
void SharedProfileSettingPage::Initialize()
{
    ...
    int currentRow = 0;
    pMainLayout->addWidget(pCaption1, currentRow, 0, 1, 4);
...
/// @brief Called on dialog ok button each extension's settings can check the 
///        parameters entered are valid.
/// @param[out]  vStrInvalidMsg On error return error description.
/// @return      bool True = success, false = error condition.
bool SharedProfileSettingPage::AreSettingsValid(gtString& invalidMessageStr)
{
//++CF:LPGPU2 Always return true here as the save is deferred to the second pass
...
bool SharedProfileSettingPage::AreSettingsValidSecondPass(gtString &vInvalidMessageStr)
{
...
bool SharedProfileSettingPage::SaveCurrentSettings()
{
    ...
    GT_IF_WITH_ASSERT((m_pProfileTypeCombo != NULL) && (m_pSystemWideRadioButton != NULL) && (m_pSystemWideWithFocusRadioButton != NULL) && (m_pSingleApplicationRadioButton != NULL))
    {
        ...
        afNewProjectDialog::instance().SaveProfileExtnParamProfileSessionType(currentProfileType);
    }
