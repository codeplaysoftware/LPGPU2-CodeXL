...
public:
    ...
    bool PrepareGetSessionInfoValueStatement()
    {
        ...
        if (!ret)
        {
            std::string s = sqlite3_errmsg(m_pReadDbConn);
            s.append("bla");
        }
        ...
        GT_IF_WITH_ASSERT(pDbName != nullptr)
        {
            ...
            GT_IF_WITH_ASSERT(rc == SQLITE_OK)
            {
                ...
                if (ret && ((profileType & AMDT_PROFILE_MODE_TIMELINE) == AMDT_PROFILE_MODE_TIMELINE))
                {
                    ...
                    //++TLRS: LPGPU2: Need to prepare the database for reading here

                    if (ret)
                    {
                        if (sqlite3_open_v2(pDbName, &m_pReadDbConn, SQLITE_OPEN_READONLY, nullptr) == SQLITE_OK)
                        {
    ...
    bool OpenProfilingDatabase(const char* dbNameAsUtf8, gtUInt64 profileType, bool isReadOnly)
    {
        ...
        //++TLRS: LPGPU2: When opening a database for writing, create the write connection
        if (m_canUpdateDB)
        {
//++AT:LPGPU2
            const auto res = sqlite3_open_v2(dbNameAsUtf8, &m_pWriteDbConn,
                flags, nullptr);
            GT_ASSERT_EX(res == SQLITE_OK, L"Cannot open DB for writing");
            if (res != SQLITE_OK)
            {
              const auto *msg = sqlite3_errmsg(m_pWriteDbConn);
              gtString msgAsGtStr;
              msgAsGtStr << msg;
              OS_OUTPUT_FORMAT_DEBUG_LOG(OS_DEBUG_LOG_ERROR,
                  L"Could not get the open DB, reason: %s",
                  msgAsGtStr.asCharArray());
            }
//--AT:LPGPU2
        }
        ...
        //--TLRS: LPGPU2: When opening a database for writing, create the write connection
//++AT:LPGPU2
        const auto res = sqlite3_open_v2(dbNameAsUtf8, &m_pReadDbConn,
            flags, nullptr);
        GT_ASSERT_EX(res == SQLITE_OK, L"Cannot open DB for writing");
        if (res == SQLITE_OK)
//--AT:LPGPU2
    ...
    bool InsertSamples(const gtVector<PPSampleData>& samples)
    {
        ...
        GT_IF_WITH_ASSERT(m_pWriteDbConn != nullptr)
        {
            ...
            for (const auto& sample : samples)
            {
                // Bind the values to the parameters.
//++AT:LPGPU2
                sqlite3_bind_int64(m_pValsInsertStmt, 1, sample.m_quantizedTime);
                sqlite3_bind_int64(m_pValsInsertStmt, 2, sample.m_counterID);
//--AT:LPGPU2
                                       ...
                                       gtMap<int, gtVector<SampledValue>>& sampledValuesPerCounter)
    {
        ...
        GT_IF_WITH_ASSERT(numOfCounterIds > 0)
        {
            ...
            {
                // Bind the parameters.
                //++TLRS: LPGPU2: Fetching a 64 bit integer from the database
                sqlite3_bind_int64(pQueryStmt, 1, samplingTimeRange.m_fromTime);
                sqlite3_bind_int64(pQueryStmt, 2, samplingTimeRange.m_toTime);
                //--TLRS: LPGPU2: Fetching a 64 bit integer from the database
...
bool AmdtDatabaseAccessor::GetCounterIdByName(const std::string& counterName, int& counterId) const
{
    bool ret = false;
    GT_IF_WITH_ASSERT(m_pImpl != nullptr)
    {
...
//++TLRS: LPGPU2: The child classes need access to the sqlite connection

/// @brief  Returns the sqlite database connection used for writing operations
/// @return sqlite3*    A pointer to the sqlite3 database connection
sqlite3* AmdtDatabaseAccessor::GetDatabaseWriteConnection() const
{
    return m_pImpl->m_pWriteDbConn;
}

/// @brief  Returns the sqlite database connection used for reading operations
/// @return sqlite3*    A pointer to the sqlite3 database connection
sqlite3* AmdtDatabaseAccessor::GetDatabaseReadConnection() const
{
    return m_pImpl->m_pReadDbConn;
}
