...
amdtProfileDbAdapter::amdtProfileDbAdapter()
{

}

/// @brief  Initialises the database accessor with the 
///         default AMDTProfilerDAL::AmdtDatabaseAccessor instance.
/// @return bool    true = The initialisation was successful,
///                 false = An error has occurred.
bool amdtProfileDbAdapter::Initialise()
{
    bool bReturn = false;    

    m_pDbAccessor = new (std::nothrow) AMDTProfilerDAL::AmdtDatabaseAccessor();    

    GT_IF_WITH_ASSERT_EX(m_pDbAccessor != nullptr, L"ERROR: Cannot allocate AmdtDatabaseAccessor")
    {
        bReturn = true;
    }
...
//++AT:LPGPU2
            static_cast<gtInt64>(pSample->m_sampleElapsedTimeMs),
            static_cast<gtInt32>(beVal.m_counterId),
