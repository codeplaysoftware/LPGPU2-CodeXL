...
//++AT:LPGPU2
// Used to convert unit types provided as strings (when using the Android session mode) to AMDTPwrUnit types
// Their names have been read by the sample TargetCharacteristics.xml file provided in the DB "shaders.cxldb"
// given by Martyn
const gtString kAndroidFrequencyUnitStr = L"Frequency";
const gtString kAndroidLoadUnitStr = L"Load";
const gtString kAndroidCentigradeUnitStr = L"Centigrade";
const gtString kAndroidFPSUnitStr = L"FPS";
const gtString kAndroidMicroAmpsUnitStr = L"MicroAmps";
const gtString kAndroidPowerWattUnitsStr = L"PowerWatts";
//--AT:LPGPU2
...
//++AT:LPGPU2
/// @brief Convert an LPGPU2 CounterElement to a PwrCounterDesc expected by CXL
/// @param deviceID ID of the device where the counter comes from
/// @param element The counter element to convert
/// @return Ptr to a CXL pwr counter description
/// @note We do not return an error code but a ptr here because we need to
///   conform with CXL's code style as much as possible since we are in CLX's code
///   base.
///   The ptr points to an object allocated on the heap; this is considered unsafe
///   in C++ but this is how CXL does things already.
AMDTPwrCounterDesc *BackendDataConvertor::ConvertToPwrCounterDesc(
    gtUInt32 deviceId, const lpgpu2::CounterElement &element)
{
  // Allocate the object.
  AMDTPwrCounterDesc* pRet = new AMDTPwrCounterDesc;

  // Assign the primitive types.
  pRet->m_deviceId        = deviceId;
  pRet->m_aggregation     = AMDT_PWR_VALUE_SINGLE;
  pRet->m_category        = ConvertToPwrCategory(element.GetUnit());
  pRet->m_counterID = element.GetId();
  pRet->m_maxValue        = 1000;
  pRet->m_minValue        = 0;
  pRet->m_units           = ConvertToPwrUnit(element.GetUnit(),
      element.GetType());

  // Copy the description string.
  size_t sz = element.GetDescription().length();

  if (sz > 0)
  {
    pRet->m_description = new char[sz + 1];
    strncpy(pRet->m_description, element.GetDescription().asASCIICharArray(), sz);
    pRet->m_description[sz] = '\0';
  }

  // Copy the name string.
  sz = element.GetName().length();
  if (sz > 0)
  {
    pRet->m_name = new char[sz + 1];
    strncpy(pRet->m_name, element.GetName().asASCIICharArray(), sz);
    pRet->m_name[sz] = '\0';
  }
...
/// @brief Utility method to match a unit defined by a string to
///        its enum variant
/// @param units String containing the unit to convert
/// @return The converted unit
AMDTPwrUnit BackendDataConvertor::ConvertToPwrUnit(const gtString &units,
    const gtString &type)
{
  if (units == kAndroidFrequencyUnitStr)
  {
    return AMDT_PWR_UNIT_TYPE_MEGA_HERTZ;
  }
  else if (units == kAndroidLoadUnitStr)
  {
    if (type == L"Float")
    {
      return AMDT_PWR_UNIT_TYPE_COUNT;
    }
    else if (type == L"Percentage")
    {
      return AMDT_PWR_UNIT_TYPE_PERCENT;
    }
  }
  else if (units == kAndroidCentigradeUnitStr)
  {
    return AMDT_PWR_UNIT_TYPE_CENTIGRADE;
  }
  else if (units == kAndroidFPSUnitStr)
  {
    return AMDT_PWR_UNIT_TYPE_RATIO;
  }
  else if (units == kAndroidMicroAmpsUnitStr)
  {
    return AMDT_PWR_UNIT_TYPE_COUNT;
  }
  else if (units == kAndroidPowerWattUnitsStr)
  {
    return AMDT_PWR_UNIT_TYPE_WATT;
  }

  return AMDT_PWR_UNIT_TYPE_COUNT;
}

/// @brief Utility method to match a category defined by a string to
///        its enum variant
/// @param units String containing the category to convert
/// @return The converted category
AMDTPwrCategory BackendDataConvertor::ConvertToPwrCategory(const gtString &units)
{
  if (units == kAndroidFrequencyUnitStr)
  {
  ...
  {
    return AMDT_PWR_CATEGORY_COUNT;
  }
  else if (units == kAndroidCentigradeUnitStr)
  {
  ...
  // NM ++ Fix AMD CXL expects this as default category when in power profiling execution mode ???
  //return AMDT_PWR_CATEGORY_COUNT;
  return AMDT_PWR_CATEGORY_POWER;
}
//--AT:LPGPU2
