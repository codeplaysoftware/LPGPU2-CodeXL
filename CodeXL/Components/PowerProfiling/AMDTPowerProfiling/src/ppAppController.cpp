...
//++AT:LPGPU2
    ...
    connect(m_pTimer, SIGNAL(timeout()), this, SLOT(onProfileTimeout()));
...
/// @brief                 Exports the given session to CSV files. It will ask the user to select an output
///                        folder and will write one CSV file per table in the session database. The file names
///                        will get the session name as a prefix.
/// @param[in] vSessionId  The id of the session to be exported.
/// @note                  Returns void because, in case of an error, it will be displayed to the user.
void ppAppController::ExportSessionToCSV(ExplorerSessionId vSessionId) const
{
    if (vSessionId != SESSION_ID_ERROR)
    {                
        auto *pTreeHandler = ProfileApplicationTreeHandler::instance();
        if (pTreeHandler != nullptr)
        {
            auto *pTreeItemData = pTreeHandler->GetSessionNodeItemData(vSessionId);
            auto *pTreeNodeData = pTreeHandler->GetSessionTreeNodeData(vSessionId);

            if (pTreeItemData != nullptr && pTreeNodeData != nullptr)
            {
                const auto dialogTitle = QString{ PP_STR_ExportToCSVDialogTitle }.arg(pTreeNodeData->m_displayName);
                QString defaultFolder{};
                const auto outputFolder = afApplicationCommands::instance()->ShowFolderSelectionDialog(dialogTitle, defaultFolder, nullptr);
                if (!outputFolder.isEmpty())
                {                    
                    lpgpu2::db::LPGPU2DatabaseAdapter dbAdapter;

                    const auto bIsReadOnly = true;
                    // Open with the mode AMDT_PROFILE_MODE_NONE since we do not care about the underlying data for this operation
                    if (dbAdapter.OpenDb(pTreeItemData->m_filePath.asString(), AMDT_PROFILE_MODE_NONE, bIsReadOnly) == lpgpu2::db::success)
                    {
                        auto fileNamePrefix = acQStringToGTString(pTreeNodeData->m_displayName);
                        fileNamePrefix << L"_";

                        const auto bOverrideExistingFiles = true;
                        if (dbAdapter.ExportToCSV(acQStringToGTString(outputFolder), fileNamePrefix, bOverrideExistingFiles) == lpgpu2::db::success)
                        {
                            afApplicationCommands::instance()->ShowMessageBox(QMessageBox::Information, PP_STR_ExportToCSVSuccessTitle, PP_STR_ExportToCSVSuccessMsg);
                        }
                        else
                        {
                            const auto dbErrorMsg = dbAdapter.GetErrorMessage();
                            const auto errorMsg = QString{ PP_STR_ExportToCSVErrorMsg }.arg(acGTStringToQString(dbErrorMsg));                            
                            afApplicationCommands::instance()->ShowMessageBox(QMessageBox::Critical, PP_STR_ExportToCSVErrorTitle, errorMsg);
                        }
                    }
                }
            }
...
void ppAppController::SetAlwaysRestartRAgent(bool bAlwaysRestartRAgent)
{
    m_bAlwaysRestartRAgent = bAlwaysRestartRAgent;
}

void ppAppController::SetStandaloneSessionDbPath(const osFilePath & vDbSessionPath)
{
    if (vDbSessionPath.exists())
    {
        m_currentSessionInfo.m_sessionDbFullPath = vDbSessionPath.asString();
    }    
}

/// @brief                                 Returns the list of all sessions loaded in
///                                        this project.
/// @returns QList<ppsessionTreeNodeData*> The list of all the sessions in this project.
const QList<ppSessionTreeNodeData*>& ppAppController::GetSessionsTreeData() const
{
    return m_sessions;
}

/// @brief                Checks if the feedback engine can be executed for the
///                       specified version.
/// @param[in] vSessionId The id of the session to which run the feedback engine.
/// @returns   bool       true: The feedback engine can be executed for this session,
///                       false: This session is not a LPGPU2 session.
bool ppAppController::CanRunLPGPU2FeedbackEngine(const ExplorerSessionId vSessionId) const
{
    auto bReturn = false;
    ...
    if (vSessionId != SESSION_ID_ERROR)
    {
        auto *pTreeItemData = ProfileApplicationTreeHandler::instance()->GetSessionNodeItemData(vSessionId);
        if (pTreeItemData != nullptr)
        {
            dbFilePath = pTreeItemData->m_filePath.asString();
        }
    }
    else
    {
        dbFilePath = m_currentSessionInfo.m_sessionDbFullPath;
    }
    ...
    if (dbFilePath.exists())
    {
        ...
        const auto bIsReadOnly = true;
        if (dbAdapter.OpenDb(dbFilePath.asString(), AMDT_PROFILE_MODE_TIMELINE, bIsReadOnly) == lpgpu2::db::success)
        {
            auto bIsLPGPU2TargetDevice = false;
            bReturn = dbAdapter.IsLPGPU2TargetDevice(bIsLPGPU2TargetDevice);
            bReturn &= bIsLPGPU2TargetDevice;
        }
    }     
...
/// @brief                 Displays the Feedback Engine dialog for the specified session id.
/// @param[in] vSessionId  The id of the session to which run the feedback engine.
/// @note                  Emits signal FeedbackEngineExecuted() if no error has occurred.
void ppAppController::RunLPGPU2FeedbackEngine(const ExplorerSessionId vSessionId)
{
    auto status = lpgpu2::PPFnStatus::failure;

    if (vSessionId != SESSION_ID_ERROR)
    {
        status = LPGPU2ppFeedbackEngineDialog::OpenLPGPU2ppFeedbackEngineDialog(vSessionId);
    }
    else
    {
        status = LPGPU2ppFeedbackEngineDialog::OpenLPGPU2ppFeedbackEngineDialog(m_currentSessionInfo.m_sessionDbFullPath);
    }
    
    if (status == lpgpu2::PPFnStatus::success)
    {
        // No need to care about the status here. The user will get a message in the feedback
        // engine dialog in case of any error during the execution of the feedback engine.
        emit FeedbackEngineExecuted();
    }
}

void ppAppController::RunLPGPU2FeedbackEngine(const osFilePath& vDbFilePath)
{
    const auto status = LPGPU2ppFeedbackEngineDialog::OpenLPGPU2ppFeedbackEngineDialog(vDbFilePath);
    if (status == lpgpu2::PPFnStatus::success)
    {
        // No need to care about the status here. The user will get a message in the feedback
        // engine dialog in case of any error during the execution of the feedback engine.
...
void ppAppController::onProfileStarted(const gtString& profileTypeStr, const spISharedProfilerPlugin* const pCallback, osProcessId processId)
{
    ...
    if ((profileTypeStr == acQStringToGTString(PM_profileTypePowerProfile)) && (pCallback == static_cast<spISharedProfilerPlugin*>(this)))
    {
        ...
        //++TLRS: LPGPU2: Making sure the remote agent is running before start profiling
        const auto& theProjSettings = afProjectManager::instance().currentProjectSettings();
        
        if (theProjSettings.isAndroidDeviceTarget())
        {
            auto &adbCommands = LPGPU2ppADBCommands::Instance();

            const auto bNeedToWaitAdb = adbCommands.StartADB();
            const auto bNeedToWaitPortForward = adbCommands.PortFoward(m_currentProjectSettings.m_remoteTargetDaemonConnectionPort, 21845);
            adbCommands.EnableLPGPU2VulkanLayer();  // this can be done at any time

            auto bIsAndroidOk = true;

            // Start ADB first
            if ((bNeedToWaitAdb == EWaitStatus::kNeedToWait) ||
                (bNeedToWaitPortForward == EWaitStatus::kNeedToWait))
            {
                const auto startLoopTime = std::chrono::system_clock::now();
                while (!adbCommands.IsADBRunning())
                {
                    qApp->processEvents();
            ...
            // Shutdown the RAgent in case it is running.
            // In theory we could avoid this if the reconnection to the
            // RAgent works properly. Right now this is the safest bet.
            if (adbCommands.IsRAgentRunning() && m_bAlwaysRestartRAgent)
            {
                const auto bNeedTowWaitForStop = adbCommands.StopRAgent();
                if (bNeedTowWaitForStop == EWaitStatus::kNeedToWait)
                {
                    const auto startLoopTime = std::chrono::system_clock::now();
                    while (adbCommands.IsRAgentRunning())
                    {
                        qApp->processEvents();
            ...
            const auto bNeedToWaitRAgent = m_bAlwaysRestartRAgent ? adbCommands.StartRAgent() : EWaitStatus::kNoNeedToWait;

            if (bNeedToWaitRAgent == EWaitStatus::kNeedToWait)
            {
                const auto startLoopTime = std::chrono::system_clock::now();
                while (!adbCommands.IsRAgentRunning())
                {
            ...
            if (!bIsAndroidOk)
            {                
                acMessageBox::instance().warning(afGlobalVariablesManager::ProductNameA(), PP_STR_ADBErrorStartProfiling);
...
//++AT:LPGPU2
    if (theProjSettings.isAndroidDeviceTarget())
    {
        // We read the android target characteristics from the ppAppController
        // instance rather than from the
        // global project settings because the android profiling is specific
        // to the power profiling session
        // mode
        coreInitDetails.m_isAndroidProfiling = true;
        coreInitDetails.m_remoteHostAddr =
          m_currentProjectSettings.m_remoteTargetName;
        coreInitDetails.m_remotePortNumber =
          m_currentProjectSettings.m_remoteTargetDaemonConnectionPort;
        coreInitDetails.m_collectionDefs =
          m_currentProjectSettings.m_AndroidCollectionDefs;
        coreInitDetails.m_targetDefinition =
          m_currentProjectSettings.m_AndroidTargetDefinition;
        coreInitDetails.m_targetCharacteristics =
          m_currentProjectSettings.m_AndroidCharacteristics;

        // Call power model factory here. Get the devicename
        //m_AndroidTargetDefinition.GetDevices()

        m_countersByCategory.clear(); // On android remote profile we clear cache away
    }
//--AT:LPGPU2
    ...
    // If the initialisation failed the first time around and the session is
    // of type Android (LPGPU2), then try once more to initialise
    // the connection, since the RAgent might have simply been restarted. This
    // avoids having the user press the Play button two times in such case.
    //
    // However if it fails a second time, simply communicate this to the user
    // and stop profiling
    if(rcMidTier >= PPR_FIRST_ERROR && theProjSettings.isAndroidDeviceTarget())
    {
      rcMidTier = m_middleTierCore.InitPowerProfiler(coreInitDetails);
    }
...
//++AT:LPGPU2
            counterData.m_color = theColorsMap.GetColorForCounterID(pCurrentCounterDesc->m_counterID);
//--AT:LPGPU2
...
//++AT:LPGPU2
                else if (theProjSettings.isAndroidDeviceTarget())
                {
                    targetAppDetails.m_isAndroidSession = true;                    
                }
//--AT:LPGPU2
...
//++AT:LPGPU2
                ...
                if (m_durationMode == lpgpu2::DurationMode::Timer)
                {
                  // Start timing here
                  m_pTimer->start(m_timerDuration);
                }
//--AT:LPGPU2
...
ppSessionTreeNodeData* ppAppController::CreateSession(const gtString& profileTypeStr, bool& wasSessionCreated)
{
    ...
    AddSessionTreeNodeData(pRetVal);
...
//++AT:LPGPU2
            if (IsMidTierInitialized())
            {
              ...
              {
                // Show msg box
                lpgpu2::FlushDataDialog dialog;
                connect(&dialog, &lpgpu2::FlushDataDialog::dialogShown, this, [&]
                {
                ...
                bool isAndroidSession = afProjectManager::instance().currentProjectSettings().isAndroidDeviceTarget();

                if (isLocalSession && !isAndroidSession)
                {
                    m_middleTierCore.ShutdownPowerProfiler();
                }
            }
//--AT:LPGPU2
            ...
            bool rc = tempBL.OpenPowerProfilingDatabaseForRead(m_pCurrentRunningSessionData->m_pParentData->m_filePath.asString(), bIsReadOnly);
...
//++NM:LPGPU2
            ...
            // Create a dbAdapter
            auto dbPath = m_pCurrentRunningSessionData->m_pParentData->m_filePath.asString();
            
            // Create and launch the selected power models once the database is ready
            // clear all models first. This is needed because session time might have been changed
            for (auto& pwDesriptor : m_powerModelDescriptors) {

                auto pwModel = lpgpuPowerModelsFactory::instance().createPowerModel(pwDesriptor, dbPath);
                pwModel->estimatePower();
...
void ppAppController::CreateLoadedSession(gtString& sessionFilePath)
{
    ...
    const bool bIsReadOnly = true;
    bool isOk = tmpBl.OpenPowerProfilingDatabaseForRead(sessionFilePath, bIsReadOnly);
    ...
    //++TLRS: LPGPU2: Store the pointer to the new session
    AddSessionTreeNodeData(pRetVal);
    //++TLRS: LPGPU2: Store the pointer to the new session
}
...
{
    //++AT:LPGPU2
    for (gtSizeType ic = 0; ic < AMDT_PWR_CATEGORY_CNT; ++ic)
    {
      gtSet<int> counters;
      auto rcPower = GetAllCountersInCategory(static_cast<AMDTPwrCategory>(ic), counters);
      if (rcPower)
      {
...
void ppAppController::AddSessionTreeNodeData(ppSessionTreeNodeData *vpSession)
{
    GT_IF_WITH_ASSERT(vpSession != nullptr && !m_sessions.contains(vpSession))
    {
        m_sessions.append(vpSession);
    }
}

void ppAppController::RemoveSessionTreeNodeData(ppSessionTreeNodeData *vpSession)
{
    GT_IF_WITH_ASSERT(vpSession != nullptr && m_sessions.contains(vpSession))
    {
        m_sessions.removeOne(vpSession);
    }
}
...
void ppAppController::OnImportSession(const QString& strSessionFilePath, bool& wasImported)
{
    ...
    if (importedFileExtension.compareNoCase(PP_STR_dbFileExt) == 0)
    {
        ...
        if (sessionOldDirectory.directoryPath() != baseDir.directoryPath())
        {
            ...
            // Define a profile BL to get the session information from:
            PowerProfilerBL tempBL;
            const bool bIsReadOnly = true;
            rc = tempBL.OpenPowerProfilingDatabaseForRead(acQStringToGTString(strSessionFilePath), bIsReadOnly);
            ...
            GT_IF_WITH_ASSERT(rc)
            {
                ...
                AddSessionTreeNodeData(pImportSessionData);
...
void ppAppController::OnSessionDelete(ExplorerSessionId deletedSessionId, SessionExplorerDeleteType deleteType, bool& canDelete)
{
    ...
    if ((pSessionData != nullptr) && (pSessionData->m_pParentData != nullptr))
    {
        ...
        //++TLRS: LPGPU2: Remove the session data from the session list
        RemoveSessionTreeNodeData(pSessionData);
        //--TLRS: LPGPU2: Remove the session data from the session list
    }
}
...
//++AT:LPGPU2
/// @brief Called by the collection timer timeout signal. It calls the
///        ppAppControlelr onProfileStopped()
/// @note We do not return anything because QT expects the function
///       signature to be like this, even though this goes against the
///       team's coding convention
void ppAppController::onProfileTimeout()
{
  onProfileStopped(this, false);
}
//--AT:LPGPU2
