...
//------------------------- General ---------------------
//++AT:LPGPU2
/// @brief  Get all the counters given a graph type and their parent device ID
/// @param[out] allRelevantCounters Vector of retrieved counters.
/// @param[in] graphType Type of graph from which to infer the type of
///            counters needed.
/// @param[in] deviceId Parent ID of the counter
/// @param[in] pSessionController Ptr to the session controller used to retrieve
///            the information
/// @returns   bool              true = the information was inserted into
///                              the database, false = an error has occurred
...
void ppDataUtils::GetRelevantCounterIdsByGraphTypeAndDeviceId(
    gtVector<int> &allrelevantCounters, GraphViewCategoryType graphType,
    int deviceId,
    ppSessionController* pSessionController)
{
  ...
  GT_IF_WITH_ASSERT(pSessionController != nullptr)
  {
    ...
    switch (graphType)
    {
      ...
      // mgb:ss add support for timeline counters
      case TIMELINE_COUNTER:
    ...
    if (isCategoryFound)
    {
      ...
      pSessionController->GetEnabledCountersByTypeAndCategoryAndDeviceId(
          tmpTyesVec, category, deviceId, allrelevantCounters);
      ...
      // mgb:ss make this conditional (for now) on not being a counter category
      if ( category != AMDT_PWR_CATEGORY_COUNT ){
