        ...
        GT_IF_WITH_ASSERT(pItemData != nullptr)
        {
            ...
            GT_IF_WITH_ASSERT(pTreeItemData != nullptr)
            {
                if (ProfileApplicationTreeHandler::instance()->FindSessionChildItemData(pItemData, AF_TREE_ITEM_PP_TIMELINE) == nullptr)
                {
                    ...
                    //++TLRS: LPGPU2: Adding new items in the power profiling explorer tree
                    auto *pItemData3 = new (std::nothrow) afApplicationTreeItemData;                    
                    pItemData3->m_itemType = AF_TREE_ITEM_PP_SOURCES;
                    pItemData3->m_filePath = pItemData->m_filePath;
                    pItemData3->m_filePathLineNumber = pItemData->m_filePathLineNumber;
                    pItemData3->setExtendedData(pTreeItemData);
                    pItem = m_pApplicationTree->addTreeItem(PP_STR_TreeNodeSources, pItemData3, pTreeItem);
                    GT_IF_WITH_ASSERT(pItem != nullptr)
                    {
                        auto *pSourcesIcon = ProfileApplicationTreeHandler::instance()->TreeItemTypeToPixmap(pItemData2->m_itemType);
                        pItem->setIcon(0, QIcon{ *pSourcesIcon });
                    }

                    auto pItemData4 = new (std::nothrow) afApplicationTreeItemData;
                    pItemData4->m_itemType = AF_TREE_ITEM_PP_FEEDBACK;
                    pItemData4->m_filePath = pItemData->m_filePath;
                    pItemData4->m_filePathLineNumber = pItemData->m_filePathLineNumber;
                    pItemData4->setExtendedData(pTreeItemData);
                    pItem = m_pApplicationTree->addTreeItem(PP_STR_TreeNodeFeedback, pItemData4, pTreeItem);
                    GT_IF_WITH_ASSERT(pItem != nullptr)
                    {
                        auto *pFeedbackIcon = ProfileApplicationTreeHandler::instance()->TreeItemTypeToPixmap(pItemData2->m_itemType);
                        pItem->setIcon(0, QIcon{ *pFeedbackIcon });
                    }

                    auto pItemData5 = new (std::nothrow) afApplicationTreeItemData;
                    pItemData5->m_itemType = AF_TREE_ITEM_PP_COMPARE;
                    pItemData5->m_filePath = pItemData->m_filePath;
                    pItemData5->m_filePathLineNumber = pItemData->m_filePathLineNumber;
                    pItemData5->setExtendedData(pTreeItemData);
                    pItem = m_pApplicationTree->addTreeItem(PP_STR_TreeNodeCompareToSession, pItemData5, pTreeItem);
                    GT_IF_WITH_ASSERT(pItem != nullptr)
                    {
                        auto *pCompareIcon = ProfileApplicationTreeHandler::instance()->TreeItemTypeToPixmap(AF_TREE_ITEM_PP_TIMELINE);
                        pItem->setIcon(0, QIcon{ *pCompareIcon });
                    }
                    //--TLRS: LPGPU2: Adding new items in the power profiling explorer tree
